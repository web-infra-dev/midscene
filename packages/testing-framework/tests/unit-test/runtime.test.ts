...
describe('registerMidsceneSuite', () => {
  ...
  it('discovers yaml cases, shares one agent, and writes per-case results + summary', async () => {
    ...
    // The same summary is the runner's hand-off channel in resultDir.
    const handoff = JSON.parse(
      readFileSync(join(resultDir, 'summary.json'), 'utf8'),
    );
    expect(handoff).toMatchObject({ total: 2, passed: 2, failed: 0 });
  });

  it('records a suite-level error in the summary when setup fails', async () => {
    const root = makeProject({ 'a.yaml': 'flow:\n  - aiAct: do a\n' });
    const resultDir = join(root, 'results');
    const { api, beforeAlls, afterAlls } = createFakeRstest();
    ...
    // beforeAll rethrows so Rstest still fails the suite...
    await expect(beforeAlls[0]()).rejects.toThrow('setup boom');
    // ...but afterAll still writes a summary that explains the 0-case run.
    for (const hook of afterAlls) await hook();
    ...
    expect(summary.suiteError).toContain('setup boom');
  });
  ...
  it('fails remaining cases fast once testRunner.bail is reached', async () => {
    const root = makeProject({
      'a.yaml': 'flow:\n  - aiAct: do a\n',
      'b.yaml': 'flow:\n  - aiAct: do b\n',
    });
    ...
    // First case actually runs (and fails); the second is bailed without
    ...
    expect(summary).toMatchObject({ total: 2, failed: 2 });
    expect(
      summary.results.find((r: { testName: string }) =>
        r.testName.endsWith('b.yaml'),
      ).error,
    ).toContain('Bailed');
  });
