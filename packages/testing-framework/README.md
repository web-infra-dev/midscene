...
agent) and honors `testRunner.testTimeout` / `retry` per case. Native
`.test.ts` files and `testRunner.bail` / `maxConcurrency` are applied in the
emitted Mode B project, where Rstest's own `include` and config control them.
