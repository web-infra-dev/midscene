...
export async function agentFromIOSSimulator(
  ...
  if (deviceName) {
    ...
    if (!targetDevice) {
      throw new Error(`Simulator with name containing "${deviceName}" not found`);
    }
  } else {
    ...
    if (!targetDevice) {
      ...
      if (targetDevice) {
        debugAgent('No booted simulator found, using first available: %s', targetDevice.name);
      }
    }
  }
  ...
  debugAgent('Using iOS simulator: %s (udid = %s)', targetDevice.name, targetDevice.udid);
