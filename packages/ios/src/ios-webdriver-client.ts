...
export class IOSWebDriverClient extends WebDriverClient {
  ...
  /**
   * Get the current device time from the iOS device.
   * Uses the Appium XCUITest driver's device time endpoint.
  ...
  async getTimestamp(format?: string): Promise<number> {
    ...
    try {
      // Try Appium's device system time endpoint
      const response = await this.makeRequest(
        'GET',
        `/session/${this.sessionId}/appium/device/system_time`,
      );

      const timeString = response.value || response;
      debugIOS(`Got device time string: ${timeString}`);

      // Parse the time string to timestamp
      const timestamp = new Date(timeString).getTime();
      if (Number.isNaN(timestamp)) {
        throw new Error(`Invalid time format received: ${timeString}`);
      }

      return timestamp;
    } catch (error) {
      debugIOS(`Appium device time endpoint failed: ${error}`);

      // Fallback: Try mobile: getDeviceTime execute method
      try {
        const response = await this.makeRequest(
          'POST',
          `/session/${this.sessionId}/execute`,
          {
            ...
            args: [{ format: format || 'YYYY-MM-DDTHH:mm:ssZ' }],
          },
        );

        const timeString = response.value || response;
        debugIOS(`Got device time via execute: ${timeString}`);
        ...
        debugIOS(`Execute getDeviceTime also failed: ${executeError}`);
        throw new Error(
          `Failed to get device time: ${error}. Execute fallback also failed: ${executeError}`,
        );
      }
    }
  }
