...
async function getSimulatorDevices(): Promise<SimulatorDevice[]> {
  try {
    const { stdout } = await executeCommand('xcrun', [
      'simctl',
      'list',
      'devices',
      '--json',
...
async function startWebDriverAgent(
  wdaPath: string,
  udid: string,
): Promise<void> {
...
async function prepareWebDriverAgent() {
  try {
    ...
    if (shouldStart) {
      ...
      console.log(
        '      npx @midscene/ios prepare --start --udid=YOUR_SIMULATOR_UDID',
      );
      console.log('');
      ...
      console.log('');
      console.log(
        '🎯 Goal achieved: You can now run automated tests with Midscene iOS!',
      );
    }
  } catch (error) {
    console.error(
