...
function screenshotBuffer(base64: string): Buffer {
  ...
  if (!match) {
    throw new Error('iOS TodoMVC screenshot is not a base64 data URL');
  }
...
describe('Test todo list', () => {
  ...
  beforeAll(async () => {
    ...
    agent = await agentFromWebDriverAgent({
      ...
      wdaHost: '127.0.0.1',
      reportFileName: 'todo-mvc-ios',
      autoPrintReportMsg: false,
      aiActionContext:
        'If any browser, permission, user agreement, or cookies popup appears, close or accept it. Operate only the visible TodoMVC page.',
    });
  ...
  afterAll(async () => {
    ...
    if (diagnosticsDir) {
      try {
        ...
        const source = (await agent.runWdaRequest({
          method: 'GET',
          endpoint: '/source',
        })) as { value: string };
        await Promise.all([
