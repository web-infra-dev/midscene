...
export interface NodeSpecDocument {
  markdown: string;
...
interface NodeSpecLocation {
  configPath?: string;
  projects: readonly { name: string; files: TestFileSelection }[];
}

const renderWorkflowLocations = (location?: NodeSpecLocation): string => {
  const sections = [
    '## Where to use these Nodes',
    'Use these Nodes in Workflow Documents (`.yaml` or `.yml` files) selected by an Execution Project. Case file patterns and the config file path below are relative to this spec file. Case file patterns use glob syntax. A custom `--config` path does not change where case files are selected.',
...
export const renderNodeSpec = (
  nodes: readonly NodeDefinition<any, any, any>[],
  location?: NodeSpecLocation,
): NodeSpecDocument => {
  const warnings: string[] = [];
  const sortedNodes = sortNodesForSpec(nodes);
  const sections = [
    GENERATED_COMMENT,
    '# Midscene Test Node Spec',
