...
import { defaultLegacyConfig as defaultConfig } from './legacy-config';

// Legacy compatibility: one argument parser for both command entry points.
...
export const parseLegacyArguments = (
  rawArgs: string[],
  options: { scriptName?: string; version?: string; help?: boolean } = {},
): {
  path?: string;
  files?: string[];
  ...
  const args = yargs(rawArgs)
    .scriptName(options.scriptName ?? 'midscene')
    .exitProcess(false)
    .fail((message, error) => {
...
Usage:
    ...
    .options({
      ...
      retry: {
        type: 'number',
        description: `Number of extra attempts for the complete failed YAML file, including tasks that already passed; default is ${defaultConfig.retry}`,
      },
      headed: {
        type: 'boolean',
    ...
    .version('version', 'Show version number', options.version ?? 'unknown')
    .help()
...
Examples:
  ...
  if (options.help === false) args.help(false).version(false);
