...
export interface CreateYamlPlayerOptions {
  /** Borrow an existing Agent; its owner remains responsible for teardown. */
  agent?: Agent;
  headed?: boolean;
  keepWindow?: boolean;
  /** Puppeteer handles. Platform-specific types are optional for other hosts. */
  browser?: object;
  page?: object;
...
export interface YamlStaticServer {
  server: Server;
  listen(port: number, host: string, callback: () => void): void;
}

export const launchServer = async (dir: string): Promise<YamlStaticServer> => {
...
export type YamlAgentConfig = Omit<MidsceneYamlScript, 'tasks'>;

/** Launch any YAML-supported platform for a native Test setup or another host. */
export async function createYamlAgent(
  file: string,
  script: YamlAgentConfig,
  options?: CreateYamlPlayerOptions,
  reportFileName?: string,
): Promise<{ agent: Agent; freeFn: FreeFn[] }> {
  if (options?.agent) return { agent: options.agent, freeFn: [] };
  // Keep optional driver types inside the selected platform boundary.
  const webOptions = options as
    | (Omit<CreateYamlPlayerOptions, 'browser' | 'page' | 'browserContext'> & {
        browser?: Browser;
        page?: Page;
        browserContext?: BrowserContext;
      })
    | undefined;
  const clonedYamlScript = structuredClone(script);
  const fileName = basename(file, extname(file));
  const preference = {
    headed: options?.headed,
    keepWindow: options?.keepWindow,
  ...
  try {
    ...
    if (targetCount > 1) {
      const specifiedTargets = [
        ...
        typeof clonedYamlScript.interface !== 'undefined' ? 'interface' : null,
      ].filter(Boolean);
    ...
    if (typeof webTarget !== 'undefined') {
      const { buildDownloadBehavior, puppeteerAgentForTarget } = await import(
      ...
      if (webTarget.serve) {
        ...
        assert(
          ...
          'Local server did not bind a TCP address',
        );
      ...
      if (webTarget.cdpEndpoint) {
        ...
        const ownsCdpBrowser = !webOptions?.browser;
        const cdpBrowser =
          webOptions?.browser ??
        ...
        let launchedAgent: Awaited<ReturnType<typeof puppeteerAgentForTarget>>;
        try {
    ...
    if (typeof clonedYamlScript.interface !== 'undefined') {
      ...
      if (
        ...
        const resolvedPath = path.resolve(process.cwd(), moduleSpecifier);
    ...
    const cleanupErrors = await cleanupFailedPlayerSetup(freeFn);
    if (cleanupErrors.length)
      throw new AggregateError(
        [error, ...cleanupErrors],
        'YAML setup and cleanup failed',
      );
    throw error;
  }
}
