...
| Phase 3 产品化/车机化 | 可靠性、性能、安全 | 达到内部工具发布标准并确定权限模型 | 3–6 周 | 🔄 进行中（可视化 ✅ / 自检 ✅ / 部署 ✅；安全与真机矩阵待做） |
...
| ~~B2~~ ✅ | ~~浮层不进截图~~ | 已实现（方案 B：`setSkipScreenshot`）并有机制级证据；**像素级回归仍待做**（读 overlay frame 精确裁切比对） | 小 |
| ~~B3~~ ✅ | ~~多诊断浮层~~ | 已落地为**单个全屏 surface** 承载四种可视化（状态栏 / 边框流光 / 元素框 / 涟漪），一次 `setSkipScreenshot` 覆盖全部，Settings 五个开关 | — |
| **B12** | **存储治理（新增，优先级高）** | 报告内嵌逐帧截图（约 3.9MB/份），20+ 次运行即吃掉 ~91MB，曾导致安装静默失败（A14-40）；计划：History 增加"保留最近 N 条"清理入口 + 写入时压缩截图 | 小–中 |
| **B13** | **自检脚本作为回归基线（新增）** | `self-check.yaml`（点 tab + 填输入框）已在 App 内一键运行；可把各步耗时记录下来做性能回归 | 小 |
| **B14** | **精确点击点（新增）** | 涟漪当前在无 point 时用 bbox 中心回退；进度总线可用后取真实 `point` | 小 |
