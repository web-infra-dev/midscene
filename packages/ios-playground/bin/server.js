...
const staticDir = path.join(__dirname, '..', '..', '..', 'apps', 'ios-playground', 'dist');
...
const startAutoServer = async () => {
    try {
        ...
        if (isRunning) {
            console.log(`✅ PyAutoGUI server is already running on port ${AUTO_SERVER_PORT}`);
            return true;
        }
        ...
        const autoServerPath = path.join(__dirname, '..', '..', 'ios', 'bin', 'server.js');
        ...
        if (isNowRunning) {
            console.log(`✅ PyAutoGUI server started successfully on port ${AUTO_SERVER_PORT}`);
            return true;
        } else {
            console.error(`❌ Failed to start PyAutoGUI server on port ${AUTO_SERVER_PORT}`);
            return false;
        }
