...
Usage:
  MIDSCENE_EXPERIMENTAL_NATIVE_XPATH_CACHE=1 pnpm exec tsx packages/android/scripts/accessibility-xpath-audit.ts capture \\
  ...
  MIDSCENE_EXPERIMENTAL_NATIVE_XPATH_CACHE=1 pnpm exec tsx packages/android/scripts/accessibility-xpath-audit.ts render --run-id <run-id>
...
export async function main(args = process.argv.slice(2)): Promise<void> {
  ...
  if (!isNativeXpathCacheEnabled()) {
    throw new Error(
      'Native XPath cache is disabled. Set MIDSCENE_EXPERIMENTAL_NATIVE_XPATH_CACHE=1 so the audit matches production cache behavior.',
    );
  }
