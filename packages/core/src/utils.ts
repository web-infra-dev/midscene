...
// carries `module.require` — using that property access (rather than the
// bare `require(...)` form) keeps downstream bundlers like rspack from
// statically picking up the dependency when they scan this module. In ESM
// Node and in the browser `module` is undefined, so the `typeof` guard
// short-circuits and `getReportTpl()` falls back to an empty template.
function getRuntimeRequire(): NodeRequire | null {
  if (cachedRuntimeRequire !== undefined) {
    return cachedRuntimeRequire;
  }
  try {
    if (typeof module === 'object' && module !== null) {
      const candidate = module.require;
      if (typeof candidate === 'function') {
        // `Module.prototype.require` resolves relative paths against
        // `this.filename`, so we have to keep the module binding when we
        // hand the function out as a plain `NodeRequire`.
        cachedRuntimeRequire = candidate.bind(module) as NodeRequire;
        return cachedRuntimeRequire;
      }
    }
  } catch {
    // Intentional fall-through — treated as "no runtime require available".
  }
  cachedRuntimeRequire = null;
  return cachedRuntimeRequire;
}
...
// (~5 MB with embedded react/react-dom), and a static import would drag
// that blob into every downstream bundle that transitively reaches
// `@midscene/core`'s barrel exports (e.g. the Studio renderer via
// `@midscene/playground`'s `LocalExecutionAdapter`). Fetching via a
// runtime-resolved `require` + a non-literal module id keeps bundlers from
...
export function getReportTpl() {
  ...
  if (cachedReportTpl !== null) {
    return cachedReportTpl;
  }

  cachedReportTpl = loadReportTplFromSiblingModule();
  return cachedReportTpl;
}
