...
export function filterPlanningReplay(
  ...
  actionOutputTagNames: readonly string[] = [],
): string {
  ...
  const protectedTags = new Set([
    ...
    ...actionOutputTagNames.map((tag) => tag.toLowerCase()),
  ]);
  const tagNames = [
    ...Object.values(componentTags).flat(),
    ...protectedTags,
  ].map((tag) => tag.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
  const tokens = new RegExp(`<\\/?(${tagNames.join('|')})\\b[^>]*>`, 'gi');
  let output = '';
  let cursor = 0;
