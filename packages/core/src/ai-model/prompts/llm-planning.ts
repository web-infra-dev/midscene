...
const vlLocateParam = (
  ...
  if (includeBbox) {
    ...
    return describeLocateParamSchema(responseFormat);
  }
...
const injectBboxIntoSample = (
  ...
  const resultKey = responseFormat?.resultType ?? 'bbox';
  const sampleResults = responseFormat
    ? SAMPLE_BBOXES.map((bbox) =>
  ...
  for (const field of locateFields) {
    if (
      ...
      result[field] = {
        ...result[field],
        [resultKey]: sampleResults[bboxIndex % sampleResults.length],
      };
      bboxIndex++;
    }
  }
...
export const descriptionForAction = (
  ...
  includeBbox = false,
  locateResponseFormat?: LocateResultResponseFormat,
) => {
  const tab = '  ';
  const fields: string[] = [];
...
export async function systemPromptToTaskPlanning({
  ...
  const locateResponseFormat = includeBbox
    ? getStandardLocateResultAdapter(modelFamily).responseFormat
    : undefined;
  ...
  const actionDescriptionList = actionSpace.map((action) => {
    return descriptionForAction(
      action,
      vlLocateParam(modelFamily, includeBbox),
      includeBbox,
      locateResponseFormat,
    );
  });
  ...
  if (includeBbox && !locateResponseFormat) {
    throw new Error(
      'locateResponseFormat is required when includeBbox is true',
    );
  }

  // Generate locate object examples based on includeBbox
  const locateExample1 =
    includeBbox && locateResponseFormat
      ? `{
    "prompt": "Add to cart button for Sauce Labs Backpack",
    ${locateResultExampleJsonEntry(locateResponseFormat, [345, 442, 458, 483])}
  }`
      : `{
    "prompt": "Add to cart button for Sauce Labs Backpack"
  }`;
  ...
  const locateNameField =
    ...
    ${locateResultExampleJsonEntry(locateResponseFormat, [120, 180, 380, 210])}
  }`
      : `{
    "prompt": "Name input field in the registration form"
  }`;
  ...
  const locateEmailField =
    ...
    ${locateResultExampleJsonEntry(locateResponseFormat, [120, 240, 380, 270])}
  }`
      : `{
    "prompt": "Email input field in the registration form"
  }`;
