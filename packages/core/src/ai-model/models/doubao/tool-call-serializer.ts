...
type SerializedDoubaoParameter = {
  name: string;
  // `false|true` is a prompt placeholder only and is never emitted by a real tool call.
  stringAttribute: 'true' | 'false' | 'false|true';
  content: string;
};

type SerializedDoubaoParameterValue = Omit<SerializedDoubaoParameter, 'name'>;

export const serializeDoubaoToolCall = ({
  functionName,
  parameters,
}: {
  functionName: string;
  parameters: SerializedDoubaoParameter[];
}) => {
  const serializedParameters = parameters
    .map(
      ({ name, stringAttribute, content }) =>
        `<parameter name="${escapeXmlAttribute(name)}" string="${stringAttribute}">${content}</parameter>`,
    )
    .join('');

  return `<${SEED_TOOL_CALL_TAG_NAME}><function name="${escapeXmlAttribute(functionName)}">${serializedParameters}</function></${SEED_TOOL_CALL_TAG_NAME}>`;
};

const serializeParameterValue = (
  value: unknown,
): SerializedDoubaoParameterValue => {
...
export const buildDoubaoPlanningActionOutput = ({
  ...
  const parameters = Object.entries(param)
    ...
    .map(([name, value]) => {
      const serializedValue: SerializedDoubaoParameterValue =
        locateFieldSet.has(name)
          ? {
              stringAttribute: 'true',
      ...
      return {
        name,
        stringAttribute: serializedValue.stringAttribute,
        content: serializedValue.content,
      };
    });

  return serializeDoubaoToolCall({ functionName: actionName, parameters });
};
