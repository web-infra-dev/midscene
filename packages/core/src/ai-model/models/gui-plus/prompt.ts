...
export const getGuiPlus20260226ComputerUseLocatePrompt = (): string => {
  return `# Tools
...
<tools>
{
  ...
  "function": {
    "name": "computer_use",
    "description": "Use a mouse to locate and click a target UI element from a computer screenshot.\\n* The screen's resolution is 1000x1000.\\n* Your only goal is to click the UI element specified by the user.\\n* Click the center of the target element. Don't click boxes on their edges unless asked.",
    "parameters": {
      "properties": {
        "action": {
          "description": "The action to perform. The available actions are:\\n* \`left_click\`: Click the left mouse button at a specified (x, y) pixel coordinate on the screen.\\n* \`terminate\`: Terminate when the target element cannot be found.",
          "enum": ["left_click", "terminate"],
          "type": "string"
        },
        "coordinate": {
          "description": "(x, y): The x (pixels from the left edge) and y (pixels from the top edge) coordinates to click. Required only by \`action=left_click\`.",
          "type": "array"
        },
        "status": {
          "description": "The status of the task. Required only by \`action=terminate\`.",
...
Response format for every step:
1) Action: a short imperative describing what to click in the UI.
2) A single <tool_call>...</tool_call> block containing only the JSON: {"name": <function-name>, "arguments": <args-json-object>}.

Rules:
- Output exactly in the order: Action, <tool_call>.
- For a visible target element, always use action=left_click with coordinate.
- Do not type text, scroll, navigate, or perform any other action.
- If the target element cannot be found, use action=terminate with status=failure.`;
};
