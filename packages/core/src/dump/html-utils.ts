...
/**
 * Extract ALL dump script contents and their attributes from an HTML file using streaming.
 * Memory usage: O(total_dump_size), not O(file_size).
 *
 * @param filePath - Absolute path to the HTML file
 * @returns Array of { content, attributes } for each dump script tag found
 */
export function extractAllDumpScriptsSync(
  ...
  const openTagPrefix = '<script type="midscene_web_dump"';
  const closeTag = '</script>';
  const results: { content: string; attributes: Record<string, string> }[] = [];
            ...
            if (tagEndIdx !== -1) {
              ...
              if (endIdx !== -1) {
                results.push({
                  ...
                  attributes: parseOpenTagAttributes(currentOpenTag),
                });
                capturing = false;
                currentContent = '';
                currentOpenTag = '';
...
/**
 * Parse attributes from an opening tag string like `<script type="midscene_web_dump" data-group-id="foo">`.
 */
function parseOpenTagAttributes(openTag: string): Record<string, string> {
