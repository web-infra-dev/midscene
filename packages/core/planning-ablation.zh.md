...
组件实验支持默认 Midscene Planning 协议和本分支的 Qwen 标准 Planning 协议。其他替换核心构造函数的标准协议，必须支持描述裁剪和保留动作内容的历史回放，才能显式声明支持实验。自定义 Planning 在未设置组件屏蔽时沿用自己的协议；显式组件屏蔽会在推理前报错。要求原样回放 assistant message 的适配器不支持字段消融；自定义 Locate 不支持 `groundingGuidance` 消融。
...
本分支基于 `077a089dce46fafad26e0c720dae74b100234453`。`@midscene/core@1.12.4-beta-20260903111111.0` 的 source map 中可提取的 148 个 Core 源码文件与该提交逐字节一致；`1.12.6-beta-20260910064536.0` 中对应的 150 个文件与 `12fd6861ce329223cf5d2e2a77a06d56ebc5c6ad` 一致。Beta 版本号由发布流程赋值，源码 checkout 保留原来的 package 版本号。

移植包含 `edef31e49`、`612b4b0d9`、`b65a604b8`、`476efc611` 和 `12fd6861c`。冲突处理保留旧版用户上下文实现；移除模式参数的改动应用到搬迁前的 `packages/test/src/midscene/index.ts`。不包含后续的 sub-goal prompt 改造。

使用 `qwen3`（或 `qwen3.5`、`qwen3.6`）模型族时，启用 Qwen 文本工具调用和归一化点坐标；`qwen3-vl` 仍使用默认协议。这里的文本工具调用不启用供应商原生 function calling。Qwen 动作和嵌套参数描述遵循组件开关，历史回放保留工具参数中字面上的框架标签。
...
如配置独立 Planning 模型，还需将其 `MIDSCENE_PLANNING_MODEL_FAMILY` 设为 `qwen3`。独立 Locate 保留 Qwen 的 `computer_use` 协议；关闭 `separateLocate` 时直接使用 Planning 输出的 Qwen 点坐标。测试验证协议和组件行为，不代表 benchmark 成绩。
