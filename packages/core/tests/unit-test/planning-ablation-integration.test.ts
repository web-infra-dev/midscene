...
import { buildQwenPlanningActionOutput } from '@/ai-model/models/qwen/tool-call';
...
import { USER_REQUEST_ONLY_GUIDANCE } from '@/device/action-guidance';
...
describe.each(['qwen3-vl', 'qwen3'] as const)(
  'Planning ablation across model calls: %s',
  (modelFamily) => {
    const options = (disabled: string) => createOptions(disabled, modelFamily);
    const protocolAction =
      modelFamily === 'qwen3'
        ...
        : actionOutput;
    const protocolResponse = response.replace(actionOutput, protocolAction);
    beforeEach(() => {
    ...
    for (let mask = 0; mask < 16; mask++) {
      ...
      it(`projects state and replay for ${disabled.join(',') || 'baseline'}`, async () => {
        ...
        expect(first.rawResponse).toBe(protocolResponse);
        ...
        for (const [part, sentinel] of [
          ...
          expect(requestText().includes(sentinel)).toBe(
            !disabled.includes(part),
          );
        }
    ...
    it('filters retry responses and keeps the original raw response for reporting', async () => {
      ...
      rs.mocked(callAI).mockResolvedValueOnce({
        content:
          '<memory>MEMORY_SENTINEL</memory><log>LOG_SENTINEL</log>' +
          (modelFamily === 'qwen3'
            ? buildQwenPlanningActionOutput({
                actionName: 'Input',
                param: { locate: 'invalid locator' },
              })
            : '<action-type>Input</action-type><action-param-json>{invalid json}</action-param-json>'),
        isStreamed: false,
      });
      const result = await run(opts);
      expect(rs.mocked(callAI)).toHaveBeenCalledTimes(2);
      expect(requestText()).not.toMatch(/MEMORY_SENTINEL|LOG_SENTINEL/);
      expect(result.rawResponse).toBe(protocolResponse);
    ...
    for (let mask = 0; mask < 8; mask++) {
      const disabled = [
        ...(mask & 1 ? ['log', 'subGoals'] : []),
        ...(mask & 2 ? ['taskScope'] : []),
        ...(mask & 4 ? ['inputVerification'] : []),
      ];
      it(`applies the LS/T/I combination ${disabled.join(',') || 'all-on'}`, async () => {
        const opts = options(disabled.join(','));
        await run(opts);
        const messages = rs.mocked(callAI).mock.calls.at(-1)![0];
        const prompt = String(messages[0].content);
        expect(prompt.includes('<update-plan-content>')).toBe(!(mask & 1));
        expect(prompt.includes('LOG_SENTINEL')).toBe(false);
        expect(prompt.includes(USER_REQUEST_ONLY_GUIDANCE)).toBe(!(mask & 2));
        expect(
          prompt.includes('Input verification after an input action'),
        ).toBe(!(mask & 4));
        expect(prompt.includes('<tool_call>')).toBe(modelFamily === 'qwen3');
        expect(prompt.includes('<action-type>')).toBe(modelFamily !== 'qwen3');
      });
    }
  },
);
