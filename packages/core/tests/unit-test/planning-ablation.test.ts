...
describe('Planning ablation configuration', () => {
  ...
  it('allows the Qwen protocol and still rejects unverified custom builders', () => {
    ...
    expect(() =>
      validatePlanningAblation(PLANNING_ABLATION_PARTS, model),
    ).not.toThrow();
    if (model.adapter.planning.kind !== 'standard')
      throw new Error('Expected standard planning');
    model.adapter = {
      ...model.adapter,
      planning: {
        ...model.adapter.planning,
        protocol: {
          ...model.adapter.planning.protocol,
          supportsPlanningAblation: false,
        },
      },
    };
    expect(() => validatePlanningAblation(['log'], model)).toThrow(
      'custom standard-planning protocol',
    );
  });
