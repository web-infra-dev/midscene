...
describe('Doubao Function Definition', () => {
  ...
  it('preserves union branches from built-in action schemas', () => {
    ...
    expect(inputDescription.parameters.properties.value.anyOf).toEqual([
      { type: 'string' },
      { type: 'number' },
    ]);
    expect(fileChooserDescription.parameters.properties.files.anyOf).toEqual([
      { type: 'string' },
      {
        type: 'array',
        items: { type: 'string' },
  ...
  it('recursively describes nested objects and arrays', () => {
    const description = buildDoubaoActionDescription({
      action: {
        name: 'Configure',
        description: 'Configure an operation',
        paramSchema: z.object({
          options: z.object({
    ...
    expect(description.parameters).toEqual({
      type: 'object',
      properties: {
        options: {
          type: 'object',
          ...
          required: ['duration'],
          description: 'Parameter options for Configure.',
        },
      },
      required: ['options'],
    });
