...
describe('bbox locate cache fix', () => {
  ...
  it('should annotate AI locate usage with semantic default intent while preserving raw resolved intent', async () => {
    ...
    expect(
      runtimeTask.log?.dump?.taskInfo?.usage?.semantic_intent,
    ).toBeUndefined();
  });
