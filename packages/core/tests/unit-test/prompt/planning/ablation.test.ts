...
const qwenRuntime = getModelRuntime({
  ...runtime.config,
  modelFamily: 'qwen3',
});
if (qwenRuntime.adapter.planning.kind !== 'standard')
  throw new Error('Expected standard Qwen planning');
const qwenPlanning = qwenRuntime.adapter.planning;
...
describe('independent Planning prompt parts', () => {
  for (const [part, fragments] of Object.entries(removedFragments)) {
    it(`removes ${part} from the Qwen prompt and tool descriptions`, async () => {
      const build = (disabled: string) =>
        buildStandardPlanningSystemPrompt({
          actionSpace: actions,
          includeLocateInPlanning: true,
          locatePromptSpec: qwenPlanning.locateResultCodec!.promptSpec,
          planningProtocol: qwenPlanning.protocol,
          ablation: parsePlanningAblation(disabled),
        });
      const before = await build('');
      const after = await build(part);
      for (const fragment of fragments) {
        expect(before).toContain(fragment);
        expect(after).not.toContain(fragment);
      }
      expect(after).toContain('<tool_call>');
      expect(after).toContain('<function=Tap>');
      expect(after).not.toContain('<action-type>');
      expect(after).toContain('"name":"Input"');
    });
  }
