...
const recordReplayScene = process.env.RECORD_FAST_REPLAY_SCENE ?? 'login-popup';
const skipRecordReplayLogin =
  !process.env.RECORD_FAST_REPLAY_ROLE || recordReplayScene !== 'login-popup';
...
it.skipIf(skipRecordReplayLogin)(
...
it.skipIf(
  !process.env.RECORD_FAST_REPLAY_ROLE ||
    recordReplayScene !== 'transient-retry',
)('records one recoverable retry from record to fast replay', async () => {
  const role = parseRole(process.env.RECORD_FAST_REPLAY_ROLE);
  const round = process.env.RECORD_FAST_REPLAY_ROUND ?? '1';
  const experimentId =
    process.env.RECORD_FAST_REPLAY_EXPERIMENT_ID ??
    'record-fast-replay-transient-retry';
  const modelName = process.env.MIDSCENE_MODEL_NAME ?? 'unknown-model';
  const modelArtifactId = modelName
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '');
  const artifactId = `${modelArtifactId}-${experimentId}-${role}-transient-retry-r${round}`;
  const reportFileName = `record-fast-replay-${artifactId}`;
  const reportRoot = path.join(process.cwd(), 'midscene_run', 'report');
  const evidenceDir = path.join(reportRoot, 'record-fast-replay-evidence');
  await mkdir(evidenceDir, { recursive: true });
  ...
  const fixturePath = path.resolve(
    __dirname,
    '../playwright/__fixtures__/record-retry-replay/index.html',
  );
  const benchmarkPath = path.join(
    process.cwd(),
    'tests',
  ...
  const agent = new PuppeteerAgent(originPage, {
    ...
    outputFormat: 'single-html',
    reportFileName,
    groupName: `Record retry to fast replay evidence: ${artifactId}`,
    groupDescription:
      'The first click is deterministically blocked; the agent must observe the changed state and retry the same deployment.',
    ...(role === 'record'
      ? {
          cache: {
            id: artifactId,
  ...
  const prompt = 'Deploy Project Atlas and finish when its status is Deployed.';
  const startedAt = performance.now();
  let runError: unknown;
  try {
  ...
  const pageEvidence = await originPage.evaluate(() => {
    const targetWindow = window as typeof window & {
      __recordRetryAttempts: number;
      __recordRetryEvents: string[];
    };
    const status = document.querySelector<HTMLElement>('#atlas-status');
    const success = document.querySelector<HTMLElement>('#success');
    const button = document.querySelector<HTMLButtonElement>('#deploy-atlas');
    return {
      deploymentState: document.body.dataset.deploymentState,
      attempts: targetWindow.__recordRetryAttempts,
      events: targetWindow.__recordRetryEvents,
      statusText: status?.textContent?.trim(),
      statusState: status?.dataset.state,
      successVisible: success?.dataset.visible === 'true',
      buttonDisabled: button?.disabled,
    };
  });
  const expectedEvents = [
    'atlas-attempt-1-blocked',
    'atlas-attempt-2-succeeded',
  ];
  const controlledRetryObserved =
    pageEvidence.attempts === 2 &&
    JSON.stringify(pageEvidence.events) === JSON.stringify(expectedEvents);
  const stateMatched =
    pageEvidence.deploymentState === 'deployed' &&
    pageEvidence.statusText === 'Deployed' &&
    pageEvidence.statusState === 'deployed' &&
    pageEvidence.successVisible &&
    pageEvidence.buttonDisabled === true;
  const xpathReplayMatched =
    role === 'record' ||
    (locate.locateTasks === 2 &&
      locate.xpathHits === 2 &&
      locate.aiLocateUsages === 0);
  ...
  context.agent = null;
  const generatedReportPath = path.join(reportRoot, `${reportFileName}.html`);
  const copiedReportPath = path.join(evidenceDir, `${artifactId}.report.html`);
  await copyFile(generatedReportPath, copiedReportPath);
  const reportSha256 = createHash('sha256')
    .update(await readFile(copiedReportPath))
    .digest('hex');
  const recordMapArtifact =
    role === 'record' && !runError && stateMatched && controlledRetryObserved
      ? await generateRecordXpathMap(
          copiedReportPath,
          path.join(evidenceDir, 'record-maps', artifactId),
        )
  ...
  const evidence = {
    ...
    gitCommit: process.env.RECORD_FAST_REPLAY_COMMIT,
    experimentId,
    scene: 'transient-retry',
    detourType: 'page-induced-same-target-retry',
    role,
    round,
    prompt,
    model: {
  ...
  if (runError) throw runError;
  expect(stateMatched).toBe(true);
  expect(controlledRetryObserved).toBe(true);
  expect(xpathReplayMatched).toBe(true);
  expect(tasks.actionTypes.filter((type) => type === 'Tap')).toHaveLength(2);
});
