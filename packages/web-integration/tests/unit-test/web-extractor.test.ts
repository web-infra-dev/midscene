...
describe(
  ...
  () => {
    ...
    describe('iframe locator E2E tests', () => {
      it('getXpathsByPoint should penetrate same-origin iframe and return compound xpath', async () => {
        const { page, reset } = await launchPage(`http://127.0.0.1:${port}`, {
          viewport: {
            width: 1080,
            height: 3000,
        ...
        // The same-origin iframe contains child.html with <h1>Child Page</h1>
        // Get the iframe's bounding rect to find a point inside it
        const iframeRect = await page.evaluateJavaScript?.(
          `JSON.parse(JSON.stringify(document.querySelector('iframe[src="child.html"]').getBoundingClientRect()))`,
        );

        expect(iframeRect).toBeDefined();

        // Click a point inside the iframe (offset from iframe top-left to hit the h1)
        const pointInIframe = {
          left: Math.round(iframeRect.left + 50),
          top: Math.round(iframeRect.top + 20),
        };

        const xpaths = await page.evaluateJavaScript?.(
          `${scriptContent}midscene_element_inspector.getXpathsByPoint(${JSON.stringify(pointInIframe)}, true)`,
        );

        expect(xpaths).toBeDefined();
        expect(xpaths).toHaveLength(1);
        // Should contain |>>| separator indicating iframe traversal
        expect(xpaths[0]).toContain('|>>|');
        // Should reference iframe element
        expect(xpaths[0]).toMatch(/iframe\[\d+\]/);

        await reset();
      });

      it('getNodeInfoByXpath should resolve compound xpath to iframe inner element', async () => {
        const { page, reset } = await launchPage(`http://127.0.0.1:${port}`, {
          viewport: {
            width: 1080,
            height: 3000,
        ...
        // First get the compound xpath by clicking inside the iframe
        const iframeRect = await page.evaluateJavaScript?.(
          `JSON.parse(JSON.stringify(document.querySelector('iframe[src="child.html"]').getBoundingClientRect()))`,
        );
        ...
        // Now resolve the compound xpath back to a node
        const nodeTag = await page.evaluateJavaScript?.(
        ...
        expect(nodeTag).not.toBeNull();

        await reset();
      });

      it('getElementInfoByXpath should return element info with correct offset for iframe element', async () => {
        const { page, reset } = await launchPage(`http://127.0.0.1:${port}`, {
          viewport: {
            width: 1080,
            height: 3000,
        ...
        // Build a known compound xpath for the button inside child.html
        // child.html has: <h1>Child Page</h1> <p>...</p> <button>Click me</button>
        const iframeRect = await page.evaluateJavaScript?.(
          `JSON.parse(JSON.stringify(document.querySelector('iframe[src="child.html"]').getBoundingClientRect()))`,
        );

        // Get the iframe's xpath first
        const iframeXpath = await page.evaluateJavaScript?.(
          `${scriptContent}midscene_element_inspector.getElementXpath(
            document.querySelector('iframe[src="child.html"]'), true, false, true
          )`,
        );

        expect(iframeXpath).toBeDefined();

        // Build compound xpath to button in child.html
        const compoundXpath = `${iframeXpath}|>>|/html/body/button[1]`;

        const elementInfo = await page.evaluateJavaScript?.(
          `${scriptContent}midscene_element_inspector.getElementInfoByXpath('${compoundXpath}')`,
        );

        expect(elementInfo).toBeDefined();
        expect(elementInfo.nodeType).toBeTruthy();

        // The rect should account for iframe offset - left/top should be greater than iframe's position
        expect(elementInfo.rect.left).toBeGreaterThanOrEqual(
          Math.floor(iframeRect.left),
        );
        expect(elementInfo.rect.top).toBeGreaterThanOrEqual(
          Math.floor(iframeRect.top),
        );

        await reset();
      });

      it('roundtrip: getXpathsByPoint → getElementInfoByXpath for iframe element', async () => {
        const { page, reset } = await launchPage(`http://127.0.0.1:${port}`, {
          viewport: {
            width: 1080,
            height: 3000,
        ...
        // Point inside iframe hitting the h1 "Child Page"
        const point = {
          left: Math.round(iframeRect.left + 50),
          top: Math.round(iframeRect.top + 20),
        };

        // Step 1: get xpath by point
        const xpaths = await page.evaluateJavaScript?.(
        ...
        // Step 2: resolve xpath to element info
        const elementInfo = await page.evaluateJavaScript?.(
          `${scriptContent}midscene_element_inspector.getElementInfoByXpath('${xpaths[0]}')`,
        );

        expect(elementInfo).toBeDefined();
        // Should have valid rect with non-zero dimensions
      ...
      it('getElementInfoByXpath should return null for invalid compound xpath', async () => {
        ...
        // Non-existent iframe index should return null
        const result = await page.evaluateJavaScript?.(
          `${scriptContent}midscene_element_inspector.getElementInfoByXpath('/html/body/iframe[99]|>>|/html/body/h1[1]')`,
        );
