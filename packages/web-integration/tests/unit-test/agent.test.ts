...
// The `with { rstest: 'importActual' }` attribute is how rstest does a partial
// mock: its factories must be synchronous, so the real module is pulled in at
// the top level and spread into the factory below. (Vitest's async
// `(importOriginal) => ...` factory has no rstest equivalent.)
