...
export class DocumentNodeRegistry {
  readonly #nodes = new Map<string, DocumentNodeDefinition<any, any>>();

  constructor(nodes: readonly DocumentNodeDefinition<any, any>[] = []) {
    for (const node of nodes) this.register(node);
  }

  register<TInput, TData>(node: DocumentNodeDefinition<TInput, TData>): this {
    defineDocumentNode(node);
    if (this.#nodes.has(node.name)) throw new DuplicateNodeError(node.name);
    this.#nodes.set(node.name, node);
    return this;
  }

  get(name: string): DocumentNodeDefinition<any, any> | undefined {
    return this.#nodes.get(name);
  }

  require(name: string): DocumentNodeDefinition<any, any> {
    const node = this.get(name);
