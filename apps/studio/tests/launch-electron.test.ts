...
async function loadLaunchScript(
  ...
  // TODO(rstest): restore await import(relativeModulePath) when variable dynamic imports become mockable (https://github.com/web-infra-dev/rstest/issues/1454). rstest's mocks
  // are injected via an rspack build-time transform, so a dynamic
  // `import(variable)` escapes the transform and loads the module via Node's
  // native ESM loader — bypassing rs.mock('node:child_process') etc. and
  // spawning a REAL Electron process. Use statically-analyzable string-literal
  // imports so rspack bundles the module and applies the mocks. Revert to
