...
export interface InfoListItem {
  id: string;
  type: 'user' | 'system' | 'result' | 'progress' | 'separator' | 'code';
  content: string;
  timestamp: Date;
  ...
  generatedCode?: import('@midscene/core').GeneratedCode;
  decomposition?: import('@midscene/core').AIActionDecomposition;
  actionType?: string;
}
...
export function BrowserExtensionPlayground({
  ...
  const handleRun = useCallback(async () => {
    ...
    if (!result.error && result.result !== undefined) {
      // Get progress steps from current session for more accurate code generation
      const currentSessionItems = infoList.filter(
        (item) =>
          item.id.includes(`-${thisRunningId}`) && item.type === 'progress',
      );

      // If we have progress steps, generate code from actual execution flow
      if (currentSessionItems.length > 0) {
        const progressSteps =
          parseProgressStepsFromInfoList(currentSessionItems);

        if (progressSteps.length > 0) {
          generatedCode = generateCodeFromProgressSteps(
            progressSteps,
            actionSpace,
            { includeComments: true, includeImports: true },
          );
        }
      }

      // Fallback to original method if no progress steps found
      if (!generatedCode) {
        const parameters = needsStructuredParams
          ? value.params || {}
          : { prompt: value.prompt };
        generatedCode = generateCode(actionType, parameters);

        // For aiAction, also generate decomposition
        if (actionType === 'aiAction' && value.prompt) {
          decomposition = decomposeAIAction(value.prompt, result.result);
        }
      }
    }

    // add result to list with integrated code generation
    const resultItem: InfoListItem = {
      ...
      // Include code generation data in the result item
      generatedCode,
      decomposition,
      actionType,
    };
                          ...
                          {item.type === 'result' ? (
                            <>
                              <PlaygroundResultView
                                ...
                                fitMode="width"
                              />
                              {/* Show code display if available */}
                              {item.generatedCode && item.actionType && (
                                <CodeDisplay
