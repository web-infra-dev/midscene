...
import { generateDefaultSessionName, exportEventsToFile } from '../utils';
...
export const useRecordingSession = (currentTab: chrome.tabs.Tab | null) => {
  ...
  const handleCreateSession = useCallback(async (values: {
    name: string;
    description?: string;
  }) => {
    const newSession: RecordingSession = {
  ...
  const handleSelectSession = useCallback((session: RecordingSession) => {
