...
const CanvasSelector: React.FC<CanvasSelectorProps> = ({ onChange, value = [] }) => {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const [elements, setElements] = useState<CanvasElement[]>(value.length > 0 ? value : DEFAULT_ELEMENTS);
    ...
    const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
        ...
        const clickedElement = elements.find(element => {
            ...
            const cbY = element.y + (element.height - CHECKBOX_SIZE) / 2;
            return (
                x >= cbX && x <= cbX + CHECKBOX_SIZE &&
                y >= cbY && y <= cbY + CHECKBOX_SIZE
            );
        });
        if (clickedElement) {
        ...
        // 检查是否点击了选项区域（高亮）
        const optionElement = elements.find(element =>
    ...
    const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
        ...
        // 检查鼠标是否在选项上
        const isOverOption = elements.some(element =>
            x >= element.x && x <= element.x + element.width &&
            y >= element.y && y <= element.y + element.height
        );
