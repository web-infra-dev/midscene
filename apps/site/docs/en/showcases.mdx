...
Pricing was checked on September 3, 2026, and Doubao costs use [Volcengine's official pricing](https://ark.volcengine.com/region:cn-beijing/model/detail?name=doubao-seed-2-1-turbo). Standard inference rates are CNY `¥3 / M input tokens`, `¥0.6 / M cached input tokens`, and `¥15 / M output tokens`; USD costs use `¥6.8 / $1`. The five cases cost approximately `¥1.5556 ($0.2288)` in total and average `¥0.3111 ($0.0458)` per case. These figures reflect this test run only; actual costs vary with task complexity, cache hit rate, and model pricing.
:::
