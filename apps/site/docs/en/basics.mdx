...
To share the same guidance across every AI-powered Agent API, set `contexts.default` when creating the Agent. `contexts.default` is only a fallback: Midscene uses it when neither the current call nor the matching API has its own context. API-specific and per-call contexts override it instead of being automatically concatenated with it.
...
To change a context after creating the Agent, use [`agent.setContext()`](./reference/#agentsetcontext):
...
- `context`: provides additional AI guidance for this call only, such as business facts, rules, constraints, or output requirements. It overrides `contexts.aiAct`, which overrides the shared fallback in `contexts.default`. An explicitly empty string disables inherited user context for this call.
