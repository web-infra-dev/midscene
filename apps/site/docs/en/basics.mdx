...
Standard Planning enables all components by default. Use `MIDSCENE_PLANNING_DISABLE_PARTS` for individual ablation: `subGoals,memory` removes both capabilities and their owned examples. The `deepThink` and `effort` mode options have been removed.
