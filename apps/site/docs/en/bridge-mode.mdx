...
Once enabled, the extension will continue listening for connection requests in the background, even without opening the sidebar. The extension icon will display a status badge: yellow dot for listening, green dot for connected.
