...
In bridge mode, these constructor options are ignored (desktop browser settings apply instead): `userAgent`, `viewportWidth`, `viewportHeight`, `deviceScaleFactor`, `waitForNetworkIdle`, `cookie`, `extraHTTPHeaders`, `downloadPath`, `chromeArgs`.

See the [API reference](./api#agentoverchromebridge-web) for details.

## Remote Access Configuration

By default, the Bridge Server only listens on `127.0.0.1`. For cross-machine communication, configure via the `allowRemoteAccess`, `host`, and `port` constructor options:
...
For the full constructor signature, see the [API reference](./api#agentoverchromebridge-web).
...
- For the full Chrome Bridge API surface, see [API reference](./api#agentoverchromebridge-web).
- Demo project
