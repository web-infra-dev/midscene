...
1. Open the Chrome extension's Bridge Mode page
2. Enable the **Auto-connect in background** toggle
...
Background bridge mode runs through the Chrome extension's Service Worker. To reduce the chance that the Service Worker is automatically suspended by the browser, the extension uses the `chrome.alarms` API as a periodic wake-up/keepalive, scheduling an alarm approximately every 24 seconds to keep the background context active and help maintain a stable connection.
