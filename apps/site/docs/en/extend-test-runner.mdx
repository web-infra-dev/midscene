...
`@midscene/test/midscene` exports seven built-in system Nodes: `aiAct`, `aiAssert`, `recordToReport`, `launch`, `terminate`, `wait`, and `agent`.
...
const midsceneNodes = createMidsceneNodes<ProjectContext>({
  ...
  // Web uses gotoUrl instead of the device lifecycle Nodes.
  includeLaunch: false,
});
```

`includeLaunch: false` also disables `terminate` by default. Set
`includeTerminate` explicitly only when the two capabilities need to be
...
`setCookies` deliberately does not accept inline cookie values. Use exactly
one of `cookiesEnv`, `profile`, or `storageStatePath`. An environment variable
may contain a Cookie header, a JSON cookie array, or Playwright storage-state
JSON. Cookie values are resolved inside the Node and are not included in the
Node result or workflow history. Relative storage-state paths resolve from the
current working directory by default; use `resolveStorageStatePath` when a
project needs a different root.
...
For Android, combine the device lifecycle Nodes from `createMidsceneNodes()`
with `runAdbShell`:
...
For iOS, register `runWdaRequest`; `launch` and `terminate` continue to use the
...
The generated reference includes every Node registered in the active Test Project, including the seven built-in system Nodes returned by `createMidsceneNodes()`, sorted by name. The runner automatically converts each Zod `inputSchema` to standard JSON Schema.
