...
The command requires pnpm and Node.js `^20.19.0 || ^22.12.0 || >=24.0.0`. It installs dependencies with pnpm and runs `describe-nodes` to write `midscene-nodes.md`.
...
In an interactive terminal, `pnpm dlx @midscene/test create` asks for the directory and platform. In CI or other noninteractive environments, supply both. Existing destination files are never overwritten, including when `--yes` is set.
