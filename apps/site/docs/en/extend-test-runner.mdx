...
Use `create` to generate a project with a platform Agent, example cases, and a Node reference:
...
The command requires Node.js `^20.19.0 || ^22.12.0 || >=24.0.0` and the selected package manager (npm or pnpm). It installs dependencies and runs `describe-nodes` to write `midscene-nodes.md`.
...
| `-y`, `--yes` | Disable prompts. Both directory and platform must be supplied. |
...
The generated project contains the following files when using pnpm. npm creates `package-lock.json` instead of `pnpm-lock.yaml`. The README is generated only as `README.md`:
