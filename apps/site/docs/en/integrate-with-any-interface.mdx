...
A typical usage of this feature is to control the interface of your own app, like an IoT device, an in-house app, a display in car, etc. And you can also use it to run interface-specific interactions with your own app.

 All these popular tools are included by default: 
...
- `actionSpace()`: the action space of the interface, which is an array of `DeviceAction` objects. Use predefined actions or define any custom action.
