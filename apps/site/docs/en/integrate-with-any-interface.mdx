...
Launch an MCP server for your Agent, so you can use your custom interface from MCP clients like Claude Desktop or Cursor.
...
You can also launch the MCP server in HTTP mode:

```ts
await mcpServerForAgent(agent).launchHttp({ port: 3000 });
```

To configure this MCP server in Claude Desktop (`claude_desktop_config.json`):
...
```json
{
  "mcpServers": {
    "my-custom-device": {
      "command": "node",
      "args": ["path/to/your/mcp-entry.mjs"]
    }
  }
}
```

If you only need to extract tool definitions (for integrating into another MCP server), use `mcpKitForAgent`:

```ts
import { mcpKitForAgent } from '@midscene/core/mcp';

const { description, tools } = await mcpKitForAgent(agent);
// tools is an array of ToolDefinition objects that can be registered on any MCP server
```

### Step 5. Add Skill support (optional)

[Agent Skills](https://github.com/anthropics/skills) let AI coding assistants (Claude Code, Cline, etc.) drive your custom interface through CLI commands — no MCP server setup required. Learn more in the [Skills documentation](./skills).
...
### Step 6. Release the npm package, and let your users use it
...
### Step 7. Invoke your class in Midscene CLI and YAML script
