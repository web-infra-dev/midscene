...
From Midscene v0.28.0, we have launched the feature to integrate with any interface. Define your own interface controller class conforming to the `AbstractInterface` class, and you can get a fully-featured Midscene Agent. All these popular tools are included by default: 

- the GUI Automation Agent SDK for TypeScript
...
- `actionSpace()`: the action space of the interface, which is an array of `DeviceAction` objects. Use predefined action spaces or define any custom action.
