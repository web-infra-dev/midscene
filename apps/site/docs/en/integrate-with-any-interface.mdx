...
- `screenshot?(): Promise<{ bytes: Uint8Array; format: 'png' | 'jpeg' | 'webp' }>`: return the original encoded bytes and their format. Midscene prefers this method when provided, avoiding a Base64 round trip. Keep `screenshotBase64()` for compatibility. The bytes must match the declared format and must not be mutated after being returned.
