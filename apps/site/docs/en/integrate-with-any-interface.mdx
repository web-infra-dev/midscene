...
Please note that only models with visual grounding capabilities can be used to control the interface. Read [Model selection](./model-selection) to pick a suitable model.
...
You can define your own action by using the `defineAction()` function. You can also use this method to define more actions for the [PuppeteerAgent](./integrate-with-puppeteer), [AgentOverChromeBridge](./bridge-mode#constructor), and [AndroidAgent](./integrate-with-android).
...
Read [Model selection](./model-selection) to learn more.
