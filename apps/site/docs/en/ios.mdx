...
Midscene connects to iOS devices through WebDriverAgent.
...
Looking for constructors, helper methods, and platform-only device APIs? See the dedicated [API reference](./api#iosagent) for detailed parameter lists plus advanced topics like custom actions. For API surfaces shared across platforms, head to the [common API reference](./api).
...
- For the iOS API reference, see [API reference](./api#iosagent).
- Demo projects
