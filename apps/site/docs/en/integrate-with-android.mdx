...
- `opts?: AndroidDeviceOpt` - Optional, the options for the AndroidDevice
  - `autoDismissKeyboard?: boolean | 'back' | 'esc' | 'midscene-ime' | 'midscene-ime-auto-install'` - Optional, how to dismiss the keyboard after inputting. (Default: `true`, which maps to `'esc'` on Android)
    - `true` / `'esc'` - Try ESC first, then BACK if needed.
    - `'back'` - Try BACK first, then ESC if needed.
    - `'midscene-ime'` - Use the MidsceneIME helper keyboard to dismiss without sending ESC/BACK key events into the focused field.
    - `'midscene-ime-auto-install'` - Same helper-IME flow, but `AndroidAgent` will use AI to approve install prompts if the device blocks silent installs. If safe installation cannot complete, Midscene throws instead of falling back to ESC/BACK.
    - `false` - Leave the keyboard open after input.
  - `keyboardDismissStrategy?: 'esc-first' | 'back-first'` - Deprecated backward-compatibility alias for `autoDismissKeyboard: 'esc' | 'back'`.
...
This is typically caused by key-event-based keyboard dismissal. After Midscene inputs text via the virtual keyboard, Android often needs an extra step to dismiss the keyboard. If that step sends **ESC** or **BACK** into the focused field, WebView JavaScript may capture it and cause side effects such as:

- Closing the popup/modal that contains the input field
- Clearing the text just entered
- Navigating away from the current view

**Recommended solution:** Use `autoDismissKeyboard: 'midscene-ime'` so Midscene switches to the helper IME and dismisses the keyboard without sending key events into the field:
...
If your ROM shows an install approval dialog for helper APKs, use `autoDismissKeyboard: 'midscene-ime-auto-install'` together with `AndroidAgent`. Midscene will ask the Agent to approve the install prompt. If safe installation still cannot complete, it throws instead of falling back to ESC/BACK.

If you do not want to use the helper IME, you can still try the legacy `'back'` strategy, which is safer than `'esc'` for many WebView flows but still sends a key event:
...
Or disable auto keyboard dismissal entirely and let the Agent manage the keyboard state:
