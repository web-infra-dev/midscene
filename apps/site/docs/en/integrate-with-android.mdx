...
- `opts?: AndroidDeviceOpt` - Optional, the options for the AndroidDevice
  ...
  - `minScreenshotBufferSize?: number` - Optional, the minimum screenshot buffer size in bytes. Default is `0`, which disables size-based validation while still rejecting empty buffers and invalid image formats.
