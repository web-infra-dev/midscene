...
- `opts?: AndroidDeviceOpt` - Optional, the options for the AndroidDevice
  ...
  - `minScreenshotBufferSize?: number` - Optional, screenshot buffer size validation threshold in bytes. Buffers below this value are treated as failed or corrupted captures. Default is `1024` (1KB). Set to `0` to skip only this size check while still rejecting empty buffers and invalid image formats.
...
> Note: This method wraps `adb shell`, so pass only the shell command itself. For example, use `input tap 100 200`, not `adb shell input tap 100 200`.
...
- Parameters:
  ...
  - `command: string` - The adb shell command to execute, without the `adb shell` prefix.
...
// Equivalent to running `adb shell dumpsys battery` with a 60s timeout
console.log(result);

await agent.runAdbShell('input tap 100 200');
// Equivalent to running `adb shell input tap 100 200`
```
...
const device = new AndroidDevice('s4ey59', {
  ...
  // the adb server to connect to (like `adb -H <host> -P <port>`), NOT the device's adbd or an `adb connect <ip:port>` target
  remoteAdbHost: '192.168.1.100',
  remoteAdbPort: 5037,
});
```

> For a cloud / remote device, just `adb connect <ip:port>` it to your local adb server and use the normal `getConnectedDevices()` flow — you only need `remoteAdbHost` / `remoteAdbPort` when the device is exposed through a separate remote adb server.
