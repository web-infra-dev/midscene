...
This method allows you to pass a uri to open a webpage or native page.
...
This method allows you to pass a adb device id to create a AndroidAgent.
...
* Parameters:
  * `deviceId?: string` - Optional parameter, the adb device id to connect. If not provided, the first connected device will be used.
  * `opts?: PageAgentOpt` - Optional parameter, the options for the AndroidAgent, refer to [constructor](./API).
...
const agent = await agentFromAdbDevice('s4ey59ytbitot4yp'); // pass deviceId
const agent = await agentFromAdbDevice(); // not pass deviceId, use the first connected device
```

### `getConnectedDevices()`

This method allows you to get all connected Android devices.
