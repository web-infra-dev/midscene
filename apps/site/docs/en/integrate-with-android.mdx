...
- `opts?: AndroidDeviceOpt` - Optional, the options for the AndroidDevice
  ...
  - `minScreenshotBufferSize?: number` - Optional, the minimum screenshot buffer size in bytes. Default is `1024` (1KB). Set to `0` to disable size-based validation while still rejecting empty buffers and invalid image formats.
