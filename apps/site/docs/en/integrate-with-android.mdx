...
- `opts?: AndroidDeviceOpt` - Optional, the options for the AndroidDevice
  ...
  - `screenshotResizeScale?: number` - **Deprecated.** Use `screenshotShrinkFactor` in `AgentOpt` instead. This option no longer affects screenshot size sent to the AI model.
