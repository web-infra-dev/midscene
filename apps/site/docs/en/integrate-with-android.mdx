...
- `opts?: AndroidDeviceOpt` - Optional, the options for the AndroidDevice
  ...
  - `hideKeyboardStrategy?: 'esc-first' | 'back-first'` - Optional, the strategy to dismiss the keyboard. 'esc-first' tries ESC key first, then back key if needed. 'back-first' tries back key first, then ESC key if needed. (Default: 'esc-first')
