...
- Parameters:
  ...
  - `options?: object` — Optional configuration. `uiContext` and image prompt options are supported. Single-element optimizations such as `xpath`, cache lookup, and `deepLocate` are not applied to `aiLocateAll()`.
