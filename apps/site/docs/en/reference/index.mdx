...
- Parameters:
  ...
  - `options?: object` — Optional configuration:
    ...
    - `screenshotIncluded?: boolean` — Whether to include a screenshot. Default: `true`.
    - `context?: string` — Additional context for this call. See [Per-call context](#per-call-context).
    - `observationRecord?: string | UIObservationRecord` — The path to an observation JSON manifest, or an already resolved record. When set, the assertion evaluates that frame window instead of the current screen.
...
function startObserving(options?: {
  outputPath?: string; // JSON manifest path; images are stored in an adjacent .frames directory
...
- `observer.exportRecord(): Promise<string>` — Finalize the JSON manifest and adjacent image directory, then return the absolute manifest path.
