...
- `ListBrowserPages({ offset? })` — Lists eight tabs at a time, starting at offset 0. Pass `{ offset: 8 }` for the next group. List feedback shortens long titles and URLs separately; an out-of-range offset throws.
- `GetBrowserPageInfo({ index })` — Returns a tab's complete title and URL without switching pages. Its planning feedback is untruncated; an invalid index throws.
- `SetActivePage({ index?, title?, url? })` — Switches by index, title substring, or URL substring. Combined selectors must match the same tab. Only Browser Agent exposes these actions.
...
- `wdaBaseUrl?: string` — Full HTTP(S) WDA API base URL, including a gateway path prefix. Takes precedence over `wdaHost` and `wdaPort`.
