...
Every RDP screenshot reads the current framebuffer, including the first screenshot after connecting. Capture waits for a 300 ms interval without framebuffer updates, with a maximum settling wait of 3 seconds per screenshot. Continuously changing content, such as video, can use the full wait before returning the latest frame. If the framebuffer remains entirely black, capture throws an error after the timeout; a disconnected session interrupts the wait.
