...
- Parameters:
  ...
  **Recommended usage:**
  ...
  - `opt: object` — Configuration:
    - `keyName: string` — **Required.** Keyboard key to press, such as `'Enter'`, `'Tab'`, or `'Escape'`. Use `aiInput()` to enter text. Web and Computer devices support modifier shortcuts such as `'Control+A'` and `'Shift+Enter'`; use `'+'` to join the keys. Harmony supports a conservative set of named keys, `A`-`Z`, `0`-`9`, and combinations such as `'Control+A'`. Output characters that do not have their own key code, such as `'?'`, are not key names. The built-in Android and iOS devices currently support single keys only. Unsupported keys throw an error, and support in custom devices depends on their implementation.
    - `deepLocate?: boolean` — Whether to enable [Deep Locate](#deep-locate-deeplocate). The deprecated `deepThink` name remains supported. Default: `false`.
    - `context?: string` — Additional context for this call. See [Per-call context](#per-call-context).
    - `xpath?: string` — XPath for the target element. Midscene tries the XPath before consulting the cache or AI model. Default: `undefined`.
    - `cacheable?: boolean` — Whether this call can use the [cache](../caching.mdx). Default: `true`.
