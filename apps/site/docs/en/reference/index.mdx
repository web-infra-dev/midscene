...
- `contexts: { default?: string; [apiName]?: string }` — Agent-level AI guidance, such as business facts, decision or interaction rules, constraints, and output requirements. `default` is the shared fallback for all AI-powered APIs and is used only when neither the current call nor the matching API has its own context. Resolution uses `options.context ?? contexts[apiName] ?? contexts.default`; only the highest-priority defined value is used, with no automatic concatenation. An empty string explicitly clears lower-priority user context. Default: `undefined`.
- `aiActContext: string` (deprecated) — Compatibility option for `contexts.aiAct`. `contexts.aiAct` takes precedence when both are provided; when it is absent, `aiActContext` behaves like `contexts.aiAct`. Use `contexts.aiAct` in new code. The older `aiActionContext` name also remains supported.
...
`aiAsk()` has the same query and return behavior as `aiString()`. They can still use separate Agent-level contexts through `contexts.aiAsk` and `contexts.aiString`.
...
`aiString()` has the same query and return behavior as `aiAsk()`. They can still use separate Agent-level contexts through `contexts.aiString` and `contexts.aiAsk`.
...
- Parameters:
  ...
  - `options?: object` — Optional runtime options.
    - `context?: string` — Additional AI guidance applied to every step in this run. For each mapped API call, it overrides the Agent's API-specific context and `contexts.default`.
...
- Parameters:
  ...
  - `context` — Additional AI guidance, such as business facts, rules, constraints, or output requirements. Use `''` to block the shared default for an API target, or `undefined` to remove that API override and fall back to `contexts.default` again.

- Example:

```typescript
agent.setContext('default', 'Prices are displayed in USD.');
agent.setContext('aiQuery', 'Return monetary values without currency symbols.');
agent.setContext('aiQuery', ''); // Do not use contexts.default for aiQuery
agent.setContext('aiQuery', undefined); // Inherit contexts.default again
```

#### `setAIActContext()` (deprecated) {#agentsetaiactcontext}

Deprecated compatibility setter for the context sent with subsequent `agent.aiAct()` or `agent.ai()` calls. Use `agent.setContext('aiAct', context)` instead. It remains equivalent to that call and replaces any existing `contexts.aiAct` value.
...
Both `agent.setAIActContext()` and the older `agent.setAIActionContext()` remain available for compatibility, but new code should use `agent.setContext('aiAct', context)`.
...
Use `contexts.default` as the shared fallback for every AI-powered API, `contexts[apiName]` to configure one API, and the optional per-call `options.context` for one request. `contexts.default` is selected only when neither of the more specific levels provides a value; it is not automatically concatenated with them.

```typescript
import { mergeAIContexts } from '@midscene/core';

const defaultContext = 'Prices are displayed in USD.';
const agent = new PlaywrightAgent(page, {
  contexts: {
    default: defaultContext,
    aiAct: '', // Explicitly ignore the default context
    aiQuery: 'Return monetary values without currency symbols.',
    aiBoolean: mergeAIContexts(
      defaultContext,
      'Only clearly visible states count as true.',
    ),
  },
});
```

The effective user context is `options.context ?? contexts[apiName] ?? contexts.default`. These layers override one another; they are not implicitly concatenated. `''` is a defined value and clears lower-priority user context, while `undefined` or an omitted key continues to the next fallback. Use `mergeAIContexts()` when you explicitly want to concatenate context fragments.
...
Framework-owned context, such as Test Runner workflow history, is appended after the selected user context and is not affected by this override chain.
...
- All other fields match the [common constructor parameters](#common-parameters), including `generateReport`, `reportFileName`, `contexts`, `aiActContext`, `modelConfig`, `cache`, `createOpenAIClient`, and `onTaskStartTip`.
...
const agent = await agentFromWebDriverAgent({
  ...
  contexts: { aiAct: 'Accept permission dialogs automatically.' },
});
```
...
const agent = await agentForComputer({
  contexts: { aiAct: 'You are automating a desktop application.' },
});
