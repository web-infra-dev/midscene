...
- `scrcpyConfig?: object` — High-performance scrcpy screenshot configuration:
  ...
  - `videoBitRate?: number` — scrcpy H.264 encoding bitrate in bits per second. Default: `100000000`. For a bandwidth-constrained remote link, explicitly set a lower value such as `4000000` and tune it for the actual transport; Midscene does not infer link capacity from the device address.
