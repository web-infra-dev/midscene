...
- `keyboardEventMode?: 'logical' | 'physical'` — How the macOS AppleScript keyboard backend sends modifier keys. `'logical'` uses standard AppleScript keyboard events and is the default. `'physical'` is an opt-in compatibility mode for VNC environments that do not preserve modifiers from the standard events. It sends explicit key-down and key-up events, assumes matching en-US keyboard layouts, and requires sequential text input or a positive `keyboardTypeDelay`. This option has no effect outside macOS or when `keyboardDriver` is `'libnut'`. See [Control a remote desktop through a VNC client](../platforms/desktop#vnc-keyboard).
