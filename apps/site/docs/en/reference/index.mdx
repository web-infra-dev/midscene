...
`startObserving()` records screen changes over a period of time. Start the recording, perform the page actions, and then call `stop()`. `stop()` returns a `UIObservation` that you can query or assert against. For example, a toast may disappear before a normal `agent.aiAssert()` runs, while `observation.aiAssert()` can still find it in the recorded frames.
...
`UIObserver` only controls the recording process:

- `observer.bufferedFrameCount: number` — Number of frames currently buffered while recording.
- `observer.stop(): Promise<UIObservation>` — Stop sampling and return the completed recording.

`UIObservation` represents the recording as it existed when `stop()` was called. It provides `aiQuery()`, `aiBoolean()`, `aiNumber()`, `aiString()`, `aiAsk()`, and `aiAssert()`. These methods only analyze recorded images; they do not read the page DOM at query time. For that reason, their options do not include `domIncluded`. Passing `domIncluded` after bypassing TypeScript's checks also throws at runtime.

The observation also exposes `frameCount`, `startedAt`, and `endedAt`. After the last query or assertion, call `observation.dispose()` to remove its temporary image files early. If you do not call it, `agent.destroy()` removes them.
