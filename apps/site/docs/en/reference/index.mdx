...
##### Planning components {#planning-components}

Standard Planning enables sub-goals, memory, observation guidance, screenshot history and separate localization by default. The old `deepThink` and experimental `effort` mode options have been removed; use independent environment-controlled components instead.

```bash
MIDSCENE_CACHE=false
MIDSCENE_PLANNING_DISABLE_PARTS=subGoals,memory
```

Disabling a capability removes its instructions, owned examples, effective output, state and replay together. `subGoals` includes its examples. `screenshotHistory` and `separateLocate` control the previous screenshot and separate localization respectively; hold them fixed in other arms. Provider thinking remains controlled through [model configuration](../model-config#model-native-reasoning).
...
`deepLocate` controls enhanced element localization. The `deepThink` planning mode has been removed from `aiAct`; use [Planning components](#planning-components). Single-step actions retain the deprecated `deepThink` alias for `deepLocate`.
