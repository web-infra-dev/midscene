...
- `waitForInitialPageReady: WaitForInitialPageReadyOptions` — Customize [initial page readiness](#initial-page-ready).
...
Configure `waitForInitialPageReady` to wait for application readiness before the Agent's first screenshot, query, assertion, or action. The constructor only stores the handler; open the page or app before using the Agent.

- `handler: ({ signal }: InitialPageReadyContext) => Promise<void>` — Required. Resolves when the initial page is ready.
- `timeoutMs?: number` — Timeout in milliseconds, default `10000`. Integer from `1` to `2147483647`.

```ts
import { setTimeout as delay } from 'node:timers/promises';
import { PlaywrightAgent } from '@midscene/web/playwright';

// The application sets data-app-ready="true" on <html> when ready.
...
- Runs once per Agent. Concurrent calls share the result; navigation, reloads, and tab switches do not reset it.
- Rejection or timeout blocks execution, including cached workflows. Later calls reuse the failure without retrying.
- `signal` aborts on timeout or `agent.destroy()`; use it to stop polling and clean up listeners. Cancelling one action leaves shared initialization running.
- Do not call the same Agent's UI methods (such as `aiWaitFor()` or `aiQuery()`) inside the handler. They also await the handler, creating a circular wait until timeout. Use underlying page/device APIs instead.
- Check a readiness flag such as `data-app-ready`. A one-time `ready` event may fire before the handler starts and be missed.

All Agent platforms support this option. Existing post-action waits and behavior without the hook remain unchanged.

For `puppeteerAgentForTarget`, pass the option in its second argument. The launcher navigates, awaits the hook instead of the initial network-idle check, then returns the ready Agent.

<a id="default-waiting-strategy"></a>

**Default waiting strategy (advanced)**

These options continue to apply after initial-page readiness:
...
| [`waitForInitialPageReady`](#initial-page-ready) | Customize readiness once per Agent, before its first UI operation. |
