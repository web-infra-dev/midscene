...
- `screenshotStrategy?: 'auto' | 'always-yadb'` — Controls the screenshot method. Default: `'auto'`.
  - `'auto'` (default) — Tries scrcpy → `adb.takeScreenshot` → `screencap`. It uses yadb only if the `screencap` command itself fails and does not detect black frames.
  - `'always-yadb'` — Skips the earlier paths and captures directly via [yadb](https://github.com/ysbing/yadb). Use this when `screencap` produces black frames for secure pages (`FLAG_SECURE`) while yadb captures them correctly (e.g. rooted devices with a Magisk hook to bypass `FLAG_SECURE`, or Android 11 and below). Yadb can only capture the default display (`displayId=0`); combining this strategy with a non-zero `displayId` throws an error. Can also be set via the `MIDSCENE_ANDROID_SCREENSHOT_STRATEGY=always-yadb` environment variable.
