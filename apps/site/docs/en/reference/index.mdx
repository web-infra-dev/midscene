...
- Parameters:
  ...
  - `options?: object` — Optional configuration:
    ...
    - `screenshotIncluded?: boolean` — Whether to include a screenshot. Default: `true`.
    - `context?: string` — Additional context for this call. See [Per-call context](#per-call-context).
    - `observationRecord?: UIObservationRecord` — A record returned by `observer.exportRecord()`. When set, the assertion evaluates that frame window instead of the current screen.
...
- `observer.exportRecord(): Promise<UIObservationRecord>` — Return the observed frame window. Frames reference image files on disk rather than embedding base64 data.
- `observer.dispose(): Promise<void>` — Delete the observer-owned image files when the exported record is no longer needed. Do not dispose the observer before the last assertion that uses its record.
