...
- Parameters:
  ...
  - `options?: object` — Optional configuration:
    ...
    - `fileChooserAllowedDir?: string` — Limits the paths this `aiAct` call may directly specify for prompt-driven file uploads. Defaults to `process.cwd()`. Relative paths in the prompt are resolved against this directory and then validated; absolute paths are validated directly. Symlinks located within this directory are also allowed.
...
`aiAct` can upload files mentioned in its prompt. By default, it resolves paths from the current working directory. Pass `fileChooserAllowedDir` to use another root directory for that call. Midscene resolves both relative and absolute paths before validating that they stay within the selected directory.
