...
- Use underlying page/device APIs inside the handler. Calling the same Agent's UI APIs would wait on its own initialization. Prefer checking readiness state to avoid missing an earlier event.
