...
- `keyboardEventMode?: 'logical' | 'physical'` — How the macOS AppleScript keyboard backend sends modifier keys. `'logical'` uses one compact `keystroke ... using` command and is the default. `'physical'` is an opt-in compatibility mode for foreground applications that require separate modifier transitions, including some remote-control clients and virtual machine consoles. Both modes use AppleScript; `'physical'` does not emit hardware events. It assumes matching en-US keyboard layouts and requires sequential text input or a positive `keyboardTypeDelay`. This option has no effect outside macOS or when `keyboardDriver` is `'libnut'`. See [Forward keyboard input through desktop clients](../platforms/desktop#keyboard-event-mode).
- `keyboardShortcutDelay?: number` — Finite non-negative delay in milliseconds around modifier transitions for the local libnut keyboard driver. A positive value sends modified keys as modifier-down, wait, primary key, wait, and modifier-up. It applies to shortcuts and to the implicit `Shift` used by uppercase letters or shifted en-US punctuation during sequential text input. Default: `0`. This option has no effect on direct RDP input or the macOS AppleScript driver. See [Forward keyboard input through desktop clients](../platforms/desktop#keyboard-event-mode).
