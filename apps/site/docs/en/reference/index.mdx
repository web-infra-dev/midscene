...
| [Runtime configuration](#runtime-configuration) | Global environment variables for run artifacts, language, Playground networking, and Debug logs |
...
- `aiContexts: { default?: string; [apiName]?: string }` — Agent-level AI guidance, such as business facts, decision or interaction rules, constraints, and output requirements. If configured, `default` is the shared fallback for all AI-powered APIs and is used only when neither the current call nor the matching API has its own context. Resolution uses `options.context ?? aiContexts[apiName] ?? aiContexts.default`; only the highest-priority defined value is used, with no automatic merging. An empty string explicitly clears lower-priority user context. Default: `undefined`.
- `aiActContext: string` (deprecated) — Compatibility option for `aiContexts.aiAct`. `aiContexts.aiAct` takes precedence when both are provided; when it is absent, `aiActContext` behaves like `aiContexts.aiAct`. Use `aiContexts.aiAct` in new code. For guidance on writing and supplying context, see [Provide application knowledge to a GUI Agent](../provide-application-knowledge-to-gui-agent).
- `aiActionContext: string` (deprecated) — Older compatibility alias for `aiContexts.aiAct`. It remains supported and is used only when both `aiContexts.aiAct` and `aiActContext` are absent. Resolution uses `aiContexts.aiAct ?? aiActContext ?? aiActionContext`; an empty string takes precedence over lower-priority aliases. Use `aiContexts.aiAct` in new code.
...
- `waitForActionReady: WaitForActionReadyOptions` — Customize [readiness after each action](#action-ready).
- `waitAfterAction: number` — Fixed delay after each action, default `300` ms. See [Default waiting strategy](#default-waiting-strategy).
...
The device CLIs expose serializable Agent behavior options for individual calls. Options containing functions, such as `waitForActionReady.createWaiter`, must be configured through the JavaScript/TypeScript SDK. Convert camelCase option names to kebab-case flags; for example, `waitAfterAction` becomes `--wait-after-action`. See [Skills](../skills) for platform-specific CLI entry points.

<a id="action-ready"></a>

**Readiness after each action**

Configure `waitForActionReady` to decide when the next step can begin. It applies to each atomic action (`Input`, `Tap`, etc.), including `aiAct` substeps and cache replay. It does not run for initial page loading or read-only queries.

- `createWaiter: (context: ActionReadyContext) => ActionReadyPlan | Promise<ActionReadyPlan>` — Runs before each action to register observation. The context contains `action: { id, name, param }` and `signal`. Each execution has a new ID and resolved parameters.
- `timeoutMs?: number` — Separate timeout for `createWaiter`, `wait`, and `dispose`, default `10000` ms. Integer from `1` to `2147483647`; does not set the action driver's timeout.

| Factory result | After the action returns |
| --- | --- |
| `'skip'` | Continue without automatic stability waiting. |
| `'default'` | Use the [default waiting strategy](#default-waiting-strategy). |
| `{ wait, dispose? }` | Await `wait(): Promise<void>`, then clean up with optional `dispose()`. |

Here, `appMonitor` is your application's monitoring adapter. It can skip known synchronous inputs, observe the current login request and rendering, or leave other actions to the default strategy.
...
const agent = new PlaywrightAgent(page, {
  waitForActionReady: {
    timeoutMs: 10_000,
    createWaiter: ({ action, signal }) => {
      if (appMonitor.canSkipWaiting(action)) return 'skip';

      const observation = appMonitor.observe(action, { signal });
      if (!observation) return 'default';

      return {
        wait: () => observation.waitUntilReady(),
...
await agent.aiAct('Enter the account and password, then log in');
```

- Register listeners in `createWaiter`; await the action's result in `wait`. The factory must not wait for an event that requires the action to run. Correlate signals with the current action so earlier readiness does not release a later action.
- `'skip'` and custom waiters replace generic action delays and platform post-action checks. Required driver timing (such as keyboard preparation and gesture duration) and lifecycle callbacks remain in effect. Unconfigured Agents keep existing behavior.
- Failure or timeout stops the workflow without automatically replaying the action or falling back to planning. A later explicit call creates a new waiter. Cleanup also runs on action failure; `signal` aborts on cancellation, timeout, or `agent.destroy()`.
- Use underlying page/device APIs in these callbacks. UI calls on the same Agent while a callback is running fail explicitly, including concurrent calls. A moment with no network requests alone does not prove the UI is ready.

All Agent platforms support this SDK option. `puppeteerAgentForTarget` accepts it in its second argument and keeps its initial network-idle wait.

<a id="default-waiting-strategy"></a>

**Default waiting strategy (advanced)**

These options apply when no custom waiter is configured or it returns `'default'`:

| Option | Behavior |
| --- | --- |
| `waitAfterAction` | Fixed delay after an action, default `300` ms. Explicit device-action delays take precedence. |
| `waitForNavigationTimeout` (Web) | Wait for `<html>` to be available, default `5000` ms. This does not guarantee navigation or rendering is complete. |
| `waitForNetworkIdleTimeout` (Puppeteer only) | Post-action network-idle timeout, default `2000` ms. Requires at most two in-flight requests for `200` ms. |

Set either Web timeout to `0` to skip its check. These checks warn and continue on timeout.
...
> The supported keys and values are listed in [Model configuration](../model-config). See [Model strategy](../model-strategy) to understand the Default, Planning, and Insight model roles.
...
This method accepts UI goals and assertions described in natural language. During execution, Midscene continuously uses AI to plan and act from the latest interface state until the goal is complete. If an assertion in the prompt fails, `aiAct` throws an error promptly.
...
- Parameters:
  ...
  - `prompt: string | object` — A natural language description of the UI goal and any optional assertions, or [prompting with images](#prompting-with-images).
  - `options?: object` — Optional configuration:
    - `cacheable?: boolean` — Whether this call can use the [cache](../caching.mdx). Default: `true`.
    - `deepThink?: 'unset' | true | false` — Controls the planning implementation used by Midscene when `aiAct` performs planning. See [`deepThink` planning mode](#aiact-deepthink).
...
##### `deepThink` planning mode {#aiact-deepthink}

`deepThink` controls the planning implementation used by `aiAct`:

- By default, `aiAct` plans the next step and locates the target element in the same planning request.
- When set to `true`, `aiAct` focuses more on task decomposition and separates task planning from element localization into different model calls. This can improve stability for complex tasks, but it also increases model calls and latency.
...
function aiInput(
  ...
  opt: {
    ...
    inputStrategy?: 'legacy' | 'sequential' | 'bulk';
...
- Parameters:
  ...
  **Recommended usage:**
  ...
  - `opt: object` — Configuration:
    ...
    - `keyboardTypeDelay?: number` — Finite non-negative delay in milliseconds between keystrokes. Whether this option affects `legacy` input depends on the platform's existing input path; see the table below. A `legacy` path that supports delay enters one Unicode code point at a time when the value is greater than `0`. Use this option when an input field drops characters during fast input.
    - `inputStrategy?: 'legacy' | 'sequential' | 'bulk'` — How Midscene sends the text to the platform. Default: `'legacy'`.
      - `'legacy'`: Preserve the platform's existing behavior. This is the compatibility-safe default, so existing tests do not change after upgrading.
      - `'sequential'`: Send one Unicode code point at a time, including when `keyboardTypeDelay` is omitted or `0`.
      - `'bulk'`: Send the complete text in one platform input operation where the platform supports it. On Web, `replace` selects the existing value and uses `insertText`, producing one insertion `input` event rather than first emitting a clearing input event and then per-character input events. Other platforms make one call from Midscene to their input backend, but the OS or driver may still synthesize multiple application events.
      - `'bulk'` requires `keyboardTypeDelay` to be omitted or set to `0`. A value greater than `0` causes Midscene to throw an error; negative and non-finite values are invalid for every strategy. An action-level value overrides the Device or Agent default, so use `keyboardTypeDelay: 0` to override a positive default when selecting `'bulk'` for one action.
  ...
  The default value of `inputStrategy` is `'legacy'` on every platform. This mode preserves Midscene's platform-specific input logic from earlier versions, so its behavior can vary by platform:

  | Platform | `legacy` behavior |
  | --- | --- |
  | Web / Playwright / Puppeteer | In `replace` mode, clear the existing value and wait for the DOM to settle, then pass the complete value to `keyboard.type(value)`. A positive delay is handled by the browser driver. |
  | Chrome Extension / Bridge | Keep the Web clear-then-type flow and send text through the remote keyboard primitive. Legacy CDP typing keeps its existing zero-delay behavior; Bridge resolves the strategy on the CLI side before forwarding the primitive. |
  | Android | Non-ASCII or shell-sensitive text normally uses one yadb call and preserves the historical behavior of ignoring delay. The native `input text` path sends a complete segment by default and switches to one call per Unicode code point when delay is greater than `0`. |
  | iOS | Use one WDA `typeText` call by default; when delay is greater than `0`, use `typeRawKeys` once per Unicode code point. |
  | HarmonyOS | Use one HDC `inputText` call by default; when delay is greater than `0`, call it once per Unicode code point. |
  | Local Computer | Use clipboard paste when delay is omitted or `0`; when delay is greater than `0`, use real keyboard events once per Unicode code point. |
  | RDP Computer | Use one backend `typeText` call when delay is omitted or `0`; when delay is greater than `0`, call the backend once per Unicode code point. |
...
- Parameters:
  ...
  **Backward-compatible usage (deprecated but still supported):**
  - `key: string` — Keyboard key to press, such as `'Enter'`, `'Tab'`, or `'Escape'`.
...
- Parameters:
  ...
  **Recommended usage:**
  ...
  - `opt: object` — Configuration:
    ...
    - `direction?: 'down' | 'up' | 'right' | 'left'` — Direction of the off-screen content to reveal, rather than the movement of currently visible content. Used only when `scrollType` is `'singleAction'`. For example, `'down'` reveals content below the current viewport. Default: `'down'`.
...
`aiAsk()` has the same query and return behavior as `aiString()`. They can still use separate Agent-level contexts through `aiContexts.aiAsk` and `aiContexts.aiString`.
...
`aiString()` has the same query and return behavior as `aiAsk()`. They can still use separate Agent-level contexts through `aiContexts.aiString` and `aiContexts.aiAsk`.
...
- Return value:
  ...
  - `center` is the target point derived from the model’s localization result, expressed in the screenshot’s physical pixel coordinates. For a more stable click target, prefer the `center` field.
...
| Entry point | Purpose |
| --- | --- |
| [`agent.aiWaitFor(condition)`](#agentaiwaitfor) | Wait for a visible condition using AI at a script step. |
| [`browserAgent.waitForNewPage(action)`](#playwright-browser-agent) | Listen before running `action`, then return the new page (Playwright/Puppeteer). |
| [`waitForActionReady`](#action-ready) | Customize when to continue after each atomic action. |

For other events, `await` the application's Promise or a browser/device API. Subscribe before triggering one-time events.

#### `startObserving()` {#agentstartobserving}

`startObserving()` records the screen continuously. Use it to check transient UI, such as toasts, banners, and page transitions.

The flow is simple: start recording, perform the page actions, and call `stop()`. `stop()` returns a `UIObservation`. You can query or assert against the recorded frames.

```typescript
function startObserving(options?: {
  intervalMs?: number; // sampling interval; default 1,000 ms, minimum 200 ms
  maxFrames?: number; // maximum number of retained frames; default 30
  watchdogMs?: number; // maximum recording time; default 300,000 ms; 0 disables the limit
}): Promise<UIObserver>;
```

`UIObserver` manages the recording:

- `observer.bufferedFrameCount: number` — Number of frames currently buffered while recording.
- `observer.stop(): Promise<UIObservation>` — Stop recording and return a `UIObservation`.

`UIObservation` contains the recorded frames. The frames no longer change after you call `stop()`.

You can call `aiQuery()`, `aiBoolean()`, `aiNumber()`, `aiString()`, `aiAsk()`, and `aiAssert()`. These methods only read recorded frames. They do not read the current page DOM.

For this reason, the methods do not support `domIncluded`. TypeScript reports this mistake. Midscene also throws at runtime if JavaScript passes the option.

`UIObservation` also contains `frameCount`, `startedAt`, and `endedAt`. Call `observation.dispose()` to remove temporary images when you finish. `agent.destroy()` also removes any images that remain.
...
const toastCount = await observation.aiNumber(
  'how many success toasts appeared?',
);
...
- Midscene uses a continuous frame source when one is available. Android uses scrcpy (`scrcpyConfig.enabled`), iOS uses WDA MJPEG (`wdaMjpegFrameSource.enabled`), and the web uses CDP screencast. Without a continuous source, Midscene takes screenshots at intervals. Sampling is slower on mobile in this mode.
- Data URL frames are written to files during sampling. `UIObserver` does not keep every image in memory. Other frame handles are decoded and saved in small batches when recording stops. Midscene reads the images when a query or assertion runs.
- The model receives up to `maxFrames` frames and the final screenshot. Increase `intervalMs` to sample less often, or decrease `maxFrames` to limit the frame count. Observed frames appear in the report timeline with an `Observed` label.
- On iOS, observed frames come from the lower-quality MJPEG stream. The final screenshot still uses full quality.
...
Each `aiWaitFor()` check consumes model time and tokens. If the application exposes a readiness signal, await it directly.
...
- Parameters:
  ...
  - `options?: object` — Optional runtime options.
    - `context?: string` — Additional AI guidance applied to every step in this run. For each mapped API call, it overrides the Agent's API-specific context and `aiContexts.default`.
...
#### `setAIContext()` {#agentsetaicontext}

Set the Agent-level shared fallback context or the context for one API.

- Type

```typescript
function setAIContext(
  target: 'default' | AiApiName,
...
- Parameters:
  ...
  - `target` — Use `'default'` for the shared fallback used by all AI-powered APIs, or an API name such as `'aiAct'`, `'aiTap'`, `'aiQuery'`, or `'aiAssert'`.
  - `context` — Additional AI guidance, such as business facts, rules, constraints, or output requirements. For an API target, use `''` to block the shared default or `undefined` to remove the API override and fall back to `aiContexts.default` again if one is configured. For the `default` target, `undefined` removes the shared fallback.

- Example:

```typescript
agent.setAIContext('default', 'Prices are displayed in USD.');
agent.setAIContext('aiQuery', 'Return monetary values without currency symbols.');
agent.setAIContext('aiQuery', ''); // Do not use aiContexts.default for aiQuery
agent.setAIContext('aiQuery', undefined); // Inherit aiContexts.default again
```

#### `setAIActContext()` (deprecated) {#agentsetaiactcontext}

Deprecated compatibility setter for the context sent with subsequent `agent.aiAct()` or `agent.ai()` calls. Use `agent.setAIContext('aiAct', context)` instead. It remains equivalent to that call and replaces any existing `aiContexts.aiAct` value.
...
- Parameters:
  ...
  - `aiActContext: string` — The background knowledge that should be sent to the AI model. For guidance on writing and supplying context, see [Provide application knowledge to a GUI Agent](../provide-application-knowledge-to-gui-agent).
...
Both `agent.setAIActContext()` and the older `agent.setAIActionContext()` remain available for compatibility, but new code should use `agent.setAIContext('aiAct', context)`.
...
A context is additional guidance for the AI, such as business facts, decision or interaction rules, constraints, and output requirements. It supplements the API's main prompt instead of replacing it.

Use `aiContexts.default` as the shared fallback for every AI-powered API, `aiContexts[apiName]` to configure one API, and the optional per-call `options.context` for one request. If configured, `aiContexts.default` is selected only when neither of the more specific levels provides a value; it is not automatically merged with them.

```typescript
const defaultContext =
  'A struck-through price is the original price, a regular price is the current price, and all prices are in USD.';
const agent = new PlaywrightAgent(page, {
  aiContexts: {
    default: defaultContext,
    aiAct: '', // Explicitly ignore the default context
    aiQuery: 'When extracting an amount, return only the number without the currency symbol.',
    aiBoolean: `${defaultContext}\n\nA product is on sale only when its current price is lower than its original price.`,
  },
});

const isOnSale = await agent.aiBoolean('Is this product on sale?');
```

The effective user context is `options.context ?? aiContexts[apiName] ?? aiContexts.default`. These layers override one another; they are not automatically merged. `''` is a defined value and clears lower-priority user context, while `undefined` or an omitted key continues to the next configured fallback. To combine context fragments, concatenate the strings explicitly as shown above.
...
- In `aiAct()`, `deepThink` has always meant **planning mode**, guiding task decomposition and planning-focused reasoning. See [`deepThink` planning mode](#aiact-deepthink) for details.
...
- `waitForNavigationTimeout`, `waitForNetworkIdleTimeout` — Advanced options for the [default waiting strategy](#default-waiting-strategy).
- `enableTouchEventsInActionSpace: boolean` — Whether to add touch gestures such as swipe to the action space. Default: `false`.
- `keyboardTypeDelay: number` — Finite non-negative per-character delay in milliseconds. In `'legacy'` mode, Puppeteer's `page.keyboard.type` handles a positive delay. Increase this value only when a controlled input drops characters during fast input. Default: `undefined`, which uses Puppeteer's default.
- `inputStrategy: 'legacy' | 'sequential' | 'bulk'` — Default text input strategy for `Input` actions. Use `'bulk'` to insert the complete value in one operation or `'sequential'` to force code-point-by-code-point input. `'bulk'` requires `keyboardTypeDelay` to be omitted or set to `0`; use `'sequential'` for delayed input. Default: `'legacy'`.
...
- `customActions: DeviceAction[]` — Add additional custom actions so the Agent can call your domain-specific actions.
...
<a id="puppeteer-browser-agent"></a>
...
- `waitForNewPage(action?, options?)` — Registers a listener before running `action`, then returns the new page without implicitly switching the active page. Pass `{ timeout: milliseconds }` to override `newPageTimeout` for this call.
...
- `waitForNavigationTimeout`, `waitForNetworkIdleTimeout` — Advanced options for the [default waiting strategy](#default-waiting-strategy).
- `enableTouchEventsInActionSpace: boolean` — Whether to add touch gestures such as swipe to the action space. Default: `false`.
- `keyboardTypeDelay: number` — Finite non-negative per-character delay in milliseconds. In `'legacy'` mode, Playwright's `page.keyboard.type` handles a positive delay. Increase this value only when a controlled input drops characters during fast input. Default: `undefined`, which uses Playwright's default.
- `inputStrategy: 'legacy' | 'sequential' | 'bulk'` — Default text input strategy for `Input` actions. Use `'bulk'` to insert the complete value in one operation or `'sequential'` to force code-point-by-code-point input. `'bulk'` requires `keyboardTypeDelay` to be omitted or set to `0`; use `'sequential'` for delayed input. Default: `'legacy'`.
...
<a id="playwright-browser-agent"></a>
...
- `keyboardTypeDelay?: number` — Finite non-negative delay in milliseconds. Legacy Bridge input preserves the existing zero-delay CDP typing behavior. Set `inputStrategy` to `'sequential'` to apply the delay between forwarded Unicode code points.
- `inputStrategy?: 'legacy' | 'sequential' | 'bulk'` — Default text input strategy. `'sequential'` forwards one Unicode code point at a time, while `'bulk'` forwards one `insertText` operation. `'bulk'` requires `keyboardTypeDelay` to be omitted or set to `0`. Default: `'legacy'`.
...
- `Input` — Enter text with `replace`/`typeOnly`/`clear` modes (`append` is a deprecated alias for `typeOnly`). Supports optional `autoDismissKeyboard`, `keyboardTypeDelay`, and `inputStrategy` parameters.
...
- `keyboardTypeDelay?: number` — Finite non-negative delay in milliseconds between keystrokes. In `'legacy'` mode, a positive value makes the native `input text` path enter one Unicode code point at a time and remains ignored by yadb, preserving existing behavior. Set `inputStrategy` to `'sequential'` to split yadb input too.
- `inputStrategy?: 'legacy' | 'sequential' | 'bulk'` — Default text input strategy for `Input` actions. Use `'sequential'` for one ADB/yadb call per code point or `'bulk'` for one backend text call where the selected IME supports it. `'bulk'` requires `keyboardTypeDelay` to be omitted or set to `0`; use `'sequential'` for delayed input. Default: `'legacy'`.
...
- All other fields match the [common constructor parameters](#common-parameters), including `generateReport`, `reportFileName`, `aiContexts`, `aiActContext`, `modelConfig`, `cache`, `createOpenAIClient`, and `onTaskStartTip`.
...
- `customActions` adds additional custom actions to `AndroidDevice`. Pass it to the Device constructor or to `agentFromAdbDevice()`.
...
- `inputStrategy?: 'legacy' | 'sequential' | 'bulk'` — Default text input strategy for `Input` actions. Use `'sequential'` for one WDA call per code point or `'bulk'` for one WDA text call. `'bulk'` requires `keyboardTypeDelay` to be omitted or set to `0`; use `'sequential'` for delayed input. Default: `'legacy'`.
...
- Use one agent per device connection.
- `customActions` adds additional custom actions to `IOSDevice`. Pass it to the Device constructor or to `agentFromWebDriverAgent()`.
...
const agent = await agentFromWebDriverAgent({
  ...
  aiContexts: { aiAct: 'Accept permission dialogs automatically.' },
});
```
...
- `keyboardTypeDelay?: number` — Finite non-negative delay in milliseconds between keystrokes. A positive value makes legacy input enter one Unicode code point at a time through `uitest uiInput inputText`. Use this option when an input field drops characters during fast input.
- `inputStrategy?: 'legacy' | 'sequential' | 'bulk'` — Default text input strategy for `Input` actions. Use `'sequential'` for one HDC call per code point or `'bulk'` for one HDC text call. `'bulk'` requires `keyboardTypeDelay` to be omitted or set to `0`; use `'sequential'` for delayed input. Default: `'legacy'`.
- `screenshotResizeScale?: number` — **Deprecated.** This option has been removed and no longer has any effect. Use `screenshotShrinkFactor` in `AgentOpt` instead to control screenshot size sent to the AI model.
- `customActions?: DeviceAction[]` — Add additional custom actions so the Agent can call your domain-specific actions.
...
`ComputerDevice` and `RDPDevice` provide the following actions to the Midscene Agent:

- `Tap` — Left-click a target.
- `DoubleClick` — Double-click a target.
- `RightClick` — Right-click a target.
- `Hover` — Move the pointer to a target and hover over it.
- `DragAndDrop` — Drag from one location and drop at another.
- `Input` — Enter text with `replace`, `typeOnly`, and `clear` modes (`append` is a deprecated alias for `typeOnly`).
- `ClearInput` — Clear the contents of an input field.
- `KeyboardPress` — Press a key or key combination, optionally focusing a target first.
- `CursorMove` — Move the text cursor left or right within an input field or text area.
- `Scroll` — Scroll the screen or a specific area, including helpers to reach the top, bottom, left, or right boundary.
- `Swipe` — Press and continuously move the primary mouse button. Use a start point and endpoint, or a direction and distance, for direct gesture interactions such as sliders; use `Scroll` to browse off-screen content.
- `ListDisplays` — List available displays. A local device returns connected displays; an RDP device returns the current remote session as one display.
...
const agent = await agentForComputer({
  aiContexts: { aiAct: 'You are automating a desktop application.' },
});
...
const agent = await agentForRDPComputer({
  aiContexts: {
    aiAct: 'You are controlling a remote Windows desktop over the RDP protocol.',
  },
  host: '10.75.166.249',
  port: 3389,
  username: 'Admin',
...
`ComputerAgent` inherits the [shared Agent interaction methods](#interaction-methods). See the desktop action space above for desktop-specific actions and limitations.

## Runtime configuration {#runtime-configuration}

These environment variables control global runtime behavior rather than model requests. They are not supported in an Agent's `modelConfig` object.
...
Setting `MIDSCENE_PLAYGROUND_HOST=0.0.0.0` listens on all network interfaces. Use this value only on a trusted network.

### Debug logs {#debug-logs}

Set `DEBUG` to one of the following selectors:

| Value | Description |
| --- | --- |
| `midscene:ai:profile:stats` | Prints model latency and Token usage in a comma-separated format. |
| `midscene:ai:profile:detail` | Prints detailed Token usage logs. |
| `midscene:ai:call` | Prints AI response details. |
| `midscene:android:adb` | Prints Android ADB command details. |
| `midscene:*` | Prints all Midscene Debug logs. |

Midscene saves logs under `<MIDSCENE_RUN_DIR>/log` even when `DEBUG` is unset. Debug logs may contain model inputs, outputs, or screenshots. Review them before sharing, and do not commit them to a source repository.

For model connectivity checks, call recording, and Tracing integrations, see [Model debugging and observability](../model-debugging-observability).
