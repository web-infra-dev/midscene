...
- Parameters:
  ...
  **Recommended usage:**
  ...
  - `opt: object` — Configuration:
    - `keyName: string` — **Required.** Keyboard key to press, such as `'Enter'`, `'Tab'`, or `'Escape'`. Web and Computer devices support modifier shortcuts such as `'Control+A'` and `'Shift+Enter'`; use `'+'` to join the keys. Harmony also supports combinations, but requires named or physical keys such as `'Control+KeyA'` and `'Shift+Slash'`. Harmony intentionally does not convert printable characters such as `'A'` or `'?'` into physical keys because that mapping depends on the active keyboard layout; use `aiInput()` to enter text. The built-in Android and iOS devices support single keys only. Support in custom devices depends on their implementation. See the [shared key name definitions](https://github.com/web-infra-dev/midscene/blob/main/packages/shared/src/us-keyboard-layout.ts); actual support varies by platform.
    - `deepLocate?: boolean` — Whether to enable [Deep Locate](#deep-locate-deeplocate). The deprecated `deepThink` name remains supported. Default: `false`.
    - `context?: string` — Additional context for this call. See [Per-call context](#per-call-context).
    - `xpath?: string` — XPath for the target element. Midscene tries the XPath before consulting the cache or AI model. Default: `undefined`.
    - `cacheable?: boolean` — Whether this call can use the [cache](../caching.mdx). Default: `true`.
