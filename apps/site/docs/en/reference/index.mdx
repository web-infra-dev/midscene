...
Observe the screen over a defined time window while other Agent calls run. Stopping the observer returns a fixed `UIObservation` whose read-only insight methods evaluate transient UI, such as toasts, banners, and screen transitions, that a live single-screen insight might miss.
...
`UIObservation` implements the same read-only `InsightAPI` as `Agent`: `aiQuery()`, `aiBoolean()`, `aiNumber()`, `aiString()`, `aiAsk()`, and `aiAssert()`. It also exposes `frameCount`, `startedAt`, and `endedAt`. Call `observation.dispose()` after its last insight call to release its image files early; otherwise `agent.destroy()` releases them.
