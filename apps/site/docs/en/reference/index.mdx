...
- AI page selection actions — Browser Agent exposes `ListBrowserPages` and `SetActivePage` in its action space. `ListBrowserPages` shows eight tabs at a time, starting at offset 0 by default; pass `{ offset: 8 }` to see the next eight. An out-of-range offset fails so the Agent can start again at 0. AI plans can switch the active page by index, title, or URL. If an index is combined with a title or URL, every selector must match the same page. A mismatch fails the action, so the Agent must list the pages again. Page Agent does not expose these actions.
