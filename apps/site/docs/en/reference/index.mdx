...
- `screenshotStrategy?: 'auto' | 'always-yadb'` — Controls the screenshot method. Default: `'auto'`.
  - `'auto'` (default) — Tries scrcpy → `adb.takeScreenshot` → `screencap` → yadb in order.
  - `'always-yadb'` — Skips the earlier paths and captures directly via [yadb](https://github.com/ysbing/yadb). Use this when `screencap` produces black frames for secure pages (`FLAG_SECURE`) while yadb captures them correctly (e.g. rooted devices with a Magisk hook to bypass `FLAG_SECURE`, or Android 11 and below). Note that yadb can only capture the default display (displayId=0). Can also be set via the `MIDSCENE_ANDROID_SCREENSHOT_STRATEGY=always-yadb` environment variable.
