...
- `Swipe` — Press and continuously move the primary mouse button. Use this for sliders and other controls that require a start point plus an endpoint or direction and distance.
...
`ComputerDevice` and `RDPDevice` support the following actions. They expose the same default action names while using their respective local or RDP input backends.
...
<a id="desktop-swipe"></a>

**Swipe**

Press and continuously move the primary mouse button. Describe the control with a start point and endpoint, or describe the gesture with a direction and distance. Use this action for direct gesture interactions such as sliders, not for browsing off-screen content.

```typescript
await agent.aiAct('swipe the volume slider 300 pixels to the right');
await agent.aiAct('move the progress slider all the way to the right');
```

Use `Scroll` when the goal is to browse off-screen content.
