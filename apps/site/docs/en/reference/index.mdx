...
- `deviceId` and `useWDA` are present in the exported `IOSDeviceOpt` type for compatibility, but the current `IOSDevice` constructor does not read them. Use `wdaHost`, `wdaPort`, and `sessionId` to select the WDA endpoint or existing session.
- For shared interaction methods, see [Shared Agent APIs](#interaction-methods).
