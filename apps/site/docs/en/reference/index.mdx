...
- AI page selection actions — Browser Agent exposes `ListBrowserPages`, `GetBrowserPageInfo`, and `SetActivePage` in its action space. `ListBrowserPages` shows eight tabs at a time, starting at offset 0 by default; pass `{ offset: 8 }` to see the next eight. Long titles and URLs are truncated independently in planning feedback so similar pages remain distinguishable. Use `GetBrowserPageInfo({ index })` to read the complete title and URL without switching the active page. An out-of-range index or offset fails so the Agent can list the pages again. AI plans can switch the active page by index, title, or URL. If an index is combined with a title or URL, every selector must match the same page. Page Agent does not expose these actions.
