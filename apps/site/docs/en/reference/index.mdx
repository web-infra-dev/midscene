...
The artifact is a versioned JSON sequence of ordered screenshots, not encoded video, because this is the temporal input consumed by Midscene assertions. Each JPEG or PNG frame is embedded as a base64 image data URL. Use ordinary `assert` without `--record` when only the current screen matters. In Web Bridge mode, the foreground recording owns the single Bridge connection, so interact with that browser manually instead of starting a second Bridge CLI action.
