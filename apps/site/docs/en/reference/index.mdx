...
Unlike Extra Actions, an element XPath map records no operation type or value. One file can contain any number of elements:

```yaml title="element-xpaths.yaml"
elements:
...
Pass the file to one `aiAct` call:

```typescript
await agent.aiAct(
  'Fill first name with Alice, last name with Chen, and notes with "Hello"',
  {
    loadElementXpaths: ['./element-xpaths.yaml'],
  },
);
```
