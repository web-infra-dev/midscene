...
- AI page selection actions — Browser Agent exposes `ListBrowserPages` and `SetActivePage` in its action space. AI plans can inspect open tabs and switch the active page by index, title, or URL. If an index is combined with a title or URL, every selector must match the same page. A mismatch fails the action, so the Agent must list the pages again. Page Agent does not expose these actions.
