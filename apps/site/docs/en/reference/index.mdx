...
- Parameters:
  ...
  **Recommended usage:**
  ...
  - `opt: object` — Configuration:
    ...
    - `keyboardTypeDelay?: number` — Finite non-negative delay in milliseconds between keystrokes. A positive value makes legacy input enter one Unicode code point at a time. Use this option when an input field drops characters during fast input.
