...
function aiAct(
  ...
  options?: {
    cacheable?: boolean;
    deepThink?: 'unset' | 'auto' | true | false;
    deepLocate?: boolean;
...
- Parameters:
  ...
  - `options?: object` — Optional configuration:
    ...
    - `deepThink?: 'unset' | 'auto' | true | false` — Controls the planning implementation used by Midscene when `aiAct` performs planning. See [`deepThink` planning mode](#aiact-deepthink).
...
- When set to `true`, `aiAct` focuses more on task decomposition and, by default, separates task planning from element localization into different model calls. This can improve stability for complex tasks, but it also increases model calls and latency.

`deepThink` accepts `'unset' | 'auto' | true | false`. The legacy `'unset'` value behaves the same as `false`.

With `deepThink: 'auto'`, Midscene makes one additional call to the configured Planning model before planning. The decision is based primarily on the task description, supplemented by resolved user context; initial screenshots and reference images are secondary evidence. Auto defaults to enabling deep thinking and disables it only when the task clearly needs no explicit sub-goal tracking. Uncertainty keeps it enabled. Known field values or independent edits alone do not establish that a complex form or multi-object task needs no tracking. The selected mode stays fixed for the entire `aiAct` call.

The report includes a separate `Planning / DeepThink` step with the decision, reason, token usage, and timing. This step does not consume a replanning round. Executable plan-cache hits skip the decision; if cached execution fails and falls back to planning, the decision runs before planning starts. An invalid decision or a failed model request fails the call instead of silently selecting a mode. `auto` requires a standard Planning adapter; custom planners such as UI-TARS and AutoGLM are not supported. Explicit experimental `effort` still takes precedence over `deepThink`.

```ts
await agent.aiAct('Compare the available plans and select one that meets my requirements', {
  deepThink: 'auto',
});
```

The `MIDSCENE_PLANNING_SEPARATE_LOCATE` environment variable independently controls whether Planning and Locate use separate model calls. It applies to standard `aiAct` planning with `deepThink: true`, `false`, or `'auto'` (and experimental `effort`).

| Value | Behavior |
| --- | --- |
| `true` / `1` | Separate Planning and Locate, regardless of the selected DeepThink mode. |
| `false` / `0` | The Planning model produces both the action and its coordinates, even when DeepThink is enabled or a separate Planning model is configured. |
| Unset / empty | Preserve the default: DeepThink separates location; ordinary planning combines it only when using the default model. |

```bash
# Combine planning and location without changing sub-goal or memory settings.
export MIDSCENE_PLANNING_SEPARATE_LOCATE=false
```

The override is read once before planning, including before the `auto` decision, and stays fixed for that invocation. Reports record the requested `separateLocate` override and the effective `includeLocateInPlanning` value. Invalid values and explicit overrides for custom Planning adapters fail before model inference. Forcing combined calls requires a Planning model family with a supported coordinate format (`MIDSCENE_PLANNING_MODEL_FAMILY`, or `MIDSCENE_MODEL_FAMILY` for the default model); missing support is an error, not a silent fallback. Executable plan-cache hits still bypass planning. This setting does not change sub-goals, memory, screenshot history, or the separate `deepLocate` enhancement.

TaskScope can be controlled with `MIDSCENE_PLANNING_TASK_SCOPE`, and `MIDSCENE_PLANNING_SCREENSHOT_COUNT` overrides the screenshot limit independently of DeepThink. All standard modes allow necessary navigation while respecting explicit user restrictions. See [Planning configuration](../model-config) for defaults and validation.
...
- Parameters:
  ...
  - `options?: object` — Optional runtime options.
    ...
    - `deepThink?: 'unset' | 'auto' | true | false` — Passed to `aiAct` for `Given` and `When` steps.
