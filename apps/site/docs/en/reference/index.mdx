...
- `scrcpyConfig?: object` — High-performance scrcpy screenshot configuration:
  ...
  - `videoBitRate?: number` — scrcpy H.264 encoding bitrate in bits per second. Midscene defaults to `4000000` only when the device endpoint or ADB server host contains a literal non-loopback IP; otherwise it defaults to `100000000`. Port numbers don't affect this decision, so `127.0.0.1:5555` remains on the local default. An explicit value always takes precedence.
