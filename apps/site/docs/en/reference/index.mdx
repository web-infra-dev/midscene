...
`ComputerDevice` and `RDPDevice` provide the following pointer actions in addition to text input, keyboard, scrolling, and platform actions:

- `Tap`, `DoubleClick`, `RightClick`, and `Hover` — Standard mouse interactions.
- `DragAndDrop` — Move one UI element to another element.
