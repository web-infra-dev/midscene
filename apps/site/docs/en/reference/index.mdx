...
- Parameters:
  ...
  - `options?: object` — Optional configuration:
    ...
    - `deepThink?: 'unset' | 'auto' | true | false` — Deprecated and ignored. Planning uses sub-goals as needed; see [Planning and optional sub-goals](#aiact-deepthink).
...
// Planning tracks sub-goals when the task needs them
await agent.aiAct('Complete the GitHub account registration form. The region must be set to "Canada". Make sure no fields on the form are missed and all form fields pass validation. Just fill in the form fields without actually submitting the registration. Finally, return the actual content filled in the form fields');
```

##### Planning and optional sub-goals {#aiact-deepthink}

Standard `aiAct` planning analyzes the current state on every turn, preserves relevant Memory, and records Log preambles separately from execution results. The model creates sub-goals when needed to track dependent requirements, coordinated outcomes, or coverage; a simple task can proceed without them.

The `deepThink` and experimental `effort` options are deprecated and ignored. Existing callers can still pass them, but they do not select a planning mode or trigger an additional classification call.

Use `MIDSCENE_PLANNING_SCREENSHOT_COUNT=1/2` to set the execution screenshot limit and `MIDSCENE_PLANNING_SEPARATE_LOCATE=true/false` to choose separate or combined Planning/Locate calls. Their defaults, validation, and report fields are described in [Planning configuration](../model-config). Memory, Log, and TaskScope no longer have switches.

These controls do not change [model-native thinking](../model-config#model-native-reasoning).
...
- Parameters:
  ...
  - `options?: object` — Optional runtime options.
    ...
    - `deepThink?: 'unset' | 'auto' | true | false` — Deprecated; forwarded to `aiAct` for compatibility and ignored.
    - `deepLocate?: boolean` — Deprecated; forwarded to `aiAct` for compatibility and ignored.
...
The old `deepThink` name has different compatibility behavior across APIs:

- For `aiAct()`, it is deprecated and ignored. Planning uses sub-goals as needed; `deepLocate` still controls the location enhancement described here.
- For single-step methods such as `aiTap` and `aiHover`, the deprecated `deepThink` alias remains equivalent to `deepLocate`. Use `deepLocate` in new code.
