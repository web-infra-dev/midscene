...
- Parameters:
  ...
  - `options?: object` — Optional configuration. `uiContext` and image prompt options are supported. Single-element optimizations such as `xpath`, cache lookup, and `deepLocate` are not applied to `aiLocateAll()`; passing these options throws an error.
...
- Return value:
  ...
  - As with `aiLocate()`, `rect` may be an approximate box when the underlying model only supports point grounding. Prefer `center` when you only need a stable click target.
  - `dpr` is a Web-only compatibility field. Other Agent types do not guarantee this field.
