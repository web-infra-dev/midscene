...
With `deepThink: 'auto'`, Midscene makes one additional call to the configured Planning model before planning. It uses the task, resolved user context, initial screenshot, and reference images to choose whether to enable deep thinking. The selected mode stays fixed for the entire `aiAct` call. Straightforward actions tend to use ordinary planning; tasks requiring dependent goals, cross-screen memory, comparison, or conditional workflows tend to use deep thinking.
...
```bash
# Keep sub-goals and memory controlled by deepThink; combine planning and location.
