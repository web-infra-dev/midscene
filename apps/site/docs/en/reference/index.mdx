...
- Use underlying page/device APIs inside the handler. The same Agent's UI APIs also await the handler, so calling them creates a circular wait until timeout.
