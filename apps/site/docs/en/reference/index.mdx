...
- Parameters:
  ...
  - `options?: object` — Optional configuration:
    ...
    - `fileChooserAllowedDir?: string` — Trusted directory from which this `aiAct` call may select files mentioned in its prompt. The model must provide relative paths inside this directory; absolute paths and paths that escape it are rejected. Symlinks within the directory are allowed. Without this option, model-driven file uploads are rejected.
...
To let `aiAct` upload files mentioned in its prompt, pass a trusted `fileChooserAllowedDir` to that call. Midscene plans the file chooser setup before the action that opens the dialog. The model can only use relative paths inside this directory.
...
await agent.aiAct(
  ...
  { fileChooserAllowedDir: process.env.UPLOAD_ASSETS_DIR },
);
```
...
This capability is available for web pages (Playwright, Puppeteer, and Chrome extension Bridge mode). Without `fileChooserAllowedDir`, prompt-specified file paths are rejected; use `options.fileChooserAccept` when your application explicitly selects the files. The configured directory also makes the prompt portable across local and CI environments.
