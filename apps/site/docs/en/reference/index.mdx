...
- `globalContext: string` — Background context included in every AI-powered Agent API call. A method's per-call `options.context` is appended after it. Default: `undefined`.
...
Set `globalContext` when creating an Agent to provide background shared by every AI-powered Agent API call. Every `agent.ai*` method also accepts an optional `context` string in its options for business knowledge, user state, terminology, or other background relevant to one request.
...
The effective context is resolved as follows:

- `aiAct`: `globalContext + (options.context ?? aiActContext)`
- Other `ai*` methods: `globalContext + options.context`

For `aiAct`, a per-call `context` therefore continues to replace `aiActContext`, including when it is explicitly set to an empty string. It does not replace `globalContext`; other AI methods do not inherit `aiActContext`.
...
- All other fields match the [common constructor parameters](#common-parameters), including `generateReport`, `reportFileName`, `globalContext`, `aiActContext`, `modelConfig`, `cache`, `createOpenAIClient`, and `onTaskStartTip`.
