...
Observe the screen over a defined time window while other Agent calls run. Stopping the observer returns a fixed `UIObservation`. Its read-only insight methods evaluate transient UI, such as toasts, banners, and screen transitions, that a live single-screen insight might miss.
...
`UIObserver` only manages recording:

- `observer.bufferedFrameCount: number` — Number of frames currently buffered while recording.
- `observer.stop(): Promise<UIObservation>` — Stop sampling, capture the final representative screenshot, and return the fixed observation window.

`UIObservation` provides the same read-only insight methods as `Agent`: `aiQuery()`, `aiBoolean()`, `aiNumber()`, `aiString()`, `aiAsk()`, and `aiAssert()`. These methods only evaluate recorded screenshots. They do not read the current DOM, and their options do not support `domIncluded`. Passing `domIncluded` from untyped JavaScript throws an error instead of mixing live DOM data with the fixed observation window.

The observation also exposes `frameCount`, `startedAt`, and `endedAt`. Call `observation.dispose()` after its last insight call to release its image files early; otherwise `agent.destroy()` releases them.
...
The platform CLIs run recording in the foreground. Start it in one terminal,
perform the interaction manually or from another terminal, then press Ctrl+C
in the recording terminal to finalize the artifact:

```bash
# Terminal 1: this command keeps running until Ctrl+C
npx -y @midscene/web@1 record start \
  --output ./submission-observation.json

# Terminal 2, while Terminal 1 is recording
npx -y @midscene/web@1 act --prompt "click the Submit button"

# Back in Terminal 1: press Ctrl+C and wait for the saved-path message
...
`record start` is also available from `@midscene/android`, `@midscene/ios`, `@midscene/harmony`, and `@midscene/computer`. Pass normal target options and `--output` to that foreground command. Capture options include `--interval-ms`, `--max-frames`, and `--watchdog-ms`. The default watchdog finalizes and saves the recording after five minutes; pass `--watchdog-ms 0` only when an unbounded recording is intentional.

The artifact consists of a versioned JSON manifest and an adjacent image directory; it is neither encoded video nor an archive. For `submission-observation.json`, frames are stored under `submission-observation.frames/`. Keep or move the manifest and that directory together. The manifest contains relative JPEG or PNG paths and no base64 image bodies:

```json
{
  "type": "midscene_ui_observation",
  "version": 1,
  "startedAt": 1785132000000,
  "endedAt": 1785132001000,
  "frames": [
    {
      "path": "submission-observation.frames/4f32c0a8.jpeg",
      "mimeType": "image/jpeg",
      "capturedAt": 1785132000123
    }
  ],
...
Use ordinary `assert` without `--record` when only the current screen matters. In Web Bridge mode, the foreground recording owns the single Bridge connection, so interact with that browser manually instead of starting a second Bridge CLI action.
...
- Data-URL frames are written to image files during sampling so the observer does not retain the full sequence in memory. Opaque device frame handles are decoded in small batches and persisted when recording stops. Images are read lazily when the assertion is sent to the model.
