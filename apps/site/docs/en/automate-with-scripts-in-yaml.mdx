...
The `web/android` part defines the basic of a task, `web` scenario uses `web` parameter, and `android` scenario uses `android` parameter, they are mutually exclusive.
