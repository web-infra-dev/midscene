...
For steps whose prompt accepts `string | Object` (see the [API reference](./api.html#prompting-with-images)), replace the string value with an object that contains:

- `prompt`: The text description sent to the model.
...
This applies to prompt parameters on `aiTap`, `aiHover`, `aiDoubleClick`, `aiRightClick`, and the `prompt` field on `aiInput`, `aiKeyboardPress`, `aiScroll`, as well as prompt-based utilities like `aiAsk`, `aiQuery`, `aiBoolean`, `aiNumber`, `aiString`, and `aiAssert`.
...
tasks:
  - name: Verify branding
    flow:
      - aiAssert:
          prompt: Check whether the GitHub logo appears on the page.
          images:
            - name: GitHub logo
