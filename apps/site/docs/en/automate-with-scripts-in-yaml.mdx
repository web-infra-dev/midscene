...
agent:
  ...
  # Deprecated compatibility alias for contexts.aiAct, optional
  aiActContext: <string>
  # The older alias aiActionContext also remains supported
...
- **contexts**: Provides additional AI guidance such as business facts, rules, constraints, or output requirements. `default` is used only when neither the current call nor the matching API has its own context. A per-call `context` overrides the API value, which overrides `default`; these values are not automatically concatenated. Empty strings explicitly clear inherited user context.
- **aiActContext**: Deprecated compatibility field for `contexts.aiAct`. `contexts.aiAct` wins when both are present. The older `aiActionContext` alias also remains supported.
