...
For steps that accept prompts as `string | Object` (see the [API reference](./api.html#prompting-with-images)), you can provide an object containing the text prompt and an `images` array. This applies to `aiTap`, `aiHover`, `aiInput.locate`, `aiKeyboardPress.locate`, `aiScroll.locate`, `aiDoubleClick`, `aiRightClick`, `aiAsk`, `aiQuery`, `aiBoolean`, `aiNumber`, `aiString`, `aiAssert`.
...
tasks:
  - name: Verify branding
    flow:
      - aiTap:
          locate:
            prompt: Tap the primary call-to-action button that matches the highlighted design.
            images:
              - name: primary button reference
                url: https://example.com/button.png
            convertHttpImage2Base64: true
      - aiAssert:
          prompt: Confirm the primary button matches the supplied design reference.
          images:
            - name: primary button reference
              url: https://example.com/button.png
```
