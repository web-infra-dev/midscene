...
agent:
  ...
  aiActContext: <string>
  # Legacy alias that is still supported for backward compatibility
...
- **aiActContext**: Provides background knowledge to the AI model, like how to handle popups, business introduction, etc. The legacy `aiActionContext` name is still accepted for backward compatibility.
