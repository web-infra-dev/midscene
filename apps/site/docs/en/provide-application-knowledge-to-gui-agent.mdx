...
#### Configure Context in @midscene/test

With `@midscene/test`, create the Agent in `setup` in `midscene.config.ts`, pass application knowledge through `aiContexts.aiAct`, and provide that Agent to `createMidsceneNodes`. Continuing with the Android example, replace `deviceId` with the ID of a connected device and make sure the app is open on its home page:
...
The Case YAML only needs to describe the goals. Every `aiAct` step using this Agent receives the application knowledge configured above:

```yaml filename=cases/ranking.yaml
cases:
  - name: Filter the price-reduction ranking
    steps:
      - aiAct: Open the price-reduction ranking
      - aiAct: Filter for new-energy MPVs launched within the past year and apply a custom price range of CNY 100,000–400,000
```

`aiActContext` is a deprecated alias for `aiContexts.aiAct` and remains supported; use `aiContexts.aiAct` in new configurations. This configures the Agent's default context, not a top-level field in Case YAML. The project `context` returned by `setup` shares resources such as the Agent between Nodes; it is not automatically sent to the model as text. See [Midscene Test configuration](./midscene-test/configuration) for more details.
...
In `@midscene/test` Case YAML, use the object form of `aiAct` and pass per-call context through `options.context`:

```yaml
- aiAct:
    prompt: Set the price range to CNY 100,000–400,000 and apply the filter
    options:
      context: |
        Custom prices can be set by tapping tick marks. Set the upper bound first,
        then the lower bound. After adjusting the range, tap the result button
