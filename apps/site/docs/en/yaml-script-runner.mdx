...
### Consume the final result as JSON

Pass `--json` when another program or Agent needs to consume the result directly:

```bash
midscene ./bing-search.yaml --json
```

In this mode, the command prints one machine-readable JSON object without progress logs. Its `summary` contains execution statistics and the generated summary-file path. Each item in `results` contains the YAML file path, status, report path, output-file path, and parsed YAML output:
...
```json
{
  ...
  "results": [
    {
      "file": "/path/to/case.yaml",
      "report": "/path/to/midscene_run/report/case.html",
      "outputPath": "/path/to/midscene_run/output/case.json",
      "output": { "key": "value" }
    }
  ]
}
...
- `--json`: Print one JSON object containing summary statistics and the summary-file path, plus each script's status, report path, output-file path, and parsed YAML output. No progress logs are printed.
