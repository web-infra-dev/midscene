...
- `--setup <file>`: A setup script that runs before the main `--files`. It runs inside the same shared browser context, so context-scoped prerequisite state such as cookies and same-origin `localStorage` is visible to every main script. Every YAML script, including setup and serial scripts, runs in its own tab; page-scoped state such as `sessionStorage` is not carried between scripts. A setup failure aborts the whole batch and the main scripts are reported as not executed. Requires `--share-browser-context`.
...
- `--share-browser-context`: Share one browser context (cookies, same-origin `localStorage`, etc.) across scripts while giving every YAML script an independent tab. Page-scoped state such as `sessionStorage`, the DOM, URL, and `window.name` is not shared, even with `--concurrent 1`. Default off.
...
When several scripts all depend on the same prerequisite (for example a login), put the prerequisite under `setup`. The setup script runs before the main `files`; once it succeeds, the main scripts run with the configured concurrency. Setup and main scripts share one browser context, so cookies and same-origin `localStorage` carry over. Every YAML script receives an independent tab, including when `concurrent` is `1`. The setup tab's `sessionStorage`, DOM, URL, `window.name`, and other page-scoped state are not copied into main tabs. `setup` requires `shareBrowserContext: true`.

:::warning Page-scoped state is not shared
`shareBrowserContext` follows browser-native storage boundaries: it does not copy or synchronize `sessionStorage` between tabs. If setup establishes authentication only in `sessionStorage`, the main scripts will not inherit that login. Prefer cookies, same-origin `localStorage`, or backend state for prerequisites that must be visible to multiple scripts. When scripts run concurrently, shared-state writes can race, so coordinate them explicitly.
:::
