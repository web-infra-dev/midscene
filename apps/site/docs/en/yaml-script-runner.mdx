...
- `--share-browser-context`: Share one browser context (cookies, `localStorage`, etc.) across scripts while giving every script an independent tab. `sessionStorage` is tab-scoped; when `--setup` is used, its initial same-origin values are copied into each main tab. Default off.
...
When several independent scripts all depend on the same prerequisite (for example a login), put the prerequisite under `setup`. The setup script runs before the main `files`; once it succeeds, the main scripts run with the configured concurrency. Setup and main scripts share one browser context, so cookies and origin storage carry over, but every YAML script receives its own tab. The setup tab's same-origin `sessionStorage` is copied into each main tab before its first navigation; subsequent changes are isolated. The setup page must finish at a non-opaque origin so its `sessionStorage` has a transferable origin. `setup` requires `shareBrowserContext: true`.
