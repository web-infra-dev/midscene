...
:::info Shared execution with Midscene Test
Existing projects can keep `tasks/flow`, batch configuration, directory layout, and the `midscene` command. The upgraded entry uses the Test kernel and generates the new Test report page. You can also run original files with `midscene-test ./flow.yaml` or `midscene-test --config ./batch.yaml`, without registering Nodes or rewriting YAML.

Legacy task continuation, whole-file retries, environment variables, result outputs, and summaries retain their semantics. Native `cases/steps` have different defaults for failure and retry policies; see [YAML field mapping](./midscene-test/extend#map-yaml-fields-to-native-capabilities) when converting manually. Regenerate old HTML reports by rerunning the tests; no report-file conversion is required.
:::
