...
- `--reuse-page`: Reuse one Page across Puppeteer Web scripts. This preserves page-scoped state such as `sessionStorage`, the current tab, and `window.name`. It requires `--share-browser-context` and `--concurrent 1`. Default off.
...
For a sequential workflow that must keep using the setup tab, enable page reuse explicitly:
...
The main script still applies its own Web target configuration and navigates to its configured `url`, but it does so in the same Page created for setup.
