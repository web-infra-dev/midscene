...
- `--share-browser-context`: Share one browser context (cookies, `localStorage`, etc.) across scripts. With `--concurrent 1`, the batch reuses one tab, so page-scoped state such as `sessionStorage` remains continuous. With concurrency greater than `1`, each script receives an independent tab; see the parallel limitation below. Default off.
...
#### Run a setup before scripts

When several scripts all depend on the same prerequisite (for example a login), put the prerequisite under `setup`. The setup script runs before the main `files`; once it succeeds, the main scripts run with the configured concurrency. Setup and main scripts share one browser context, so cookies and origin storage carry over. With `concurrent: 1`, setup and every main script reuse one tab, preserving `sessionStorage` and other page-scoped state between scripts. With concurrency greater than `1`, every main script receives an independent tab. The setup tab's same-origin `sessionStorage` is copied into each main tab before its first navigation. The setup page must finish at a non-opaque origin so its `sessionStorage` has a transferable origin. `setup` requires `shareBrowserContext: true`.

:::warning Parallel shared-context limitation
When `concurrent` is greater than `1`, the main scripts run in independent tabs. Each tab receives only the initial same-origin `sessionStorage` snapshot captured after `setup`; later `sessionStorage`, DOM, and other page-scoped changes are not shared between scripts. Cookies and `localStorage` remain shared, but concurrent writes can race. Parallel scripts must not depend on execution order or another script's page state. Use cookies, `localStorage` with explicit coordination, or backend state when tasks need mutable shared data.
:::
