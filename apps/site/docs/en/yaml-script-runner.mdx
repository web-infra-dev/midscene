...
You can also install `@midscene/test` and run the original files with `midscene-test ./flow.yaml` or `midscene-test --config ./batch.yaml`, without registering Nodes or rewriting YAML. Use native `cases/steps`, TypeScript project configuration, and Nodes for new and refactored work. See [Migrate from legacy YAML](./midscene-test/migrate-from-yaml) for the steps and mappings.
:::
