...
- `--setup-files <file1> <file2> ...`: Setup scripts that run serially, in order, before the main `--files`. They run inside the same shared browser context, so prerequisite state such as a login established here is visible to every main script. Any setup failure aborts the whole batch and the main scripts are reported as not executed. Requires `--share-browser-context`.
...
#### Run a serial setup before parallel scripts

When several independent scripts all depend on the same prerequisite (for example a login), list the prerequisite under `setupFiles`. Setup scripts run serially, in order, before the main `files`; once they all succeed, the main scripts run with the configured concurrency. Setup and main scripts share one browser context, so the login state is carried over. `setupFiles` requires `shareBrowserContext: true`.
...
If any setup script fails, the batch is aborted and the main scripts are reported as not executed.
