...
In Android, there is an additional parameter:

* `autoDismissKeyboard: boolean`: If true, the keyboard will be dismissed after input text. (Default: true)
...
// Use deepThink feature to precisely locate the element
await agent.aiRightClick('The file name at the top of the page', { deepThink: true });
```
...
```typescript
function aiQuery<T>(dataShape: string | Object, options?: Object): Promise<T>;
```

* Parameters:
  * `dataShape: T`: A description of the expected return format.
  * `options?: Object` - Optional, a configuration object containing:
    * `domIncluded?: boolean` - Whether to send simplified DOM information to the model, usually used for extracting invisible attributes like image links. Default: False
...
const dataD = await agent.aiQuery('{name: string, age: string, avatarUrl: string}[], table data records', { domIncluded: true });
```
...
// Use domIncluded feature to extract invisible attributes
const boolB = await agent.aiBoolean('Whether the login button has a link', { domIncluded: true });
```
...
// Use domIncluded feature to extract invisible attributes
const numberB = await agent.aiNumber('The value of the remaining points element', { domIncluded: true });
```
...
// Use domIncluded feature to extract invisible attributes
const stringB = await agent.aiString('The link of the first item in the list', { domIncluded: true });
```
