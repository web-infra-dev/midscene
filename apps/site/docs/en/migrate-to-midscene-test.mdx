...
This example changes the input structure without changing business execution order. During a real migration, run `pnpm exec midscene-test nodes --project <name>` to generate the current Project's `midscene-node-spec.<project-name>.md`. Use this Node Spec—not examples from another platform—to confirm whether `gotoUrl`, `aiInput`, `aiWaitFor`, and the other referenced Nodes are available and how to provide their inputs.
