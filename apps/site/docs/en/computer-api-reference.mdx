...
### `agentFromDesktop(opts?): Promise<ComputerAgent>`
...
Different platforms have different keyboard layouts and shortcuts:
...
- Primary modifier: `Control` (Ctrl)
- System key: `Super` key
- Shortcuts vary by desktop environment

## Key Name Mapping

The following key name aliases are supported:

| User Input | Mapped To |
|-----------|----------|
| Windows, Win | win |
| Cmd | command (for modifiers) / cmd (for single press) |
| Ctrl | control |
| Esc | escape |
| Del | delete |
| ArrowUp/Down/Left/Right | up/down/left/right |
| VolumeDown/Up | audio_vol_down/up |
| MediaPlay/Pause/Stop | audio_play/pause/stop |
