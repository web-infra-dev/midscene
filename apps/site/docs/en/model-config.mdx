...
| `MIDSCENE_MODEL_REASONING_EFFORT` | Controls reasoning effort level. Mapped to model-specific parameters based on `MIDSCENE_MODEL_FAMILY` (e.g., `reasoning_effort` for doubao, `reasoning.effort` for gpt-5, `thinking_budget` for qwen). Common values: `off`, `low`, `medium`, `high` |
