...
### Planning screenshots and Locate

Standard `aiAct` planning always includes planning analysis, Memory, Log, and task-scope/completion rules. The model creates and updates sub-goals only when the task needs them; simple tasks can proceed without a plan. There is no separate Auto classification call.

The former `MIDSCENE_PLANNING_MEMORY`, `MIDSCENE_PLANNING_LOG`, and `MIDSCENE_PLANNING_TASK_SCOPE` environment variables have been removed. The `deepThink` and experimental `effort` API options are accepted for compatibility but ignored, with a deprecation warning. These options no longer change the prompt, screenshots, or Locate strategy.

| Name | Description |
|------|-------------|
| `MIDSCENE_PLANNING_SCREENSHOT_COUNT` | Accepts `1` (default) or `2`. `1` sends the current execution screenshot; `2` sends it and the most recent historical screenshot when available. The first round has only one screenshot. User reference images are counted separately. |
| `MIDSCENE_PLANNING_SEPARATE_LOCATE` | `true` / `1` uses separate Planning and Locate calls. `false` / `0` asks Planning for both the action and coordinates. When unset or empty, these are combined when using the default model and separated when a dedicated Planning model is configured. |

These settings are captured once per `aiAct` invocation and recorded in Plan reports. The Locate boolean is case-insensitive. Invalid values or explicit use with custom Planning adapters fail before model inference. Combined calls require a Planning model family with a supported coordinate format. Executable plan-cache hits still bypass planning.

Memory preserves observed facts, while Log includes model preambles and cumulative execution results. A preamble describes an intended action; it is not execution evidence. Task-scope rules remain part of Completion Rules, including explicit stopping points and restrictions on unrequested submission or saving.

```ini
MIDSCENE_PLANNING_SCREENSHOT_COUNT=2
MIDSCENE_PLANNING_SEPARATE_LOCATE=true
```
