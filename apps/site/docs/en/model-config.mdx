...
DeepThink continues to control sub-goals; it no longer enables or disables memory or log. The existing Locate strategy, screenshot history limits, and other planning rules are unchanged. In particular, `MIDSCENE_PLANNING_SEPARATE_LOCATE` continues to control independent Locate calls.
