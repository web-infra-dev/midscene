...
Screenshots and reference images are prepared before model messages are built. PNG inputs are compressed to WebP; existing JPEG and WebP images are reused when no pixel changes are needed. Context shrinking, cropping, padding, and annotation are composed in a shared image pipeline, with encoding only at the end. The model transport sends the prepared images without transcoding them. Remote image URLs are left unchanged. The original capture is retained for reports, independently of model output encoding.
