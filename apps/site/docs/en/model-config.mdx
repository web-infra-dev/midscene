...
| `MIDSCENE_PLANNING_SCREENSHOT_COUNT` | A positive safe integer limiting execution screenshots per Planning request, including the current screenshot. `1` means current only; `2` means current plus the most recent historical screenshot. Reference images are separate. Fewer images are sent when history is unavailable; older images may already have been removed by history compression. |
