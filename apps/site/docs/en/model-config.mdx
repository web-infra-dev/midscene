...
| `MIDSCENE_MODEL_BASE_URL` | API endpoint URL, usually ending with a version (e.g., `/v1`); do not append `/chat/completions` or `/responses` here since the underlying sdk will add it automatically |
| `MIDSCENE_MODEL_NAME` | Model name |
| `MIDSCENE_MODEL_FAMILY` | Model family, determine the way of dealing with the coordinates |

## Protocol type {#model-api-type}

Midscene uses OpenAI's Chat Completions protocol by default and also supports the Responses protocol. These are different ways to send requests to models; your use of the Midscene Agent API stays the same. Chat Completions was introduced earlier than Responses and currently has broader support.

Select the protocol through an environment variable: `chat-completion` (the default) or `responses`. For example, to use Responses:

```bash
MIDSCENE_MODEL_API_TYPE="responses"
```

When configuring separate Planning or Insight models, use the corresponding environment variables. Each model can use a different protocol:
...
- Some models or providers may not yet support the Responses protocol. Check with your model provider before using it.
- Even the Chat Completions and Responses APIs from the same provider may use different Base URLs. Confirm the endpoint for each protocol with your provider.
- Midscene may not yet support some providers' Responses API implementations. Confirm compatibility with Midscene before using them.
...
| Name | Description |
|------|-------------|
| `MIDSCENE_MODEL_API_TYPE` | HTTP protocol: `chat-completion` (default) or `responses`. The latter requires a provider that supports the Responses API. |
...
| `MIDSCENE_MODEL_RESPONSE_FORMAT` | Structured response strategy: `auto` (default) lets Midscene automatically use `response_format` (Chat Completions) or `text.format` (Responses) in appropriate scenarios to specify a structured output format (usually JSON), making the model response as suitable for structured parsing as possible; `none` does not set those format parameters, for models that do not support structured output. |
...
| `MIDSCENE_MODEL_EXTRA_BODY_JSON` | JSON blob merged into each model request body. Unlike `MIDSCENE_MODEL_INIT_CONFIG_JSON` (which configures the SDK client), this is spread into every protocol request sent to the model, e.g. enabling thinking mode in vLLM: `'{"chat_template_kwargs":{"enable_thinking":true}}'` |
...
| `MIDSCENE_INSIGHT_MODEL_BASE_URL` | API endpoint URL (omit the trailing `/chat/completions` or `/responses`) |
| `MIDSCENE_INSIGHT_MODEL_API_TYPE` | HTTP protocol: `chat-completion` (default) or `responses`. |
...
| `MIDSCENE_PLANNING_MODEL_BASE_URL` | API endpoint URL (omit the trailing `/chat/completions` or `/responses`) |
| `MIDSCENE_PLANNING_MODEL_API_TYPE` | HTTP protocol: `chat-completion` (default) or `responses`. |
