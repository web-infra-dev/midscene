...
For more details, refer to the [Model configuration](./model-config#configure-models-by-task-type-advanced) documentation.
...
### Doubao Vision Language Models (✨ recommended)
...
For more details and example configurations, please see [Model configuration](./model-config).
...
* To learn more about the model configuration, see [Model configuration](./model-config)
...
Some configurations used in Midscene 0.x versions (such as `OPENAI_API_KEY` `OPENAI_BASE_URL` `MIDSCENE_USE_QWEN_VL`) remain compatible in 1.x versions. For details, see [Compatible with Midscene 0.x configurations](./model-config.mdx).
