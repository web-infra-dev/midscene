...
When you need to customize device behavior, integrate Midscene into a standalone framework, or troubleshoot HDC issues, refer to this section. See [API reference](./api#harmonyagent) for more constructor parameters.
...
- HarmonyOS-specific parameters and interfaces: [API reference](./api#harmonyagent)
- Demo projects
...
Midscene automatically dismisses the keyboard after entering text. HarmonyOS defaults to the ESC key to avoid triggering page-back. If ESC doesn't close the keyboard in your app, set `keyboardDismissStrategy: 'back-first'`. If your input field listens for Back and clears, set `autoDismissKeyboard: false` and use `aiAct` to manually dismiss the keyboard.

For full constructor signatures, see the [API reference](./api#harmonydevice).

### How to use a custom HDC path?

Set the `HDC_HOME` environment variable, or pass `hdcPath` via the `HarmonyDevice` constructor. See the [API reference](./api#harmonydevice) for details.
