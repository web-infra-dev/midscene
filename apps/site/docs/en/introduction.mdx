...
- **iOS Automation**: Use [Javascript SDK](./ios-getting-started) with WebDriverAgent to control your local iOS device.
...
- **MCP**: Allows other MCP Clients to directly use Midscene's capabilities across web bridge mode and Android. Read the [MCP server guide](./mcp) for both setups.
...
- **[Android Playground](./android-playground)**: There is also a built-in Android playground to control your local Android device.
- **[iOS Playground](./ios-playground)**: There is also a built-in iOS playground to control your local iOS device.

## Model choices

Midscene recommends using vision models for UI automation to improve task success rates and speed up debugging.

- **Vision-first**: Prioritize vision models with strong UI understanding to achieve more accurate targeting and actions.
- **Supported models**: Doubao Seed (vision model), Qwen3-VL, Gemini-3-ProPro, etc.
- **Multi-model combo**: Configure different models for task planning and interaction phases. You can mix commercial and open-source models to further improve completion rates. See [Multi-model setup](./model-strategy#advanced-combining-multiple-models) for details.

Please read [Model strategy](./model-strategy) to learn more about model selection.
...
* **Debugging Experience**: When you start using UI automation, you'll find debugging and maintaining automation scripts is the real pain point. No matter how powerful the model is, you still need to invest in debugging to keep it stable. Midscene.js offers a visualized report file, a built-in playground, and a Chrome Extension to simplify the debugging process. These are the tools most developers truly need, and we're continually working to improve the debugging experience.
