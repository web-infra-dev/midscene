...
Traditional UI tests lean on selectors and the DOM, so they break whenever the interface is refactored — and they can't reach `<canvas>`, native mobile apps, or cross-origin iframes at all. Keeping a suite green turns into a constant maintenance tax.

Midscene takes a different route: it **locates elements from the screenshot alone**, using a vision-language model, and you describe each step in natural language. That changes what UI testing feels like:
...
- **Test anything you can see.** Web, Android, iOS, HarmonyOS, and desktop — one API, the same way everywhere, including surfaces the DOM can't describe.
