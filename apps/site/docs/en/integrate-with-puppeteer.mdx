...
Each Agent instance is bound to a single page. Midscene intercepts new tabs by default (e.g. `target="_blank"` links) and opens them in the current page. To allow new tabs, set the `forceSameTabNavigation` constructor option to `false` and create a new Agent for each tab.

For the full constructor signature, see the [API reference](./api#puppeteeragent-web).
...
- For the Puppeteer API reference, see [API reference](./api#puppeteeragent-web).
- Demo projects
