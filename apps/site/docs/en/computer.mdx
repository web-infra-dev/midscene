...
Configure the desktop environment permissions and display dependencies before running desktop automation.
...
Xvfb creates a virtual display so that mouse, keyboard, and screenshot operations work without a physical monitor. See [API reference](./api#agentforcomputer-pc-desktop) for details.
...
For the full list of constructor options (`host`, `port`, `username`, `password`, `domain`, `ignoreCertificate`, `desktopWidth`, `desktopHeight`, `adminSession`), see the [API reference](./api#agentforrdpcomputer-pc-desktop).

## Multi-Display Support

Use `ComputerDevice.listDisplays()` and the `displayId` option to control a specific display:
...
See the [API reference](./api#computerdevice-utilities) for `listDisplays()` and other utility methods.
...
Use `checkComputerEnvironment()` to verify your system configuration:
...
See the [API reference](./api#computerdevice-utilities) for the full return type.
...
- [API reference](./api#agentforcomputer-pc-desktop)
