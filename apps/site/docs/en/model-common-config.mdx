...
| Model version | Commonly used model names | `MIDSCENE_MODEL_FAMILY` | Notes |
| --- | --- | --- | --- |
| GPT-6 series | `gpt-6-astra`, `gpt-6-sol`, `gpt-6-luna` | `gpt-6` | GPT-6 Sol delivers excellent visual localization accuracy at just one-fifth the price of GPT-6 Astra. GPT-6 Luna is very inexpensive while also offering good visual localization capabilities. |
...
Environment variable configuration example, using `gpt-6-sol`:
...
MIDSCENE_PLANNING_MODEL_NAME="gpt-6-sol"
...
MIDSCENE_INSIGHT_MODEL_NAME="gpt-6-sol"
MIDSCENE_INSIGHT_MODEL_FAMILY="gpt-6"
```

  </ModelConfigTab>

  <ModelConfigTab type="default" apiType="responses">
...
```
  ...
  <ModelConfigTab type="planning" apiType="responses">
...
```
  ...
  <ModelConfigTab type="insight" apiType="responses">

```bash
MIDSCENE_INSIGHT_MODEL_API_TYPE="responses"
MIDSCENE_INSIGHT_MODEL_BASE_URL="https://api.openai.com/v1" # OpenAI API endpoint; or your compatible endpoint
MIDSCENE_INSIGHT_MODEL_API_KEY="sk-..."
...
- For UI localization with GPT, Midscene currently supports `gpt-5.4` and later models. To get the best localization quality, image requests need `"detail": "original"`. According to OpenAI, this option is available on `gpt-5.4` and future models, while some smaller GPT-5 variants, as well as older models, do not support `original` and will fail if you send it. See the [Images and Vision guide](https://developers.openai.com/api/docs/guides/images-vision) and the [Computer use guide](https://developers.openai.com/api/docs/guides/tools-computer-use).
...
- Azure OpenAI's Chat Completions API may not handle `"detail": "original"` correctly, causing click-coordinate offsets. If you encounter this issue, you can switch to the Responses API. See [Clicks are offset when using Azure OpenAI](./faq#clicks-are-offset-when-using-azure-openai).
...
| Model version | Commonly used model names | `MIDSCENE_MODEL_FAMILY` | Notes |
| --- | --- | --- | --- |
| V2.x series | `mimo-v2.6-pro`, `mimo-v2.6-flash`, `mimo-v2.6-pro-ultraspeed`, `mimo-v2.5` | `xiaomi-mimo` | All 2.6 models support multimodal input; for 2.5, only the Omni series supports multimodal input. |

</div>

Environment variable configuration example, using `mimo-v2.6-pro`:
...
MIDSCENE_PLANNING_MODEL_API_KEY="......"
MIDSCENE_PLANNING_MODEL_NAME="mimo-v2.6-pro"
...
MIDSCENE_INSIGHT_MODEL_API_KEY="......"
MIDSCENE_INSIGHT_MODEL_NAME="mimo-v2.6-pro"
