...
### Doubao-X CUA (Experimental) {#doubao-x}

Use `doubao-x` with an Ark GUI task-processing endpoint that follows the Seed Computer Use XML65 action protocol:

```bash
MIDSCENE_MODEL_BASE_URL="https://ark.cn-beijing.volces.com/api/v3"
MIDSCENE_MODEL_API_KEY="...."
MIDSCENE_MODEL_NAME="ep-xxxx" # GUI task-processing endpoint ID from Volcano Engine
MIDSCENE_MODEL_FAMILY="doubao-x"
```

Midscene translates the actions available on the current device into the model's XML65 action definitions at runtime. This family is intended for `aiAct` and `aiTap`; use a separate Insight model for APIs such as `aiAssert` and `aiQuery`.
