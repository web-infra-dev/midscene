...
The returned result must belong to the scheduled project, document, and Case. Midscene Test rejects mismatched identities so a remote service cannot accidentally attach another Case's result. The framework does not contain provider-specific SDKs or provision remote resources on its own.
