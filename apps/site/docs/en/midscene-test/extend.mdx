...
- [Register a Node](#register-custom-business-nodes): define an operation, call it from YAML, and verify registration through the generated Node Spec.
...
TypeScript infers the type of `input` from the schema. Field descriptions written with `.describe()` appear in the generated Node Spec.
...
### 3. Generate a Node Spec to verify registration

After saving the configuration, generate the Node Spec:

```bash
pnpm exec midscene-test nodes
```

The command loads the project configuration and writes `midscene-node-reference.md` to the current directory. When `projects` is declared, it generates `midscene-node-reference.<project-name>.md` for each Project instead. Check that the Spec includes:
...
This verifies that the Node is registered and its input schema can be exported; it does not execute the Node or create user data. After changing Node definitions or registrations, you can regenerate the Spec for case authors and AI Agents to consult. For complete usage, see [Generate the current Project's Node Spec](./use#generate-the-current-projects-node-spec).
