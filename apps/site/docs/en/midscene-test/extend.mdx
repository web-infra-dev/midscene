...
The command loads the project configuration and writes `midscene-node-spec.md` to the current directory. When `projects` is declared, it generates `midscene-node-spec.<project-name>.md` for each Project instead. Check that the Spec includes:
