...
For a config already built as JavaScript, pass its `.mjs` path to the CLI's `--config` option or to `loadTestProject()`. The `.mjs` file is loaded as native ESM; it is not transformed as TypeScript. The default `midscene.config.ts` workflow remains unchanged.
...
- **`loadTestProject()`**: asynchronously loads a TypeScript (`.ts`) or JavaScript ESM (`.mjs`) project configuration. Midscene Test does not support synchronous loading.
