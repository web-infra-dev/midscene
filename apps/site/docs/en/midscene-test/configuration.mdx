...
| `executor` | Optionally delegates each isolated Case to another process or service. Requires `test.executionUnit: 'case'`. |
...
`aiAssert`, `aiWaitFor`, `aiBoolean`, `aiNumber`, `aiString`, `aiAsk`, `recordToReport`,
and `wait`.
...
`getTestRunnerNodeDefinitions()`. PlaywrightAgent, PlaywrightPageAgent, PlaywrightBrowserAgent, PuppeteerAgent, PuppeteerPageAgent, PuppeteerBrowserAgent, AndroidAgent, IOSAgent, and HarmonyAgent register their common and platform Node definitions together. The base Agent and ComputerAgent only provide common Agent Nodes.
...
Puppeteer exposes the same `gotoUrl`, `setCookies`, `clearCookies`, and `setViewportSize` Node names. Install the `puppeteer` peer dependency and pass `PuppeteerAgent` (or `PuppeteerBrowserAgent`) to `createMidsceneNodes()`:
...
const midsceneNodes = createMidsceneNodes<ProjectContext>({
  agentClass: PuppeteerAgent,
  getAgent: ({ context }) => {
    context.agent ??= new PuppeteerAgent(context.page, {
      testRunner: { baseURL: 'https://example.com' },
    });
    return context.agent;
  },
});
...
The Playwright and Puppeteer integrations use the same Web Node schemas and execution implementation. Browser-specific code is limited to a driver adapter. For portable YAML, use `domcontentloaded`, `load`, or `networkidle` for `gotoUrl.waitUntil`; the Puppeteer adapter maps `networkidle` to `networkidle0`. Existing engine-specific values remain supported: Playwright accepts `commit`, while Puppeteer accepts `networkidle0` and `networkidle2`. Using one of those values with the other driver produces an explicit error. Cookie references and result redaction also behave the same on both integrations. Relative URLs resolve against `testRunner.baseURL`, then against the current HTTP(S) page URL.
...
export default defineTestProject<ProjectContext>({
  ...
  test: {
    executionUnit: 'project', // Use 'case' to isolate and schedule each Case
    maxConcurrency: 1, // Maximum number of active scheduling units
    bail: 0, // Stop scheduling new tasks after this many failed cases when > 0
    testTimeout: 120_000,
    executorRetry: 0, // Retry classified executor failures, not Case failures
  },
  output: {
...
`test.executionUnit` selects the scheduling unit. Its default value, `project`, preserves the shared-project lifecycle: `setup` runs once and YAML files and Cases within a project run sequentially. `test.maxConcurrency` then limits the number of active projects.

Set `executionUnit: 'case'` to isolate and schedule every Case independently. Each Case gets its own project `setup`, document `beforeAll`/`afterAll`, and project cleanup. `maxConcurrency` limits active Cases. Add `resources` to a Case when two Cases must not use the same account, device, or other exclusive resource at the same time:

```yaml
cases:
  - id: update-profile
    name: Update profile
    resources: [account:member]
    steps:
      - aiAct: Change the nickname to Midscene User
```
...
export default defineTestProject<ProjectContext>({
  setup: playwrightSetup,
  nodes: midsceneNodes,
  test: { executionUnit: 'case', maxConcurrency: 4 },
});
```

Cases with a shared resource key wait for each other; unrelated Cases can still run concurrently. Resource keys are project-defined strings and do not provision resources by themselves.

An explicit Case `id` is also used as a single result-directory segment. It must be at most 128 characters, start with an ASCII letter or number, and contain only letters, numbers, dots, underscores, or hyphens. When `id` is omitted, Midscene generates a safe deterministic ID from the Project, source path, and Case position. Because the generated value includes the Case position, inserting or reordering Cases changes it. Declare an explicit `id` before persisting `--case-id` selections in CI or external schedulers.

By default, case scheduling runs asynchronously in the same Node.js process. To delegate isolated Cases to another process, container, device service, or remote runner, configure a custom [Test Executor](./extend#delegate-cases-with-a-test-executor). Midscene Test has no dependency on a specific infrastructure provider.
