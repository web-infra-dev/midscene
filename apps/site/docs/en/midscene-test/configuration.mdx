...
Cookie references and result redaction behave the same on both browser integrations. Puppeteer supports `domcontentloaded`, `load`, `networkidle0`, and `networkidle2` for `gotoUrl.waitUntil`. Relative URLs resolve against `testRunner.baseURL`, then against the current HTTP(S) page URL.
