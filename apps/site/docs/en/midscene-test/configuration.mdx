...
An explicit Case `id` is also used as a single result-directory segment. It must be at most 128 characters, start with an ASCII letter or number, and contain only letters, numbers, dots, underscores, or hyphens. When `id` is omitted, Midscene generates a safe stable ID from the Project, source path, and Case position.
