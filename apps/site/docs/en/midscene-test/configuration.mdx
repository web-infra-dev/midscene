...
Use `defineTestProject()` to define and export your project configuration. An Execution Project selects cases and supplies their runtime environment.

Midscene Test supports the following configuration files:
...
Midscene Test loads `midscene.config.ts` or `midscene.config.mjs` by default. If both files exist, it reports a conflict instead of choosing one silently; use `--config <path>` to select the intended file explicitly.
...
The complete configuration below shows how to create a browser page, connect a Midscene Agent, and register official Nodes for an execution project. After creating a project with the scaffold, use this example to understand and adjust `midscene.config.ts`.
...
## Register official Nodes

### Common AI operations and Agent integration

The Midscene Test kernel does not register Nodes implicitly. Configuration generated by `midscene-test create` calls `createMidsceneNodes()` from `@midscene/test/midscene` to register official Nodes for the selected platform. In handwritten configuration, explicitly add the returned definitions to top-level `nodes` or `projects[].nodes`.

`createMidsceneNodes()` returns the common and platform Node definitions declared by `agentClass.getTestRunnerNodeDefinitions()`, plus the Test-provided `wait` Node. Common capabilities cover AI operations, assertions and extraction, interface location, and report recording. Platform Agents add browser or device operations. The website does not maintain a duplicate exhaustive list; use the current Project's generated [Node Spec](./use#generate-the-current-projects-node-spec) for exact names and inputs.
...
`createMidsceneNodes()` does not mutate a global registry; its Nodes become available only after the returned array is included in project configuration. `agentClass` is the sole source of Agent-backed Node definitions, while `getAgent` describes how execution obtains an Agent instance from the current context. The factory throws during registration if the class does not expose `getTestRunnerNodeDefinitions()`. PlaywrightAgent, PlaywrightPageAgent, PlaywrightBrowserAgent, AndroidAgent, IOSAgent, and HarmonyAgent provide common and platform Node definitions together. The base Agent, other Web Agents, and ComputerAgent only provide common Agent Nodes.
...
Once registered, these Nodes can be called from YAML. See [Calling other Nodes](./use#calling-other-nodes) for parameter syntax, and consult the project-generated Node Spec for specific fields.
...
| `projects[].files.order` | `sorted` sorts by path; `listed` preserves include declaration order and repeated file invocations. Defaults to `sorted`. |
...
| `projects[].retryScope` | `case` retries failed Cases; `document` retries the complete YAML file. Defaults to `case`. |
| `projects[].fileConcurrency` | Number of YAML files that can run concurrently in this Project. Defaults to `1`. |
| `projects[].setupFile` | One prerequisite YAML file that runs before main files; a failure prevents main files from being scheduled. |
| `projects[].documentSetup` | Creates document-owned context and resources before each file attempt, including whole-document retries. |
...
`setupFile` must resolve to one file and cannot also be part of the main file list. It differs from the Project's TypeScript `setup`: `setup` creates runtime resources, while `setupFile` is a prerequisite YAML test document.

The default `retryScope: 'case'` reruns only the failed Case's `beforeEach → steps → afterEach`. With `document`, Test reruns the complete YAML file, including Cases that passed, `beforeAll`, and `afterAll`. Use `document` when migrating legacy whole-file retries.

When `fileConcurrency` is greater than one, concurrent files still share the context returned by Project `setup`. To create an independent Page or Agent for each file, use `documentSetup` to derive a document context from the Project context and register document cleanup with its `onTeardown()`. Keep `fileConcurrency: 1` for devices that cannot run safely in parallel, or split work into Projects connected to different devices.
...
### Generate a Node Spec for each Project

Run `midscene-test nodes` to generate a Node Spec of the available Nodes. Use `--project` to select an execution project and list the Nodes available after combining its global and local registrations:

```bash
pnpm exec midscene-test nodes --project android-smoke
```

Without a selector, `nodes` generates a shared Spec only when every Project has the same effective Node set. If the sets differ, it reports an error asking you to select a Project instead of merging potentially different definitions with the same name.

For Spec contents and general command options, see [Generate the current Project's Node Spec](./use#generate-the-current-projects-node-spec).
