...
| `nodes` | Registers the default Node list, inherited only when an execution project's `nodes` field is omitted. |
...
The top-level `nodes` field supplies the default Node list and defaults to `[]` when omitted. A Project inherits this list only when its own `nodes` field is omitted. Setting a Project's `nodes` replaces the entire global list for that Project; `nodes: []` makes no Nodes available. To reuse shared Nodes, include them explicitly with `nodes: [...sharedNodes, ...platformNodes]`. Duplicate names within the top-level list or any individual Project list cause an error.
...
Run `midscene-test nodes` to generate a Markdown reference of the available Nodes. Use `--project` to select an execution project and list its effective Nodes:
