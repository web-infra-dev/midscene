...
- `context` holds runtime resources. Nodes access the page through callbacks. When an Agent is first needed, `getAgent` creates an instance and stores it in `context.agent` for subsequent calls.
...
Each execution project runs `setup` once before executing its YAML files. All Nodes in the project share the returned `context`. In the example above, the context holds the browser and page, and Nodes access the resources they need through callbacks such as `getPage` and `getAgent`.
...
For an Android/iOS registration example, see [Configure multiple execution projects](#configure-multiple-execution-projects). Register each platform Node once: a platform Agent factory already includes its platform definitions.
...
Import each platform's preset Node factory from its corresponding entry point. These functions use callbacks to access runtime resources such as pages or Agents, so you can choose your own field names for `context`.
...
Use `createPlaywrightNodes()` to register `gotoUrl`, `setCookies`, `clearCookies`, and `setViewportSize`. Before using these Nodes, install `playwright` in your project. It is a peer dependency of `@midscene/web`:

```bash
pnpm add -D playwright
```

The following example uses `ProjectContext` from the Playwright configuration example above. Add the returned Nodes to the project’s `nodes` array:
...
Like Playwright, `gotoUrl` succeeds when navigation completes. Even if the server returns an HTTP 4xx or 5xx response, the Node succeeds and returns the status code, allowing later steps to check the error page. Network errors and navigation timeouts still fail the Node.
...
Use `createAndroidNodes()` to register `launch`, `terminate`, `runAdbShell`, `back`, `home`, and `recentApps`. The supplied Agent must provide the methods these Nodes call:
...
Use `createIOSNodes()` to register `launch`, `terminate`,
...
Use `createHarmonyNodes()` to register `launch`, `terminate`,
