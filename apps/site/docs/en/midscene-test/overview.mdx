...
Midscene Test is an AI end-to-end testing framework provided by `@midscene/test`. It separates declarative test cases from programmable extensions and is intended to replace the previous YAML automation solution.

Midscene Test is currently in **Beta**, and its test protocol and APIs continue to evolve. If you have questions or suggestions, we welcome your feedback on GitHub.
...
In this scenario, the custom Node `order.prepare` defined by engineering builders automatically compiles into the following Markdown reference using the `nodes` tool:
...
- Read [Create and use test projects](./use) to create a project, write YAML cases, and inspect results.
- Read [Develop custom Nodes](./extend) to implement business operations and share data between Nodes.
- Read [Configure test projects](./configuration) to manage runtime resources and configure multiple execution projects.
- Read [From application knowledge to test cases](../from-application-knowledge-to-test-cases) to learn how to organize supporting information and place its comprehension cost in either case generation or runtime execution.
