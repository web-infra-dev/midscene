...
All platforms provide these common capabilities: `aiAct` operates the interface from natural-language instructions, `aiAssert` checks an expected result, `aiWaitFor` waits for an observable condition, and `wait` pauses for a specified duration.
...
Use the repeatable `--file` option to run one or more files or directories. Paths are resolved from the test project root:

```bash
# Run all test cases in a specific directory
pnpm exec midscene-test --file ./cases/smoke

# Run one test case file
pnpm exec midscene-test --file ./cases/order.yaml
```

You can also select Cases by ID or tags. Repeated options are combined: IDs are matched exactly, included tags use OR matching, and excluded tags always remove a Case. Use explicit Case IDs for persisted CI or scheduler selections; generated IDs change when a Case moves within its file.

```bash
pnpm exec midscene-test --case-id checkout-001 --case-id checkout-002
pnpm exec midscene-test --tag smoke --exclude-tag destructive
```
