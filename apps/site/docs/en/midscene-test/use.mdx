...
The Midscene Test report combines the complete test result and Midscene Agent details in one interactive HTML file. It reveals information progressively instead of opening on the full execution tree:

1. **Overview** shows the final pass rate, first-pass rate, retry-passed Cases, final failures, Run time, an expandable Project → Case breakdown tree, and a compact prioritized list. Expand a Project node to inspect its Cases in place, or open **Project overview** for that Project's full workspace. The run timeline is not shown in this summary view.
...
- Retry Attempts, `not-run` reasons, collection errors, and setup or teardown failures remain visible instead of being flattened into Agent traces. Run and Project time still include setup and installation even though the Overview currently omits the phase timeline.
