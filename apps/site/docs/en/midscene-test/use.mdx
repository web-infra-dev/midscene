...
Existing `tasks/flow` cases and legacy batch configuration can run through the Test kernel without changing their files. The format remains supported but is not recommended for new cases. See [Migrate to Midscene Test](../migrate-to-midscene-test) for commands, compatibility boundaries, and field mappings.
:::
...
The files enter the compatibility path automatically; they do not require Node registration or configuration rewrites. Use the `cases/steps` format described on this page for new files. See [Migrate to Midscene Test](../migrate-to-midscene-test) for migration steps and field mappings.
