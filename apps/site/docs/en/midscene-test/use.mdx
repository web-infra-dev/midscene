...
Call `setAIContext` in `beforeAll` to configure context before the current document's cases run. It applies to subsequent calls on the same Agent. Its parameters match `agent.setAIContext(target, context)`: `target` is `default` or an AI API name such as `aiAct`, `aiTap`, or `aiAssert`.
