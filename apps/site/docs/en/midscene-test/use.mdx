...
├── midscene-node-spec.web.md  # Node Spec generated after installation
├── package.json
├── tsconfig.json
├── .env.example
└── README.md
...
Use `setAIContext` to configure guidance for subsequent calls on the same Agent. Its parameters match `agent.setAIContext(target, context)`: `target` is `default` or an AI API name such as `aiAct`, `aiTap`, or `aiAssert`; `context` is an optional string.
...
steps:
  - setAIContext:
      target: aiAct
      context: |
        Use the test account throughout the flow.
        Dismiss coupon dialogs before continuing.
  - aiAct: Open the profile page
  - setAIContext:
      target: aiAct
      context: ""
  - aiAct: Return to the home page
  - setAIContext:
      target: aiAct
```

Omitting `context` passes `undefined`, removing the selected value and allowing an API to fall back to `default`. An empty string disables inherited context. A call's `options.context` overrides the Agent-level value; values are not concatenated. Changes remain on the Agent until replaced or removed, so reset them when reusing an Agent across cases.
...
```bash
pnpm exec midscene ./legacy-flow.yaml
pnpm exec midscene --config ./legacy-batch.yaml
```

`midscene-test` does not auto-detect or execute these files, and it does not accept legacy batch configuration or legacy CLI options. Convert a complete file to `cases/steps` before running it with `midscene-test`. Both kinds of files may coexist in separate directories while a repository is being migrated, but one command invocation must select only one format. See [Migrate to Midscene Test](../migrate-to-midscene-test) for migration steps and field mappings.
