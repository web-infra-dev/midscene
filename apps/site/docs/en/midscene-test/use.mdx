...
- **Execution Project**: an entry within a Test Project that describes one execution target, including its platform, Project setup, local Nodes, file and tag filters, variables, and retry strategy. It is not a peer of the Test Project.
...
Each Execution Project inherits the top-level `nodes` and can add or override definitions in `projects[].nodes`. A local Node replaces a global Node with the same name only in that Project; duplicate names within one layer are errors. Top-level `nodes` may be omitted and default to `[]`. A shared YAML file is validated and executed against each selected Project's effective Nodes, so the same `launch` step can use Android and iOS implementations in one configuration.

Before writing platform-specific Steps, inspect the target Project:

```bash
pnpm exec midscene-test nodes --project android-smoke
```

`nodes` writes a Markdown reference for the selected Project. Without `--project`, it generates a shared reference only when every Project has the same effective Node set; otherwise it reports an error asking you to select one Project. See [project configuration](./extend#configure-and-manage-a-test-project) for a mixed-platform example.
...
After each test run, the console displays Case statuses and a brief summary. Midscene Test also generates a visual test report locally.

### Midscene visual report

Midscene Test records detailed test Steps, UI screenshots, and the AI decision process in an interactive HTML report.
