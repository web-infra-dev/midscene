...
* [Gemini-3-Flash](./model-common-config#gemini-3-pro)
...
Based on extensive real-world usage, we recommend these defaults for Midscene: Doubao Seed, Qwen3.x, Zhipu GLM-V, Zhipu AutoGLM, and Gemini-3-Flash.
...
| Gemini-3-Flash<br />[Quick setup](./model-common-config#gemini-3-pro) | [Google Cloud](https://ai.google.dev/gemini-api/docs/models/gemini) | ⭐⭐⭐<br />Price is higher than Doubao and Qwen |
...
The `deepThink` option controls how strongly Midscene guides the model to think during planning.

- By default, `aiAct` asks the model to plan the next step and locate the target element in the same planning request.
- When `deepThink` is enabled, the planning stage focuses on planning only and does not locate the target element during planning. Midscene guides the model to break down sub-goals and reason through the task structure to improve planning quality.
