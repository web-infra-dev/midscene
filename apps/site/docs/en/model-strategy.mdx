...
- When `deepThink` is enabled, the planning stage focuses on planning only and does not return element location results. Midscene guides the model to break down sub-goals and reason through the task structure to improve planning quality.
