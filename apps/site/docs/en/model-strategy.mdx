...
If your model provider offers reasoning-related parameters, you can use `deepThink` to explicitly control whether to enable it. If you don't configure this parameter, the strategy will be determined by the model provider (not necessarily disabled).

Currently, Midscene has adapted this parameter for the `qwen3-vl` and `doubao-seed-1.6` model families. Once the reasoning feature is enabled, you can see the `Reasoning` section on the right side of the report. Planning will take longer but produce more accurate results.
