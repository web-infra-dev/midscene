...
### "No visual language model (VL model) detected" or "MIDSCENE_PLANNING_STYLE is required"

Make sure you have correctly configured the `MIDSCENE_PLANNING_STYLE` environment variable for VL models.

Starting with version 1.0, Midscene recommends using `MIDSCENE_PLANNING_STYLE` to specify the vision model type. The legacy `MIDSCENE_USE_...` configurations are still supported but deprecated.

For detailed configuration instructions, refer to [Model configuration](./model-config).
