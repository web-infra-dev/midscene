...
⁠Midscene offers a way to do this kind of automation with `.yaml` files, which helps you to focus on the script itself instead of the test infrastructure. 
...
Write a yaml file to `bing.yaml`
...
```bash
midscene run ./bing.yaml
# or if you installed midscene inside the project
npx midscene run ./bing.yaml
```
...
```bash
midscene run /path/to/yaml
```

### Run all `.yaml` files under a folder

```bash
midscene run /dir/of/yaml/

# glob is OK to use
midscene run /dir/**/yaml/
```
...
target:
  ...
  viewportWidth: <width> # number, the viewport width, optional
  viewportHeight: <height> # number, the viewport height, optional
  deviceScaleFactor: <scale> # number, the device scale factor (dpr), optional
  headed: <boolean> # boolean, run in headed mode, default is false
  cookie: <path-to-cookie-file> # string, the path to the json format cookie file, optional
  waitForNetworkIdle: # object, wait for network idle, optional
