...
- New `aiAct` planning strategy `planningStrategy`
  ...
  - `standard` mode (v1.0 default) separates planning and targeting and shares the previous step’s screenshot to improve long-task success rates; token usage is slightly higher than `fast`
  - If you want to keep the lower token usage from 0.x, switch to `fast` manually
