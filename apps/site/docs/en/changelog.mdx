...
## v1.1 - Deep thinking and extensible MCP SDK

v1.1 delivers breakthroughs in model planning capabilities and MCP extensibility, making automation more stable in complex scenarios while providing more flexible solutions for enterprise deployments.

### Model deep thinking capability (deepThink)

#### Deep thinking enabled for Qwen3-vl and Doubao-vision
- **Higher accuracy**: The model performs deeper reasoning during the planning phase, especially suitable for complex forms, multi-step workflows, pages with element occlusion or dense similar controls
- **Flexible control**: Can be enabled in single calls or global configuration, balancing speed and accuracy as needed
- **Use cases**: Critical business processes, test cases with high regression failure rates, complex UI interactions

Example usage:

```typescript
// Method 1: Enable in a single call
await agent.aiAct('Submit complex order form', { deepThink: true });

// Method 2: Enable globally in Agent configuration
const agent = await createAgent({
  modelOptions: { deepThink: true }
});
```

### MCP extension and self-hosting capabilities

#### Launcher support and SDK exposure
- **Multi-platform MCP services**: Added launcher support for Android, iOS, and Web MCP, enabling you to start and manage MCP services on your own servers
- **Enterprise deployment**: Supports private device pool management, intranet deployment, and integrating Midscene capabilities as microservices into existing platforms
- **Flexible extension**: Exposes SDK interfaces for easy integration with custom toolchains and business logic
...
- **Dynamic configuration**: Support reading replanning cycle limit from environment variables for easy adjustment across different environments

### Cross-platform compatibility improvements
- Optimized `getWindowSize` method to automatically fall back to legacy endpoint when newer API is unavailable, improving cross-browser compatibility
