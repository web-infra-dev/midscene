...
## v1.9.x - Studio Recorder, Markdown Replay & Harmony YAML Target

The v1.9 line covers v1.9.0 through v1.9.2. The current latest release is available from [GitHub Releases](https://github.com/web-infra-dev/midscene/releases/latest), which automatically redirects to the latest tag.

### Midscene Studio Beta Recorder

Midscene Studio remains in **Beta**. Download it from the [latest release page](https://github.com/web-infra-dev/midscene/releases/latest) by choosing a `midscene-studio-beta-*` asset. Current v1.9.2 direct downloads: [macOS Apple Silicon DMG](https://github.com/web-infra-dev/midscene/releases/download/v1.9.2/midscene-studio-beta-v1.9.2-darwin-arm64.dmg), [Windows x64 ZIP](https://github.com/web-infra-dev/midscene/releases/download/v1.9.2/midscene-studio-beta-v1.9.2-win32-x64.zip), [Linux x64 ZIP](https://github.com/web-infra-dev/midscene/releases/download/v1.9.2/midscene-studio-beta-v1.9.2-linux-x64.zip).

- **Record and replay panel**: Studio can record operations performed in the live device preview, keep a visual event timeline with screenshots, and replay the captured flow inside Studio.
- **Script generation and exports**: Recorded sessions can generate structured Midscene scripts, Markdown replay output with screenshots, and Web Playwright output for Web recordings.
- **Multi-platform context**: The recorder works alongside the same Studio shell that hosts Android, iOS, HarmonyOS, Computer, and Web previews, so manual device control and API Playground runs stay in one desktop app.

![Studio API Playground execution flow](/blog/studio-beta-api-playground.png)

![Studio Record and replay timeline](/blog/studio-beta-record-replay.png)
...
- Report Markdown exports keep image paths aligned with exported screenshots.
- Computer automation fixed blank first-frame screenshots in RDP sessions, self-heals missing execute permission on the phased-scroll helper, and warns about elevated Windows app input drops.
...
- Web integration no longer caps `longPress` duration at 600ms.
- HarmonyOS MCP startup no longer fails in environments where `photon` / `sharp` WASM initialization had to be externalized.
...
- **Android `scrcpy` preview flag**: Android CLI now exposes a flag to enable `scrcpy`-based preview
- **Web typing delay**: `WebPageOpt` now supports configuring keyboard typing delay for Web integrations
...
- **Qwen3.7 support**: Added support for the Qwen3.7 model
- **Gemini thinking mode adaptation**: Gemini thinking mode handling was adapted to match provider behavior more closely
...
- **Less redundant report capture**: Removed a redundant before-action screenshot in reports
...
- Fixed pending Computer input being disarmed when destroying the agent
...
- Fixed HarmonyOS input clearing, keyboard dismiss behavior, and unwanted system action delay overrides
- Fixed Android screenshots passing `null` as `displayId`
- Fixed a service import cycle that could break model verification
