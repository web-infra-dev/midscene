...
- YAML scripts now support the HarmonyOS target in the CLI, so HarmonyOS automation can be driven through the same script runner workflow as Web, Android, iOS, and Computer. See: [Automate with Scripts in YAML](./automate-with-scripts-in-yaml), [HarmonyOS API](./api#harmonyagent)
...
- **Android `runAdbShell` timeout**: A `timeout` option is now accepted in both the JavaScript API and YAML scripts. See: [Android API](./api#androidagent), [Automate with Scripts in YAML](./automate-with-scripts-in-yaml)
...
- Added `terminate` action on Android and HarmonyOS to force-stop apps, useful for resetting app state during testing. See: [Android API](./api#androidagent), [HarmonyOS API](./api#harmonyagent)
...
- **iOS App Termination by bundleId**: You can now terminate iOS apps by bundleId, making it easier to reset app state during testing. See: [iOS API Reference](./api#iosagent)
...
See docs: [PC Desktop Automation](./computer)
...
See documentation: [PC Desktop Automation](./computer)
...
See documentation: [Scrcpy Screenshot Mode](./api#androiddevice)
...
The biggest highlight of v0.29 is the official introduction of iOS platform support! Now you can connect and automate iOS devices through WebDriver, extending Midscene's powerful AI automation capabilities to the Apple ecosystem, details: [Support iOS automation](./ios).
...
- New keyboard hiding strategy([keyboardDismissStrategy](./api#androiddevice)), allowing you to specify the way to automatically hide the keyboard
...
* [Custom Report Node API documentation](./api#log-screenshot)

* [API documentation for more Android configuration items](./api#androiddevice)
...
Read more: [Android automation](./android)
