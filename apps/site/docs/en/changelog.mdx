...
### deepThink Mode Sub-goal Planning Restored

Restored sub-goal planning support in deepThink mode. In complex task scenarios, the model can break down large tasks into multiple sub-goals and execute them step by step, improving completion rates for complex workflows. The MCP act tool also supports enabling deep thinking.
...
- **Timing display format optimization**: Adjusted timing display format for better readability
- **Report screenshot logic fix**: Corrected screenshot presentation logic in reports
- **Directory mode support for mergeReports**: `mergeReports` now supports directory mode report files
- **History scroll pause**: Auto-scroll pauses when users scroll up to view history in the visualizer
