...
## v1.9 - Harmony YAML Target & Model Adapter Refactor

v1.9 extends YAML automation to HarmonyOS and refactors model adapter internals.
...
- Fixed CLI dependency resolution so `@rstest/core` is resolved from the CLI package instead of `process.argv[1]`

## v1.8 - YAML Workflow, Device Integrations & Model Behavior Updates

v1.8 refines YAML execution, model reasoning defaults, report tooling, and device/platform integration options.
...
- Android CLI now exposes a flag to enable `scrcpy`-based preview
- `WebPageOpt` now supports configuring keyboard typing delay for Web integrations
...
- Added support for the Qwen3.7 model
- Gemini thinking mode handling was adapted to match provider behavior more closely

### Report, Cache, and Tooling

- Report tooling now supports a `merge` action in the report-tool CLI
- Image prompts are now supported in assert CLI / MCP tools
- Removed a redundant before-action screenshot in reports
...
- Fixed action API return values to align with docs
- Fixed port probing on `0.0.0.0` to match the actual listen host
- Fixed HarmonyOS input clearing, keyboard dismiss behavior, and system action delay overrides
- Fixed Computer default scroll distance to align with Web and disarmed pending input when destroying the agent
