...
## v0.28 - 📱 Support for Integration with Any Interface (Preview Feature)

### 🚀 Support for Integration with Any Interface (Preview Feature)

v0.28 introduces the capability to integrate with any interface. Define an interface controller class that conforms to the `AbstractInterface` definition, and you can get a fully-featured Midscene Agent.
