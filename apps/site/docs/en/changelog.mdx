...
We've prepared multiple new showcases:
* [Auto-like tweet](./showcases#ios)
* [Booking hotel](./showcases#android)
...
- Renamed `OPENAI_API_KEY` → `MODEL_API_KEY` (new variable preferred, old variable as fallback)
- Renamed `OPENAI_BASE_URL` → `MODEL_BASE_URL` (new variable preferred, old variable as fallback)
