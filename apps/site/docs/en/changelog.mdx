...
- New `aiAct` planning strategy `planningStrategy`
  - `fast` mode merges planning and localization in the vision model for quicker runs
  - `standard` mode (default) separates planning and targeting and shares the previous step’s screenshot to improve long-task success rates
- Multi-model combinations
  - Per-task model configuration lets Insight and Planning use stronger comprehension models (e.g., GPT-5.1 for planning plus Doubao for interaction) to achieve the best results
