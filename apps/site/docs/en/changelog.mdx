...
## v1.7 - Qwen 3.6, Report Tool Enhancements & Stability Fixes

v1.7 adds Qwen 3.6 model support, configurable YAML output language in Chrome extension, enhanced report tools, along with numerous stability fixes and platform improvements.

### Qwen 3.6 Model Support

Added support for the Qwen 3.6 model. See: [Model Config](./model-config)

### Chrome Extension Configurable YAML Output Language

The Chrome extension now allows you to choose the output language when recording YAML scripts.

### Report Tool Enhancements

- Report merging tool now supports processing individual report files
- Added report splitting tools to break large reports into multiple independent files
- Report Markdown output now includes screen size and locate center point for easier debugging

See: [Consume Report Files](./consume-report-file)

### Android / HarmonyOS Improvements

- Added terminate action on Android and HarmonyOS to force-stop apps, useful for resetting app state during testing. See: [Android API](./android-api-reference)
...
- Model config now supports running a connectivity test on save to catch configuration errors early
- Skill CLI now supports image-based prompting
