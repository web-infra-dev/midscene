...
- The `report-tool` CLI now supports `split` and `to-markdown` actions, allowing you to extract screenshots and JSON data from HTML reports, or convert them to Markdown for consumption by other tools. See: [Consume Report Files](./consume-report-file)
- `mergeReports()` API now accepts a single report and no longer requires at least two
