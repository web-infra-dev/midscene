...
Try v1.0 with the `@beta` tag:

```bash
# web
npm install @midscene/web@beta

# android
npm install @midscene/android@beta

# ios
npm install @midscene/ios@beta

# cli
npm install @midscene/cli@beta
```
...
### 🚀 Better results on complex or long-running tasks

For complex and long-running scenarios, Midscene adds:
- New `aiAct` planning strategy `planningStrategy`: `fast` merges planning and localization in the vision model for quicker runs; `standard` (default) separates planning and targeting and shares the previous step’s screenshot to help the model understand the flow
- Deep thinking support in `aiAct` to keep reflection and flexibility in complex workflows
- Multi-model combinations: per-task model configuration lets Insight and Planning use stronger comprehension models (e.g., GPT-5.1 for planning plus Doubao for interaction) to achieve the best results
...
* More professional: right-side visualized interaction parameters, cleaner left-side flow, and a timeline view that helps pinpoint issues quickly
* Better aesthetics: dark mode, element highlighting, and clearer presentation of model decisions

### 🚀 MCP architecture redesign

We redesigned the MCP architecture to expose Action Spaces in iOS, Android, and Web as MCP services, giving upstream Agents stable UI operation capabilities and helping developers build higher-level Agents. See [MCP architecture](./mcp).
...
#### 2⃣ Upgrade to IndexedDB for storage
...
#### 3⃣ Customize replanning cycle limit
...
#### 1⃣ New screenshot path generation
...
#### 1⃣ New chat-style user interface
...
#### 2⃣ Flexible timeout configuration
...
#### 3⃣ Unified Puppeteer and Playwright configuration
...
#### 4⃣ New data export callback mechanism
...
#### 1⃣ Input experience improvement
...
#### 2⃣ Support for passing XPath to locate elements
