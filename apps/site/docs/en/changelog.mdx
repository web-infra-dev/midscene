...
## Unreleased - Transport-Safe Task Errors

- Task failures now retain only a bounded `SerializedError`. Arbitrary upstream payloads, the complete runner graph, executor functions, and raw error identity are omitted before errors reach test runners, report dumps, or structured loggers.
- `TaskExecutionError` keeps an actionable wrapper stack, the original executor stack in `cause.stack`, whitelisted diagnostics, and a bounded task summary. Every free-form diagnostic string is limited to 4,096 characters.
- `ExecutionTask.error` is now a `SerializedError` instead of the original `Error`. Consumers should use `task.error.message`, `task.error.stack`, `task.error.code`, `task.errorMessage`, or `task.errorStack`; raw-error identity and `instanceof Error` checks are no longer supported on this field.
- `Agent.recordErrorToReport()` accepts any thrown value and normalizes it at the report-capture boundary. General user-facing error formatting is also bounded and no longer serializes arbitrary message-less payloads.
...
- YAML scripts now support the HarmonyOS target in the CLI, so HarmonyOS automation can be driven through the same script runner workflow as Web, Android, iOS, and Computer. See: [Automate with Scripts in YAML](./automate-with-scripts-in-yaml), [HarmonyOS API](./reference/#harmonyos)
...
v1.8 introduces **Midscene Studio**, a brand-new desktop application, alongside new interaction APIs (long press / clear input), refined model planning behavior, and broad upgrades across device integrations, the report system, and the MCP toolset.

### Midscene Studio — A New Desktop Application (Beta)

Midscene Studio is an Electron-based desktop app that brings every Midscene playground into a single native shell. It is currently in **Beta** — download the current Studio build from the [latest release page](https://github.com/web-infra-dev/midscene/releases/latest) by choosing a `midscene-studio-beta-*` asset, try it out, and let us know what works (and what doesn't):
...
- **Android `runAdbShell` timeout**: A `timeout` option is now accepted in both the JavaScript API and YAML scripts. See: [Android API](./reference/#android), [Automate with Scripts in YAML](./automate-with-scripts-in-yaml)

### New Interaction APIs

- **`agent.aiLongPress()`**: Long-press a target element to trigger long-press menus and similar gestures. See: [API Reference](./reference/#agentailongpress)
- **`agent.aiClearInput()`**: Clear the contents of an input field, ideal when clearing needs to be its own step. See: [API Reference](./reference/#agentaiclearinput)
...
- **Native thinking off by default**: For supported model families, Midscene now disables model-native thinking by default to improve execution speed and stability. See: [Model-Native Thinking Mode](./model-config#model-native-reasoning)
...
- Added `terminate` action on Android and HarmonyOS to force-stop apps, useful for resetting app state during testing. See: [Android API](./reference/#android), [HarmonyOS API](./reference/#harmonyos)
...
Pinch/zoom gestures are now supported on Android, iOS, and HarmonyOS, enabling use cases like map zooming and image preview. See: [API Reference - aiPinch](./reference/#agentaipinch)
...
The `deepThink` parameter in element-locating APIs has been renamed to `deepLocate` to better describe its "deep locate" purpose. The original `deepThink` parameter still works but we recommend migrating. See: [API Reference](./reference/#common)
...
- **iOS App Termination by bundleId**: You can now terminate iOS apps by bundleId, making it easier to reset app state during testing. See: [iOS API Reference](./reference/#ios)
- **CLI Version Display**: CLI now shows version info in health checks, helping you troubleshoot environment issues

### Task Cancellation Support

`aiAct` now supports `AbortSignal`, allowing you to cancel operations mid-execution instead of waiting for them to complete. See: [API Reference - aiAct](./reference/#agentaiact)
...
See docs: [PC Desktop Automation](./platforms/desktop)
...
See documentation: [PC Desktop Automation](./platforms/desktop)
...
See documentation: [Scrcpy Screenshot Mode](./reference/#scrcpy)
...
See: [aiTap file upload](./reference/#agentaitap)
...
The biggest highlight of v0.29 is the official introduction of iOS platform support! Now you can connect and automate iOS devices through WebDriver, extending Midscene's powerful AI automation capabilities to the Apple ecosystem, details: [Support iOS automation](./platforms/ios).
...
- Support freezing page context([freezePageContext](./reference/#agentfreezepagecontext)/[unfreezePageContext](./reference/#agentunfreezepagecontext)), so that all subsequent operations reuse the same page snapshot, avoiding repeated page status acquisition
...
- New keyboard hiding strategy([keyboardDismissStrategy](./reference/#androiddevice)), allowing you to specify the way to automatically hide the keyboard
...
- Support using images as AI prompt input, see [Prompting with images](./reference/#prompting-with-images)
...
- agent supports specifying HTML report name, see [reportFileName](./reference/#common)
...
Read more: [API documentation](./reference/#agent_unstablelogcontent)
...
* [Custom Report Node API documentation](/reference/#log-screenshot)

* [API documentation for more Android configuration items](/reference/#androiddevice)
...
Read more: [Use JavaScript to Optimize the AI Automation Code](./basics#javascript-orchestration)
...
Read more: [Android automation](./platforms/android)
...
Experience it now 👉: [Use Chrome Extension to Experience Midscene](./quick-start#chrome-extension)
