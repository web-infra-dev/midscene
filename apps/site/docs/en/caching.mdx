...
Automatically read existing cache and update cache files during execution. This is the recommended approach for development and testing phases.

```javascript
// Direct Agent creation - explicit cache ID (defaults to read-write strategy)
const agent = new PuppeteerAgent(page, {
  cache: { id: "my-cache-id" },
});

// Or explicitly specify strategy (same effect)
const agent = new PuppeteerAgent(page, {
...
```yaml
# YAML configuration - explicit cache ID (defaults to read-write strategy)
agent:
  cache:
    id: "my-cache-test"
...
- ✅ Automatically write updates during execution
- ✅ No manual cache management needed
- ✅ Suitable for development and testing

### Read-only mode, manual write

Configuration: `cache: { strategy: "read-only", id: "my-cache-id" }`

Only read cache, no automatic writing to cache files. Requires manual `agent.flushCache()` call to write cache files. Suitable for production environments or CI/CD scenarios to ensure cache consistency and control.
...
**Features:**
- ✅ Read existing cache
- ⚠️ No automatic write, requires manual `flushCache()` call
- ✅ Precise control over cache write timing
