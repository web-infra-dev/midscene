...
Midscene automatically cleans unused cache records when an Agent is destroyed, ensuring cache files stay lean and maintainable.

### Automatic Cleaning Mechanism

When `agent.destroy()` is called, the system will:
...
Depending on how you use Midscene, cache cleaning is triggered at different times:

**Playwright AI Fixture**: Automatic cleanup
- When using `PlaywrightAiFixture`, `destroy()` is automatically called when the page closes
- No manual management needed, cache is automatically cleaned when tests finish

**Direct PuppeteerAgent Creation**: Manual cleanup required
- When using `new PuppeteerAgent(page, {...})`, you need to manually call `agent.destroy()` to trigger cache cleanup
- Recommended to call it in test `afterEach`:
...
**YAML Script Mode**: Automatic cleanup
- When using YAML scripts, the system automatically calls `destroy()` after script execution

### Cleanup Behavior by Mode

- **read-write mode**: Auto-clean and write to file
- **read-only mode**: Clean but requires manual `flushCache()` to persist
