...
### Cache not available for Canvas, iframe, and similar scenarios

Since element location caching is based on DOM structure and XPath, caching is not available in the following scenarios:
...
In these scenarios, Midscene can still locate and interact with elements through visual recognition, but it requires calling the AI service each time and cannot leverage caching for acceleration.
