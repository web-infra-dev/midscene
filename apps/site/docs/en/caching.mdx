...
For Android, iOS, HarmonyOS, and macOS desktop automation, element location caching relies on the platform accessibility or UI inspector tree. It will not work for pure screenshot pixels, views hidden from that tree, nodes without a unique stable or semantic identity, or desktop apps that do not expose a usable macOS accessibility window tree. A positional XPath alone is not considered sufficient proof of a native cache hit.
