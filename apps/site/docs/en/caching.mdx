...
### When will it not hit the cache?

There are currently two types of caches, one is Plan (planning), and the other is Locate (element location), the former will always save and hit the cache, while the latter has cases where it will not hit.

The principle of Locate (element location) caching is to save the [xpath](https://developer.mozilla.org/en-US/docs/Web/XML/XPath) of the hit element locally and use this xpath to locate the element the next time it runs.

Therefore, when the xpath of the element changes, the cache will not be hit, which typically occurs in the following situations:

1. The HTML tag type of the ancestor node of the hit element or the position of the relative sibling elements changes
2. The text or HTML tag of the hit element itself changes
