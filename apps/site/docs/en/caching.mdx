...
Internally, native platform adapters convert their tree shape into a shared `UiNode` representation with `type`, `attrs`, `bounds`, and `children`. Cache generation selects the smallest, deepest node at the located point and records a unique stable or semantic attribute as its target identity. A positional XPath may be stored as a fallback, but replay must still match that identity. If the node has no unique stable or semantic identity, Midscene skips native element caching for that locate. Missing, zero-size, ambiguous, or identity-mismatched results fall back to AI locating.
