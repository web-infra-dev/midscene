...
2. For native platforms, the resolved node no longer has the stable or semantic identity recorded with the cache entry.
3. The DOM or native UI tree structure changed so the cached selectors no longer identify the target.
4. The XPath resolves to more than one element, because ambiguous matches are treated as stale cache.
