...
| Android | `uiautomator` XML | Supported when the target appears in the accessibility/UIAutomator tree. |
...
Internally, native platform adapters convert their tree shape into a shared `UiNode` representation with `type`, `attrs`, `bounds`, and `children`. Cache generation selects the smallest, deepest node at the located point and records a unique stable or semantic attribute as its target identity. Application roots and structural window nodes are never accepted as element targets: if an app exposes its outer window but not the control inside it, Midscene skips the XPath cache instead of replaying a click at the window center. A positional XPath may be stored as a fallback, but replay must still match that identity. If the node has no unique stable or semantic identity, Midscene skips native element caching for that locate. Missing, zero-size, ambiguous, structural-only, or identity-mismatched results fall back to AI locating.
