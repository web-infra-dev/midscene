...
Midscene supports caching two kinds of data: AI Planning steps and matched element localization information. The former can be used across automation workflows to reduce AI model calls and improve execution efficiency; the DOM localization part (XPath) in the latter is only available for web automation tasks, and has [certain limitations](#limitations-of-xpath-in-caching-element-location).
