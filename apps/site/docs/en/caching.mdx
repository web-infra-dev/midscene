...
### Can I guarantee that my scripts run consistently when using a cache?

We have noticed many scenarios where the cache may miss when the page changes, such as when the element position changes slightly or the DOM structure changes. We believe caching is just a feature that accelerates the local debugging process, rather than a tool for ensuring long-term script stability.
