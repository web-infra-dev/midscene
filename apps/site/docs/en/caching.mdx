...
Configuration: `cache: { strategy: "read-write", id: "my-cache-id" }` or `cache: { id: "my-cache-id" }`

Automatically read existing cache and update cache files during execution. If you only configure `cache: { id: "my-cache-id" }` without `strategy`, the default strategy is `read-write`.

```javascript
// Use read-write strategy for agent
const agent = new PuppeteerAgent(page, {
...
```yaml
# YAML configuration - explicit cache ID and use read-write strategy
agent:
  cache:
    id: "my-cache-id"
    strategy: "read-write"
