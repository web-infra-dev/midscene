...
Internally, platform adapters convert their native tree shape into a shared `UiNode` representation with `type`, `attrs`, `bounds`, and `children`. Cache generation ranks stable attributes first, then semantic text attributes, and finally a positional XPath fallback. Cache replay only hits when one XPath resolves to exactly one non-zero-size rect; missing, zero-size, or ambiguous matches are treated as stale cache and fall back to AI locating.
...
1. The element at the same XPath no longer matches the cached element.
2. The DOM or native UI tree structure changed from the cached one.
3. The XPath resolves to more than one element, because ambiguous matches are treated as stale cache.
...
For Android, iOS, HarmonyOS, and macOS desktop automation, element location caching relies on the platform accessibility or UI inspector tree. It will not work for pure screenshot pixels, views hidden from that tree, or desktop apps that do not expose a usable macOS accessibility window tree.
