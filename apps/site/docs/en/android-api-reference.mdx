...
- `minScreenshotBufferSize?: number` &mdash; Optional lower bound for screenshot buffers in bytes. Default `0` disables size-based validation; Midscene still rejects empty buffers and invalid image formats.
