...
- `minScreenshotBufferSize?: number` &mdash; Optional lower bound for screenshot buffers in bytes. Default `1024` (1KB). Set to `0` to disable size-based validation; Midscene still rejects empty buffers and invalid image formats.
