...
2. **Build and deploy to device**:
   ```bash
   # Replace YOUR_DEVICE_ID with your device's ID
   xcodebuild -project WebDriverAgent.xcodeproj \
             -scheme WebDriverAgentRunner \
...
- `deviceId: string` - Device ID or simulator identifier
...
- Parameters:
  ...
  - `deviceId?: string` - Optional, device ID of the iOS device to connect to. If not provided, uses the first connected device
...
const agent = await agentFromIOSDevice('00008120-001234567890123E'); // Pass Device ID
const agent = await agentFromIOSDevice(); // No Device ID, uses first connected device
```
