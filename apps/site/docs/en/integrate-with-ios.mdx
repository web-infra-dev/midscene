...
```typescript
// Execute a WebDriverAgent API request
const result = await agent.callAction('RunWdaRequest', {
  method: 'POST',
  endpoint: '/session/your-session-id/wda/pressButton',
  data: { name: 'home' }
});

// Launch an app
await agent.callAction('Launch', {
  uri: 'com.apple.mobilesafari'
});
```
