...
Use `defineAction()` to define custom gestures and pass them via `customActions` when constructing `HarmonyDevice`. Midscene appends custom actions to the planner, allowing AI to invoke your domain-specific action names.
