...
The default is `auto`, which tries scrcpy → `adb.takeScreenshot` → `screencap` → yadb in order. Setting it to `always-yadb` skips the earlier paths and uses yadb directly. Note that yadb can only capture the default display (displayId=0).
