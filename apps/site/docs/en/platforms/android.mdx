...
The yadb tool creates a virtual display via the `secure` parameter of `SurfaceControl.createDisplay`, which can capture secure content directly on Android 11 and below. On Android 12 and above, the system restricts this parameter, so you need root plus a Magisk hook to bypass `FLAG_SECURE`.
...
The default is `auto`, which tries scrcpy → `adb.takeScreenshot` → `screencap`. It uses yadb only if the `screencap` command itself fails; it does not detect black frames. For secure pages that produce a valid but black image, set `always-yadb` to skip the earlier paths and use yadb directly. Yadb can only capture the default display (`displayId=0`), so combining `always-yadb` with a non-zero `displayId` throws an error.
