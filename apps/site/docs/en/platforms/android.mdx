...
Use `defineAction()` for custom gestures and pass them through `customActions` when constructing `AndroidDevice`. Midscene will append them to the planner so AI can call your domain-specific action names.
