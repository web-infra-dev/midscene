...
### Windows: the cursor misses the target when display scaling is enabled

Windows display scaling (for example, 125%, 150%, or 175%) can make native pointer APIs and screenshots use different coordinate spaces. The symptom is consistent offset: Midscene identifies the right element, but the cursor lands somewhere else. Running as Administrator does not change this coordinate mapping.

`@midscene/computer` calibrates pointer coordinates when the Device connects, using the logical bounds of the selected display. This also supports secondary displays with non-zero or negative desktop offsets. If you change display scaling, resolution, display order, or the selected monitor while a Device is connected, destroy that Device and create a new one so calibration runs again. A failed calibration stops the connection or action instead of clicking at an unverified location.
...
> Use this check only when the cursor reaches the correct position. If the cursor itself is offset, follow the display-scaling guidance above; Administrator privileges do not fix coordinate drift. The health check logged at connection time prints this troubleshooting link when Midscene is not running as Administrator on Windows.
