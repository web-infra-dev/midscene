...
## Control a remote desktop through a VNC client on macOS {#vnc-keyboard}

`@midscene/computer` does not connect to a VNC server directly. Open a VNC client on the same Mac that runs Midscene, connect it to the remote desktop, and keep its window in the foreground. Midscene then controls the VNC client as a local desktop application.

### Preserve modifier keys

VNC clients may not forward modifiers from the default macOS AppleScript keyboard events. Symptoms include uppercase letters becoming lowercase or shortcuts losing `Shift` and `Control`.

Set `keyboardEventMode` to `physical` so Midscene sends explicit key-down and key-up events for each modifier. Text input must also operate one key at a time. Set `inputStrategy` to `sequential`, as shown below, or set a positive `keyboardTypeDelay` when using the default `legacy` input strategy.
