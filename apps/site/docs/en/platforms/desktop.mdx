...
## Control a remote desktop through a VNC client {#vnc-keyboard}

`@midscene/computer` does not connect to a VNC server directly. Open a VNC client on the computer that runs Midscene, and use the client to connect to the remote desktop. Keep the VNC client in the foreground while Midscene controls it as a local desktop application. This workflow is not limited to macOS.

### Forward modifier keys on macOS

Some VNC environments do not preserve modifiers from the standard macOS AppleScript `keystroke ... using` form. Symptoms include uppercase letters becoming lowercase or shortcuts losing `Shift` and `Control`.

The available evidence confirms the compatibility issue, but it does not include low-level macOS input events or RFB packets. It cannot show where the modifier state is lost in the end-to-end input path.
...
The default value is `keyboardEventMode: 'logical'`. It keeps the standard AppleScript behavior for native macOS applications.

:::warning
Treat `physical` as an opt-in compatibility mode for affected VNC environments. Test it with your VNC client, server, and keyboard layout. The current verification covers generated AppleScript and integration behavior, but not a real VNC session.
