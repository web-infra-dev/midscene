...
View the [full report](https://lf3-static.bytednsdoc.com/obj/eden-cn/nupipfups/Midscene/1.0-showcases/x.html), or explore more [Midscene showcases](../showcases.mdx).
...
2. Click the gear button to enter the configuration page and paste your API Key configuration. See [Supported models and setup](../model-common-config) if you still need a model configuration.
...
Promise.resolve(
  (async () => {
    ...
    const agent = new IOSAgent(page, {
      aiContexts: {
        aiAct: 'If any location, permission, user agreement, etc. popup appears, click agree. If login page appears, close it.',
      },
    });
    await page.connect();
...
For constructors, helper methods, and platform-specific device APIs, see the [iOS section of the API reference](../reference/#ios). It includes detailed parameter lists and advanced topics such as custom actions. For APIs shared across platforms, see the [Common section](../reference/#common).
...
Midscene appends `/status`, `/session`, and session commands to this base URL. Set either `wdaBaseUrl` or `wdaHost`/`wdaPort`; combining them throws an error. The MJPEG stream uses its separate `wdaMjpegPort` setting.
...
- For iOS-specific APIs, see [API reference (iOS)](../reference/#ios).
