...
Midscene appends `/status`, `/session`, and session commands to this base URL. `wdaBaseUrl` takes precedence over `wdaHost` and `wdaPort`. The MJPEG stream uses its separate `wdaMjpegPort` setting.
