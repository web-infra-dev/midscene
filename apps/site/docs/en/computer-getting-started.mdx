...
Xvfb creates a virtual display so that mouse, keyboard, and screenshot operations work without a physical monitor. See [API Reference](./computer-api-reference) for configuration details and [cross-platform CI validation](./platform-ci-validation) for repository workflows and example reports.
