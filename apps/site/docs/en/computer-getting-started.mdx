...
Xvfb creates a virtual display so that mouse, keyboard, and screenshot operations work without a physical monitor. See [API Reference](./computer-api-reference) for configuration details and [GitHub Actions integration](./github-actions-integration) for repository workflows and example reports.
