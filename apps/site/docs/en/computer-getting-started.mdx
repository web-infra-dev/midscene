...
**Windows**: No extra setup is needed for ordinary apps. However, Windows isolates input across privilege levels (UIPI): a non-elevated process **cannot** send mouse or keyboard input to a window that runs **as Administrator** (elevated). The input is silently dropped — the cursor still moves to the right spot, but clicks and keystrokes have no effect. If your target application runs as Administrator, **run your terminal or Node.js as Administrator too** so both processes share the same privilege level. See [Windows: clicks have no effect on some apps](#windows-clicks-have-no-effect-on-some-apps).
...
If the cursor moves to the correct position but clicks (or keystrokes) do nothing on a particular application — while other apps work fine — the target app is almost certainly running **as Administrator** (elevated). Windows UIPI blocks input injected from a lower-privilege process into an elevated window and drops it silently, with no error.
...
> The health check logged at connection time will print a heads-up when Midscene is not running as Administrator on Windows.
