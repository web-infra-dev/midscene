...
- `keyboardTypeDelay: number` &mdash; Per-character delay (ms) forwarded to Puppeteer's `page.keyboard.type`. When omitted, Midscene leaves the option unset and Puppeteer types instantly (`delay` defaults to `0`). Raise it (e.g. `80`) when a controlled input drops characters under fast typing; set it to `0` for the fastest input.
...
- `enableTouchEventsInActionSpace: boolean` &mdash; Add touch gestures (like swipe) to the action space so the agent can handle touch-only interactions. Default `false`.
- `keyboardTypeDelay: number` &mdash; Per-character delay (ms) forwarded to Playwright's `page.keyboard.type`. When omitted, Midscene leaves the option unset and Playwright types instantly (`delay` defaults to `0`). Raise it (e.g. `80`) when a controlled input drops characters under fast typing; set it to `0` for the fastest input.
