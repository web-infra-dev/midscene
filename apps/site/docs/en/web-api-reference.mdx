...
- `scrollMethod: 'wheel' | 'gesture'` &mdash; Choose how Midscene scrolls. Default `wheel`. Use `gesture` for Cocos, WebGL, Canvas, or custom-drawn scroll areas where synthetic wheel events do not trigger scrolling.
...
- `scrollMethod: 'wheel' | 'gesture'` &mdash; Choose how Midscene scrolls. Default `wheel`. Use `gesture` for Cocos, WebGL, Canvas, or custom-drawn scroll areas where synthetic wheel events do not trigger scrolling. `gesture` requires a Chromium-based browser.
- `enableTouchEventsInActionSpace: boolean` &mdash; Add touch gestures (like swipe) to the action space so the agent can handle touch-only interactions. Default `false`.
...
- `options.scrollMethod` (default `'wheel'`) chooses how scrolling is synthesized in the connected tab. Use `'gesture'` for Cocos, WebGL, Canvas, or custom-drawn scroll areas.
