...
Use this section when you need to customize device behavior, wire Midscene into your framework, or troubleshoot adb issues. For detailed constructor parameters, jump to the [API reference](./api#androidagent).
...
After entering text, Midscene automatically dismisses the keyboard by sending an **ESC key event**. Some input fields (especially inside WebView) listen for ESC, which can clear text, close popups, or navigate away.

Try these solutions in order of priority:

1. Set `keyboardDismissStrategy: 'back-first'` to use the Android BACK key instead of ESC.
2. Set `autoDismissKeyboard: false` to disable auto-dismiss and let the AI Agent manage the keyboard.

For full constructor signatures, see the [API reference](./api#androiddevice). With auto dismiss disabled, you can use `aiAct` to manually dismiss the keyboard, or install [ADBKeyBoard](https://github.com/senzhk/ADBKeyBoard) — a minimal virtual keyboard that barely affects screen interactions.

### English text is rewritten by the Android keyboard

If the report shows correct input parameters but the app receives different text, the active Android input method may be rewriting it (common with Chinese or autocorrect keyboards).

Set `imeStrategy: 'always-yadb'` to force all text input through yadb. Configurable via constructor, YAML (`android.imeStrategy`), or the `MIDSCENE_ANDROID_IME_STRATEGY` environment variable. See the [API reference](./api#androiddevice) for details.

This differs from "text cleared after typing" — if text disappears after being entered correctly, check `keyboardDismissStrategy` or `autoDismissKeyboard` instead.
...
You can also pass `androidAdbPath`, `remoteAdbHost`, and `remoteAdbPort` via the `AndroidDevice` constructor. See the [API reference](./api#androiddevice) for the full signature.

## More

- For every Agent method, check the [API reference (Common)](./api#interaction-methods).
- For the Android API reference, see [API reference](./api#androidagent).
- Demo projects
