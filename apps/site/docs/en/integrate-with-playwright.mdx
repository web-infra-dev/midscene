...
Review the full catalog of action, query, and utility methods in the [Agent API reference](./reference/#interaction-methods). When you need lower-level control, you can use `agentForPage` to obtain the underlying `PageAgent` instance and call any API directly:
...
2. Download only the commonly used `chromium`
...
- Use `waitForNetworkIdleTimeout` and `waitForNavigationTimeout` parameters in [Agent](/reference/#constructors).
...
- For all the methods on the Agent, please refer to [API Reference](./reference/#interaction-methods).
- For the Playwright API reference, see [Playwright Agent API](./reference/#playwright-agent).
- Demo projects
