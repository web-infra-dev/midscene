...
- `useDirectoryReport`: If `true`, uses directory-based report format with screenshots saved as separate PNG files instead of inline base64. Useful when report files are too large. Default is `false`. **Note**: When using this option, reports must be served via HTTP server and cannot be opened directly using `file://` protocol (because browser CORS restrictions block loading local images via relative paths from the file protocol). Navigate to the report directory and run one of the following commands:
