...
The `type` option of the `reporter` configuration can be `merged` or `separate`, the default value is `merged`, which means multiple test cases generate one report, the optional value is `separate`, which means one report for each test case.
