...
- `ai` or `aiAct` - General AI interaction
...
- `setAIActionContext` - Set AI action context, send background knowledge to AI model when calling `agent.aiAct()`
- `evaluateJavaScript` - Execute JavaScript in page context
- `recordToReport` - Log screenshot with description in report file
