...
In default Puppeteer mode, you can override the default `1920x1080` viewport with `--viewport-width <width>` and `--viewport-height <height>`. These flags are not used in `--bridge` or `--cdp` mode.
