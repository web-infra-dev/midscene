...
In CDP mode, use `--extra-http-headers '<json-object>'` to add HTTP headers to page requests. Header values must be strings, and the option must be passed to every separate CLI command that may issue requests because each command creates a new CDP session:
