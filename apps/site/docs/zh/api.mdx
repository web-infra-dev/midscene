...
- 参数：
  ...
  - `opt: Object` - 配置对象，包含：
    - `scale: number` - **必填。** 缩放比例。`scale > 1` 表示放大（双指张开），`scale < 1` 表示缩小（双指收拢）。例如 `2` 表示放大到 200%，`0.5` 表示缩小到 50%。
...
```typescript
// 在地图上放大到 300%
await agent.aiPinch('地图区域', { scale: 3 });

// 在屏幕中心缩小
await agent.aiPinch(undefined, { scale: 0.5 });

// 自定义持续时间放大
await agent.aiPinch('图片', { scale: 2, duration: 1000 });
```
