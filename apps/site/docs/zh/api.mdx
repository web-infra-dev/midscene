...
- 参数：
  ...
  - `options?: Object` - 可选，一个配置对象，包含：
    ...
    - `deepThink?: boolean` - 当模型支持时（取决于 MIDSCENE_MODEL_FAMILY），是否开启规划阶段的深度思考能力。默认值为 undefined，跟随模型服务商的默认策略。[详情参阅 deepThink 说明](./model-strategy#关于-aiact-方法的-deepthink-参数)。
