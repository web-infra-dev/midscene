...
各平台命令行工具（`@midscene/android`、`@midscene/web`、`@midscene/ios`、`@midscene/harmony` 等）的 `assert` 子命令支持与 JS SDK 完全一致的多模态入参，命令行参数名与 SDK 字段一一对应：

- `--images`：JSON 数组形式的 `{ "name", "url" }` 列表。每个 `url` 可以是 http(s) 链接、`data:` URI，或本地文件路径，由 `@midscene/core` 统一解析。
