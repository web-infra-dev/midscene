...
function startObserving(options?: {
  ...
  maxBufferedFrames?: number; // 帧缓冲上限,默认 30(缓冲满时自动抽稀)
}): Promise<UIObserver>;
```
...
- `observer.aiAssert(assertion: string, msg?: string, opt?: Object): Promise<void>` - 对观察窗口做断言。判断采用「任一帧」语义:只要断言在任一观察帧中成立即通过,即使该 UI 在窗口结束时已消失。
- `observer.aiBoolean(prompt: string, opt?: Object): Promise<boolean>` - 对观察窗口做布尔查询,帧语义同上。
...
- 缓冲内的所有帧(最多 `maxBufferedFrames` 帧,外加最终代表帧)都会发给模型,确保长窗口中的瞬态 UI 不会因抽样而遗漏。如需控制 token 成本,可增大 `intervalMs`(降低采样密度)或减小 `maxBufferedFrames`(缩小缓冲区)。观察到的帧会显示在报告时间线中。
