...
各平台命令行工具（`@midscene/android`、`@midscene/web`、`@midscene/ios`、`@midscene/harmony` 等）的 `assert` 子命令新增了三个参数，用于支持图片提示词：

- `--images`：JSON 数组形式的 `{ "name", "url" }` 列表。`url` 支持 http(s) 链接或 `data:` URI。
- `--imageFiles`：本地图片路径，可用英文逗号分隔，也可以传 JSON 数组。命令行会自动读取文件并 base64 编码成 `data:` URI 后传给模型。
...
npx @midscene/android assert \
  ...
  --images '[{"name":"badge","url":"https://example.com/badge.png"}]'
```
