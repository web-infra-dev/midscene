...
在一段时间内持续采样屏幕，后续断言会基于这段记录判断目标状态是否出现过。它适合捕捉单张截图容易错过的短暂 UI，比如 toast、横幅或页面切换。
...
- `observer.aiAssert(assertion: string, msg?: string, opt?: Object): Promise<void>` - 基于已观察到的时间段执行断言。模型会收到所有缓冲帧和最后一张截图，并根据这段记录判断断言是否成立。当 `opt.keepRawResponse` 为 true 时，返回 `{ pass, thought, message }` 而不是在失败时抛异常。
