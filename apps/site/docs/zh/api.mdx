...
- 参数：
  ...
  - `options?: Object` - 可选，一个配置对象，包含：
    ...
    - `deepThink?: 'unset' | true | false` - 控制 Midscene 在规划时引导模型思考的力度。开启后，`aiAct` 会让规划阶段专注于任务拆解和规划，不再从规划请求里返回元素定位结果。为了兼容旧写法，`'unset'` 仍然可以传入，并会被按 `false` 处理。[详情参阅 deepThink 说明](./model-strategy#关于-aiact-方法的-deepthink-参数)。
