...
各平台 CLI 通过一次 `observe` 命令提供相同能力。这个命令会在同一个进程内启动观察、执行操作和停止观察，然后基于整个观察窗口完成断言：
...
`@midscene/android`、`@midscene/ios`、`@midscene/harmony` 和 `@midscene/computer` 也提供该命令。`observe` 可以使用各平台原有的目标参数，并支持 `--interval-ms`、`--max-frames` 和 `--watchdog-ms`。

如果要验证操作过程中可能短暂出现并消失的 UI，请使用 `observe`。如果只关心当前或最终画面，请使用普通的 `assert` 命令。
