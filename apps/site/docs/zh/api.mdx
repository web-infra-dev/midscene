...
- `useDirectoryReport: boolean`: 如果为 true，则使用目录格式的报告，将截图保存为独立的 PNG 文件而非内嵌 base64。适用于报告文件过大的场景。默认值为 false。**注意**：使用此选项时，报告需要通过 HTTP 服务器或 CDN 地址访问，无法直接在本地通过 `file://` 协议打开（浏览器安全策略限制）。
