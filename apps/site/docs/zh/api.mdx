...
- `useDirectoryReport: boolean`: 如果为 true，则使用目录格式的报告，将截图保存为独立的 PNG 文件而非内嵌 base64。适用于报告文件过大的场景。默认值为 false。**注意**：使用此选项时，报告需要通过 HTTP 服务器或 CDN 地址访问，无法直接在本地通过 `file://` 协议打开（由于浏览器对 `file://` 协议的 CORS 跨源资源共享限制，无法通过相对路径加载本地图片）。如需在本地预览报告，可在报告目录下启动本地 HTTP 服务器（例如运行 `npx http-server .`）后通过 `http://localhost:...` 访问。
