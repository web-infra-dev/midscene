...
- 参数：
  ...
  - `options?: Object` - 可选，一个配置对象，包含：
    ...
    - `signal?: AbortSignal` - 可选的 [AbortSignal](https://developer.mozilla.org/zh-CN/docs/Web/API/AbortSignal)，用于取消执行。当信号被中止时，正在进行的规划循环和任务执行将在下一个检查点停止，并抛出 `MidsceneAbortedError`。适用于实现超时控制或用户主动取消操作。

- 返回值：

  - 返回一个 Promise。当所有步骤执行完成时解析为 void；若执行失败，则抛出错误。如果通过 `signal` 中止了操作，则抛出 `MidsceneAbortedError`。
...
import { MidsceneAbortedError } from '@midscene/core';
...
try {
  await agent.aiAct('完成一个复杂的多步骤工作流', {
