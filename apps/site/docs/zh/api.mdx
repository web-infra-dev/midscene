...
在运行多个测试用例时，每个测试都会生成独立的报告文件。`ReportMergingTool` 提供了将多个测试报告合并为单个报告的能力，便于统一查看和管理测试结果。

### 使用场景

- 在测试套件中运行多个测试用例，希望生成一个统一的测试报告
- 跨平台测试(如 Web 和 Android)需要合并不同平台的测试结果
- CI/CD 流程中需要生成汇总的测试报告
...
将报告添加到待合并列表中。通常在每个测试用例结束后调用此方法。
...
- 参数:
  ...
  - `reportInfo: ReportFileWithAttributes` - 报告信息对象，包含:
    ...
    - `reportAttributes: object` - 报告属性
      - `testId: string` - 测试用例的唯一标识符
      - `testTitle: string` - 测试用例标题
      - `testDescription: string` - 测试用例描述
      - `testDuration: number` - 测试用例执行时长(毫秒)
      - `testStatus: 'passed' | 'failed' | 'timedOut' | 'skipped' | 'interrupted'` - 测试状态
...
// 指定自定义文件名
afterAll(() => {
  reportMergingTool.mergeReports('my-test-report');
});
...
以下是在 Vitest 测试框架中使用 `ReportMergingTool` 的完整示例:
...
describe('Android 设置测试', () => {
  ...
  afterAll(() => {
    // 合并所有测试报告
    reportMergingTool.mergeReports('android-settings-test-report');
  });
...
合并后的报告文件会保存在 `midscene_run/report` 目录下。你可以使用浏览器打开合并后的 HTML 文件查看所有测试用例的执行情况。
