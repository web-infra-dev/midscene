...
- 参数：
  ...
  **推荐用法**：
  ...
  - `opt: Object` - 配置对象，包含：
    ...
    - `xpath?: string` - 目标元素的 xpath 路径，用于执行当前操作。如果提供了这个 xpath，Midscene 会优先使用该 xpath 来找到元素，然后依次使用缓存和 AI 模型。默认值为空
    - `cacheable?: boolean` - 当启用 [缓存功能](./caching.mdx) 时，是否允许缓存当前 API 调用结果。默认值为 true
    - `autoDismissKeyboard?: boolean | 'back' | 'esc' | 'midscene-ime' | 'midscene-ime-auto-install'` - 控制移动端输入后的键盘关闭行为。`true` / `false` 适用于各个移动平台；Android 额外支持 `'back'`、`'esc'`、`'midscene-ime'` 和 `'midscene-ime-auto-install'`。
      - `true` - 使用平台默认的自动收起行为；在 Android 上等价于 `'esc'`。
      - `false` - 不自动关闭键盘。
      - `'back'` / `'esc'` - Android 专属的显式按键策略。
      - `'midscene-ime'` - Android 专属的辅助输入法策略，不会向当前输入框发送 ESC/BACK 按键事件。
      - `'midscene-ime-auto-install'` - Android 专属的辅助输入法策略；当需要安装辅助输入法时，`AndroidAgent` 会尝试自动点击安装确认。如果无法安全安装完成，会直接抛错，而不是退回到按键事件。
