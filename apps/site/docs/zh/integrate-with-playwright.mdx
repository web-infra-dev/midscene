...
- `useDirectoryReport`: 如果为 `true`，则使用目录格式的报告，将截图保存为独立的 PNG 文件而非内嵌 base64。适用于报告文件过大的场景。默认值为 `false`。**注意**：使用此选项时，报告必须通过 HTTP 服务器访问，无法直接使用 `file://` 协议打开（因为浏览器的 CORS 限制会阻止从 file 协议加载相对路径的本地图片）。进入报告目录后运行以下命令之一：
