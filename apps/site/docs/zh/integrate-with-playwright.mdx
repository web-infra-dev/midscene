...
完整的交互、查询和辅助 API 请参考 [Agent API 参考](./reference/#interaction-methods)。如果需要调用更底层的能力，可以使用 `agentForPage` 获取 `PageAgent` 实例，再直接调用对应的方法：
...
使用 `defineAction()` 定义自定义动作。构造 Agent 时，通过 `customActions` 传入这些动作。Midscene 会把这些动作追加到规划器中，让 Agent 可以调用你定义的领域特定动作。
...
- 使用 [Agent](/zh/reference/#%E6%9E%84%E9%80%A0%E5%99%A8) 上的 `waitForNetworkIdleTimeout` 和 `waitForNavigationTimeout` 参数
...
- 更多 Agent 的 API 文档请参考 [API 参考](./reference/#interaction-methods)。
- Playwright 的 API 文档请参考 [Playwright Agent API](./reference/#playwright-agent)。
