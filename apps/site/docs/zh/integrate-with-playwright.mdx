...
与 Playwright 的集成方式有两种，一种是通过 Playwright 的测试用例管理集成 Midscene，另一种是直接用脚本方式集成和调用 Midscene Agent。前者适合需要管理测试用例的场景，后者适合快速体验、原型开发、数据抓取和自动化脚本等场景。

前者适合需要管理测试用例的场景，后者适合快速体验、原型开发、数据抓取和自动化脚本、与三方测试框架集成等场景。
