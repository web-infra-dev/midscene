...
例如，一个切换浏览器 UA 语言的自定义节点 `browser.setLanguage`，经 `describe-nodes` 工具自动导出的 Markdown 说明书结构如下：

```markdown
## browser.setLanguage

* **标题**：设置 UA 语言
* **说明**：切换浏览器 UA 语言，并重新打开当前页面。
* **参数格式 (JSON Schema)**：

{
  ...
  "properties": {
    "language": {
      "type": "string",
      "enum": ["en", "zh"],
      "description": "用于创建 Playwright Context 的 UA 语言。"
    }
  },
  "required": ["language"]
}
