...
- `--setup <file>`：在主 `--files` 之前执行的前置脚本。它与主脚本共享同一个浏览器上下文，因此在这里建立的前置状态（如登录态）对每个主脚本都可见。前置脚本失败会中止整个批次，主脚本将被标记为未执行。需要配合 `--share-browser-context` 使用。如需串行执行多个前置脚本，请改用配置文件中的 `setupFiles` 数组。
...
当多个互不依赖的脚本都依赖同一个前置条件（例如登录）时,可以把前置任务写在 `setupFiles` 下。前置脚本会在主 `files` 之前按顺序串行执行;全部成功后,主脚本再按配置的并发数运行。前置脚本与主脚本共享同一个浏览器上下文,因此登录态会被带过去。`setupFiles` 需要配合 `shareBrowserContext: true` 使用。
...
如果任一前置脚本失败,整个批次会被中止,主脚本将被标记为未执行。
