...
当需要让其他程序或 Agent 直接消费执行结果时，传入 `--json`：

```bash
midscene ./bing-search.yaml --json
```

此模式下，命令只会输出一个可被机器消费的 JSON 对象，不会输出执行进度日志。`summary` 包含执行统计数据和生成的汇总文件路径；`results` 的每项包含 YAML 文件路径、状态、Report 路径、输出文件路径及解析后的 YAML 输出：
...
- `--json`：输出一个 JSON 对象，其中包含执行统计和汇总文件路径，以及每个脚本的状态、Report 路径、输出文件路径和解析后的 YAML 输出。不会输出执行进度日志。
