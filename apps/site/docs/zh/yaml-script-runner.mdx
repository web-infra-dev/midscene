...
:::info 与 Midscene Test 共用执行内核
已有项目可继续使用 `tasks/flow`、原来的批配置、目录结构和 `midscene` 命令；升级后的入口由 Test 内核执行，并生成新的 Test 报告页面。也可用 `midscene-test ./flow.yaml` 或 `midscene-test --config ./batch.yaml` 运行原文件，无需注册 Node 或改写 YAML。

原来的 task continuation、整文件重试、环境变量、结果输出和 summary 语义继续保留。原生 `cases/steps` 的默认失败和重试策略有所不同；手动转换时请参阅 [YAML 字段映射](./midscene-test/extend#yaml-字段与原生能力的对应关系)。旧 HTML 报告文件无需转换，重新运行即可生成新页面。
:::
