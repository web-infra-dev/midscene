...
- `--reuse-page`：让多个 Puppeteer Web 脚本复用同一个 Page，可保留 `sessionStorage`、当前标签页和 `window.name` 等页面级状态。必须同时设置 `--share-browser-context` 和 `--concurrent 1`。默认关闭。
...
如果串行流程必须接着使用 setup 打开的标签页，可以显式开启 Page 复用：
...
主脚本仍会应用自己的 Web target 配置，并打开其中设置的 `url`，但导航会发生在 setup 创建的同一个 Page 中。
