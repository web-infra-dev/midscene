...
- `--setup <file>`：在主 `--files` 之前执行的前置脚本。它与主脚本共享同一个浏览器上下文，因此 Cookies、同源 `localStorage` 等上下文级前置状态对每个主脚本都可见。包括 setup 和串行脚本在内，每个 YAML 都在独立标签页中运行；`sessionStorage` 等页面级状态不会在脚本之间传递。前置脚本失败会中止整个批次，主脚本将被标记为未执行。需要配合 `--share-browser-context` 使用。
...
- `--share-browser-context`：多个脚本共享一个浏览器上下文（Cookies、同源 `localStorage` 等），同时每个 YAML 使用独立标签页。即使 `--concurrent 1`，`sessionStorage`、DOM、URL、`window.name` 等页面级状态也不会共享。默认关闭。
...
当多个脚本都依赖同一个前置条件（例如登录）时，可以把前置任务写在 `setup` 下。前置脚本会在主 `files` 之前执行；成功后，主脚本再按配置的并发数运行。前置脚本与主脚本共享同一个浏览器上下文，因此 Cookies 和同源 `localStorage` 会被带过去。包括 `concurrent: 1` 在内，每个 YAML 都使用独立标签页；setup 标签页的 `sessionStorage`、DOM、URL、`window.name` 和其他页面级状态不会复制到主标签页。`setup` 需要配合 `shareBrowserContext: true` 使用。

:::warning 页面级状态不会共享
`shareBrowserContext` 遵循浏览器原生的存储边界，不会在标签页之间复制或同步 `sessionStorage`。如果 setup 只把登录态写入 `sessionStorage`，主脚本将无法继承该登录态。需要让多个脚本看到前置状态时，请优先使用 Cookies、同源 `localStorage` 或服务端状态。脚本并发运行时，共享状态的写入可能发生竞争，需要显式协调。
:::
