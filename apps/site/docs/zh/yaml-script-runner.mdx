...
也可以安装 `@midscene/test`，直接用 `midscene-test ./flow.yaml` 或 `midscene-test --config ./batch.yaml` 运行原文件，无需注册 Node 或改写 YAML。后续新增和重构建议采用原生 `cases/steps`、TypeScript 项目配置和 Node。具体步骤见[从旧版 YAML 迁移](./midscene-test/migrate-from-yaml)。
:::
