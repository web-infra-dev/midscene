...
- `--setup-files <file1> <file2> ...`：前置脚本，会在主 `--files` 之前按顺序串行执行。它们与主脚本共享同一个浏览器上下文，因此在这里建立的前置状态（如登录态）对每个主脚本都可见。任一前置脚本失败会中止整个批次，主脚本将被标记为未执行。需要配合 `--share-browser-context` 使用。
