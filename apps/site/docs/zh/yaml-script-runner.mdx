...
- `--setup <file>`：在主 `--files` 之前执行的前置脚本。它与主脚本共享同一个浏览器上下文，因此在这里建立的前置状态（如登录态）对每个主脚本都可见，但每个主脚本仍在独立标签页中运行。前置标签页同源的初始 `sessionStorage` 会复制到每个主标签页，之后的修改按标签页隔离。前置脚本失败会中止整个批次，主脚本将被标记为未执行。需要配合 `--share-browser-context` 使用。
...
- `--share-browser-context`：多个脚本共享一个浏览器上下文（Cookies、`localStorage` 等），但每个脚本使用独立标签页。`sessionStorage` 属于标签页；使用 `--setup` 时，其同源初始值会复制到每个主标签页。默认关闭。
...
当多个互不依赖的脚本都依赖同一个前置条件（例如登录）时，可以把前置任务写在 `setup` 下。前置脚本会在主 `files` 之前执行；成功后，主脚本再按配置的并发数运行。前置脚本与主脚本共享同一个浏览器上下文，因此 Cookies 和同源存储会被带过去，但每个 YAML 脚本使用独立标签页。前置标签页同源的 `sessionStorage` 会在主标签页首次导航前复制过去，后续修改互不影响。前置页面必须停留在非 opaque origin，才能确定并传递 `sessionStorage` 所属的源。`setup` 需要配合 `shareBrowserContext: true` 使用。
