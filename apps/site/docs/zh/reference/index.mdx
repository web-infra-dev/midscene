...
##### Planning 组件 {#planning-components}

标准 Planning 默认开启子目标、memory、观察建议、截图历史及独立定位。旧的 `deepThink` 和实验性 `effort` 总模式参数已移除，改用环境变量独立控制组件。
...
关闭能力时会一起移除它的提示、示例、有效输出、状态及回放。`subGoals` 包含子目标示例，无需另设示例开关。`screenshotHistory` 和 `separateLocate` 分别控制上一张截图与独立定位，其他组应保持它们固定。模型原生思考仍由[模型配置](../model-config#model-native-reasoning)控制。
...
`deepLocate` 控制增强元素定位。`aiAct` 的 `deepThink` 总模式已移除，请使用 [Planning 组件](#planning-components)。单步动作仍兼容旧的 `deepThink` 定位别名，其含义等同于 `deepLocate`。
