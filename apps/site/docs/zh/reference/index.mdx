...
- AI 页面选择 action —— Browser Agent 的 `actionSpace` 包含 `ListBrowserPages` 和 `SetActivePage`。`ListBrowserPages` 每次显示 8 个标签页，默认从 offset 0 开始；传入 `{ offset: 8 }` 可查看接下来的 8 个。offset 超出范围会报错，Agent 可从 0 重新列出。AI 可以按 index、title 或 URL 切换 active page。index 可以与 title 或 URL 一起使用；此时所有 selector 必须匹配同一个页面。匹配失败后，Agent 需要重新获取页面列表。Page Agent 不包含这些 action。
