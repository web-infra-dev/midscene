...
产物是带版本号的 JSON 有序截图序列，而不是编码后的视频，因为 Midscene 断言实际消费的就是这类时间序列输入。每一帧 JPEG 或 PNG 都会以内嵌 base64 图片 data URL 的形式保存。如果只关心当前画面，直接使用不带 `--record` 的普通 `assert` 命令。在 Web Bridge 模式下，前台录制会占用唯一的 Bridge 连接，因此应手动操作浏览器，不要再启动第二个 Bridge CLI 操作。
