...
- `keyboardEventMode?: 'logical' | 'physical'`：控制 macOS AppleScript 键盘后端发送修饰键的方式。`'logical'` 使用一条 `keystroke ... using` 命令，也是默认值。`'physical'` 是可选的兼容模式，适用于需要分别接收修饰键状态变化的前台应用。例如，部分远程控制客户端和虚拟机控制台需要这个模式。两种模式都使用 AppleScript，`'physical'` 不会发送硬件事件。这个模式要求本地与远程系统使用相同的 en-US 键盘布局。文本输入还需要使用 `'sequential'`，或设置正数 `keyboardTypeDelay`。这个选项仅对 macOS AppleScript 键盘后端生效。详见[通过桌面客户端转发键盘输入](../platforms/desktop#keyboard-event-mode)。
- `keyboardShortcutDelay?: number`：本机 libnut 键盘驱动的修饰键时序间隔，单位为毫秒。取值必须是有限的非负数。设置正数后，Midscene 会依次执行“按下修饰键、等待、发送主键、等待、释放修饰键”。这个选项会作用于组合键。逐键输入文本时，它也会作用于大写字母和 en-US 键盘 Shift 标点中隐含的 `Shift`。默认值为 `0`。直接使用 RDP 或 macOS AppleScript 驱动时，这个选项不会生效。详见[通过桌面客户端转发键盘输入](../platforms/desktop#keyboard-event-mode)。
