...
在一段时间内持续采样屏幕。停止 observer 后会得到固定的 `UIObservation`，其只读 insight 方法基于整段有序画面判断状态或变化，适合捕捉实时单屏 insight 容易错过的 toast、横幅或页面切换。
...
`UIObservation` 与 `Agent` 实现相同的只读 `InsightAPI`：`aiQuery()`、`aiBoolean()`、`aiNumber()`、`aiString()`、`aiAsk()` 和 `aiAssert()`。它还提供 `frameCount`、`startedAt` 和 `endedAt`。最后一次 insight 调用完成后，可以调用 `observation.dispose()` 提前释放图片文件；否则 `agent.destroy()` 会负责清理。
