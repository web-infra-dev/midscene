...
- `contexts: { default?: string; [apiName]?: string }`：Agent 级 AI 补充信息，可包含业务事实、判断或操作规则、约束以及输出要求。`default` 是所有 AI API 共享的缺省回退值，仅当本次调用和对应 API 都没有自己的 context 时使用。解析规则为 `options.context ?? contexts[apiName] ?? contexts.default`，只使用优先级最高的已定义值，不会自动拼接。空字符串会显式清空低优先级的用户 context。默认值为空。
- `aiActContext: string`（已废弃）：`contexts.aiAct` 的兼容配置。两者同时存在时以 `contexts.aiAct` 为准；没有 `contexts.aiAct` 时，它与 `contexts.aiAct` 的作用相同。新代码请使用 `contexts.aiAct`；更早的名称 `aiActionContext` 也继续兼容。
...
`aiAsk()` 与 `aiString()` 的查询和返回行为相同，但仍可分别通过 `contexts.aiAsk` 和 `contexts.aiString` 配置 Agent 级 context。
...
`aiString()` 与 `aiAsk()` 的查询和返回行为相同，但仍可分别通过 `contexts.aiString` 和 `contexts.aiAsk` 配置 Agent 级 context。
...
- 参数：
  ...
  - `options?: object` - 可选的运行选项
    - `context?: string` - 应用于本次运行每个步骤的 AI 补充信息。对于每个映射后的 API 调用，它会覆盖 Agent 的 API 级 context 和 `contexts.default`
...
- 参数：
  ...
  - `context`：提供给 AI 的补充信息，例如业务事实、规则、约束或输出要求。对 API target 传入 `''` 会屏蔽共享默认值；传入 `undefined` 会移除该 API 的覆盖值，重新回退到 `contexts.default`。

- 示例：

```typescript
agent.setContext('default', '页面中的价格单位是美元。');
agent.setContext('aiQuery', '金额不返回货币符号。');
agent.setContext('aiQuery', ''); // aiQuery 不使用 contexts.default
agent.setContext('aiQuery', undefined); // aiQuery 重新继承 contexts.default
```

#### `setAIActContext()`（已废弃） {#agentsetaiactcontext}

已废弃的兼容性 setter，用于设置后续 `agent.aiAct()` 或 `agent.ai()` 调用发送给 AI 模型的背景知识。新代码请使用 `agent.setContext('aiAct', context)`。该方法仍与后者等价，并覆盖原有的 `contexts.aiAct`。
...
`agent.setAIActContext()` 和更早的 `agent.setAIActionContext()` 都仅为兼容而保留；新代码请使用 `agent.setContext('aiAct', context)`。
...
使用 `contexts.default` 为所有 AI API 提供共享的缺省回退值，使用 `contexts[apiName]` 配置某个 API，再使用单次调用的 `options.context` 配置某一次请求。只有两个更具体的层级都没有提供值时，才会选用 `contexts.default`；它不会与其他层级自动拼接。

```typescript
import { mergeAIContexts } from '@midscene/core';

const defaultContext = '页面中的价格单位是美元。';
const agent = new PlaywrightAgent(page, {
  contexts: {
...
最终用户 context 为 `options.context ?? contexts[apiName] ?? contexts.default`。这些层级互相覆盖，不会隐式拼接。`''` 是一个已定义值，会清空低优先级用户 context；`undefined` 或未配置字段则继续向下 fallback。需要显式拼接时，可以使用 `mergeAIContexts()`。
...
框架自有 context（例如 Test Runner 工作流 history）会追加在选中的用户 context 之后，不受上述覆盖链影响。
...
- 其余字段与[通用构造参数](#common-parameters)一致，包括 `generateReport`、`reportFileName`、`contexts`、`aiActContext`、`modelConfig`、`cache`、`createOpenAIClient` 和 `onTaskStartTip` 等。
