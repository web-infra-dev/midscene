...
- `ListBrowserPages({ offset? })` —— 每次列出 8 个标签页，默认从 offset 0 开始；传入 `{ offset: 8 }` 查看下一组。列表反馈会分别缩短过长的 title 和 URL；offset 越界时报错。
- `GetBrowserPageInfo({ index })` —— 读取标签页的完整 title 和 URL，不切换页面。规划反馈不截断；index 越界时报错。
- `SetActivePage({ index?, title?, url? })` —— 按 index、title 子串或 URL 子串切换页面。组合使用时，所有 selector 必须匹配同一页面。这三个 action 仅供 Browser Agent 使用。
...
- `wdaBaseUrl?: string` —— 完整的 HTTP(S) WDA API 基地址，可包含网关路径前缀。优先于 `wdaHost` 和 `wdaPort`。
