...
- `waitForInitialPageReady: WaitForInitialPageReadyOptions`：自定义[首次页面就绪](#initial-page-ready)判断。
...
配置 `waitForInitialPageReady`，让 Agent 在首次截图、查询、断言或动作前等待应用就绪。构造器只保存 handler；首次使用 Agent 前，应先打开页面或启动 App。

- `handler: ({ signal }: InitialPageReadyContext) => Promise<void>`：必填，初始页面就绪时 resolve。
- `timeoutMs?: number`：超时时间，默认为 `10000` 毫秒，取值为 `1` 到 `2147483647` 之间的整数。

```ts
import { setTimeout as delay } from 'node:timers/promises';
import { PlaywrightAgent } from '@midscene/web/playwright';

// 应用就绪后，在 <html> 上设置 data-app-ready="true"。
...
- 每个 Agent 只执行一次，并发调用共享结果；跳转、刷新和切换标签页不会重置。
- reject 或超时会阻止执行，包括缓存回放。后续调用复用失败结果，不会重试。
- 超时或 `agent.destroy()` 会中止 `signal`，回调应据此停止轮询、清理监听。取消单次动作不会中止共享初始化。
- 不要在 handler 内调用同一个 Agent 的 UI 方法（如 `aiWaitFor()`、`aiQuery()`）。这些方法也要等待 handler 完成，会造成循环等待，直到超时。请使用底层 page/device API。
- 请检查 `data-app-ready` 等就绪状态。一次性的 `ready` 事件可能在 handler 开始前已发出，后续监听会错过。

所有平台的 Agent 均支持此配置。动作后的等待策略及未配置 hook 时的行为保持不变。

使用 `puppeteerAgentForTarget` 时，在第二个参数中传入配置。启动器先导航，再等待 hook（替换初始网络空闲检查），然后返回就绪的 Agent。
...
| [`waitForInitialPageReady`](#initial-page-ready) | 自定义首次就绪判断，每个 Agent 在首次读取或操作页面前执行一次。 |
