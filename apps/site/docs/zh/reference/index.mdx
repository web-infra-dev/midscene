...
- 参数：
  ...
  - `options?: object` - 可选，一个配置对象，包含：
    ...
    - `screenshotIncluded?: boolean` - 是否向模型发送截图。默认值为 true。
    - `context?: string` - 本次调用的额外上下文。详见[单次调用上下文](#单次调用上下文)。
    - `observationRecord?: UIObservationRecord` - `observer.exportRecord()` 返回的观察记录对象。设置后，断言会基于这段画面记录执行，而不是读取当前屏幕。
...
- `observer.exportRecord(): Promise<UIObservationRecord>` - 返回录制得到的帧序列。各帧引用磁盘上的图片文件，不在对象中嵌入 base64 数据。
- `observer.dispose(): Promise<void>` - 不再需要导出的观察记录后，删除 observer 持有的图片文件。使用该记录执行最后一次断言前，不要调用此方法。
...
await agent.aiAssert('过程中弹出了成功提示 toast', undefined, {
  observationRecord,
});
```
