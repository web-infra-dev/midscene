...
- `screenshotStrategy?: 'auto' | 'always-yadb'` —— 控制截图方式，默认 `'auto'`。
  - `'auto'`（默认）—— 按 scrcpy → `adb.takeScreenshot` → `screencap` → yadb 的顺序依次尝试。
  - `'always-yadb'` —— 跳过前面的路径，直接使用 [yadb](https://github.com/ysbing/yadb) 截图。适用于 `screencap` 对安全页面（`FLAG_SECURE`）截出黑屏、但 yadb 能正常截取的场景（如已 root 并通过 Magisk hook 绕过 `FLAG_SECURE` 的设备，或 Android 11 及以下系统）。注意 yadb 只能截取默认显示器（displayId=0）的内容。也可通过环境变量 `MIDSCENE_ANDROID_SCREENSHOT_STRATEGY=always-yadb` 设置。
