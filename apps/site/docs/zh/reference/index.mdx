...
- `aiActContext: string`（已废弃）：`aiContexts.aiAct` 的兼容配置。两者同时存在时以 `aiContexts.aiAct` 为准；没有 `aiContexts.aiAct` 时，它与 `aiContexts.aiAct` 的作用相同。新代码请使用 `aiContexts.aiAct`。关于上下文知识的编写和传入方式，参见[为 GUI Agent 补充应用知识](../provide-application-knowledge-to-gui-agent)。
- `aiActionContext: string`（已废弃）：`aiContexts.aiAct` 的更早兼容名称，仍受支持。仅当 `aiContexts.aiAct` 和 `aiActContext` 均未设置时使用。解析规则为 `aiContexts.aiAct ?? aiActContext ?? aiActionContext`；空字符串也会覆盖低优先级的兼容字段。新代码请使用 `aiContexts.aiAct`。
...
- `waitForActionReady: WaitForActionReadyOptions`：自定义[每个动作后的就绪判断](#action-ready)。
- `waitAfterAction: number`：每次动作后的固定延时，默认为 `300` 毫秒，见[默认等待策略](#default-waiting-strategy)。
...
设备 CLI 可以按单次调用传入可序列化的 Agent 行为参数。`waitForActionReady.createWaiter` 等包含函数的配置需要通过 JavaScript/TypeScript SDK 传入。把 API 的 camelCase 参数名转换成不带平台前缀的 kebab-case flag，例如 `waitAfterAction` -> `--wait-after-action`。各平台 CLI 入口请参考 [Skills](../skills)。

<a id="action-ready"></a>

**每个动作后的就绪判断**

配置 `waitForActionReady`，自定义何时可以进行下一步。它作用于每个原子动作（如 `Input`、`Tap`），包括 `aiAct` 子步骤和缓存回放；首次页面加载和只读查询不会触发。

- `createWaiter: (context: ActionReadyContext) => ActionReadyPlan | Promise<ActionReadyPlan>`：在每个动作前注册监听。context 包含 `action: { id, name, param }` 和 `signal`；每次执行都有新 ID，参数已完成解析和定位。
- `timeoutMs?: number`：分别限制 `createWaiter`、`wait` 和 `dispose` 的耗时，默认为 `10000` 毫秒，取值为 `1` 到 `2147483647` 之间的整数；不改变驱动执行动作的超时。

| 工厂返回值 | 动作执行返回后 |
| --- | --- |
| `'skip'` | 跳过自动稳定等待，继续下一步。 |
| `'default'` | 使用[默认等待策略](#default-waiting-strategy)。 |
| `{ wait, dispose? }` | 等待 `wait(): Promise<void>` 完成，再调用可选的 `dispose()` 清理。 |

下面的 `appMonitor` 是用户自己的业务监控适配器：确认同步完成的输入可以跳过等待，登录则观察本次请求及渲染完成，其他动作沿用默认策略。
...
await agent.aiAct('输入账号密码登录');
```

- `createWaiter` 负责注册监听，`wait` 负责等待动作结果；不要在工厂里等待尚未执行的动作触发事件。就绪信号应对应本次动作，避免旧状态提前放行。
- `'skip'` 和自定义 waiter 替代通用动作延时及平台动作后检查。键盘准备、手势时长等驱动必需时序和生命周期回调仍然执行。未配置时行为不变。
- 失败或超时会终止流程，不会自动重放动作或回退到规划；下一次显式调用会创建新 waiter。动作失败时也会清理；取消、超时或 `agent.destroy()` 会中止 `signal`。
- 回调中请使用底层 page/device API。回调运行期间，调用同一 Agent 的 UI 方法会明确报错，并发调用也一样。仅凭当前没有网络请求，不能保证 UI 已就绪。

所有平台的 Agent 均支持此 SDK 配置。`puppeteerAgentForTarget` 在第二个参数中接收它，并保留初始网络空闲等待。

<a id="default-waiting-strategy"></a>

**默认等待策略（高级配置）**

未配置自定义 waiter 或返回 `'default'` 时，以下参数生效：

| 参数 | 行为 |
| --- | --- |
| `waitAfterAction` | 动作后的固定延时，默认为 `300` 毫秒。设备动作显式声明的延时优先。 |
| `waitForNavigationTimeout`（Web） | 等待 `<html>` 元素可用，超时默认为 `5000` 毫秒，不保证导航或渲染已完成。 |
| `waitForNetworkIdleTimeout`（仅 Puppeteer） | 动作后网络空闲检查，超时默认为 `2000` 毫秒；要求连续 `200` 毫秒最多只有两个进行中的请求。 |

两项 Web 超时参数均可设为 `0` 跳过检查；检查超时会记录警告并继续执行。
...
| 入口 | 用途 |
| --- | --- |
| [`agent.aiWaitFor(condition)`](#agentaiwaitfor) | 在脚本步骤中通过 AI 等待可见条件。 |
| [`browserAgent.waitForNewPage(action)`](#playwright-browser-agent) | 先监听，再执行 `action`，然后返回新页面（Playwright/Puppeteer）。 |
| [`waitForActionReady`](#action-ready) | 自定义每个原子动作之后何时可以继续。 |

其他事件直接 `await` 应用的 Promise 或浏览器、设备 API；一次性事件应先订阅再触发。
...
`aiWaitFor()` 每次检查都会消耗模型调用时间和 token。应用提供就绪信号时，可直接等待该信号。
...
- 参数：
  ...
  - `aiActContext: string` - 要发送给 AI 模型的背景知识。关于上下文知识的编写和传入方式，参见[为 GUI Agent 补充应用知识](../provide-application-knowledge-to-gui-agent)。
...
`ComputerDevice` 与 `RDPDevice` 提供以下动作，Midscene Agent 在规划任务时可以使用这些操作：

- `Tap` —— 左键点击目标。
- `DoubleClick` —— 双击目标。
- `RightClick` —— 右键点击目标。
- `Hover` —— 将鼠标移动到目标上并悬停。
- `DragAndDrop` —— 从一个位置拖拽并放到另一个位置。
- `Input` —— 输入文本，支持 `replace`、`typeOnly` 和 `clear` 模式（`append` 是 `typeOnly` 的已废弃别名）。
- `ClearInput` —— 清空输入框内容。
- `KeyboardPress` —— 按下指定按键或组合键，可选先聚焦目标。
- `CursorMove` —— 在输入框或文本区域中向左或向右移动文本光标。
- `Scroll` —— 滚动屏幕或指定区域，支持滚动到顶、底、左、右边界。
- `Swipe` —— 按住鼠标主键并连续移动。适用于滑块等直接手势交互，可指定起点和终点，或方向与距离；浏览屏幕外内容请使用 `Scroll`。
- `ListDisplays` —— 列出可用显示器。本机设备返回已连接的显示器；RDP 设备将当前远程会话作为单个显示器返回。
...
`ComputerAgent` 继承[通用 Agent 交互方法](#interaction-methods)。桌面端特有的动作及其限制见上方的 Action Space。
