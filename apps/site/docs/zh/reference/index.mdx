...
`startObserving()` 用来记录一段时间内的屏幕变化。先开始录制，再执行页面操作，最后调用 `stop()`。`stop()` 返回一个 `UIObservation`，你可以用它查询或断言录制期间出现过的内容。例如，某个 toast 只短暂出现，调用普通 `agent.aiAssert()` 时可能已经消失；`observation.aiAssert()` 仍可以从录制帧中判断它是否出现过。
...
- `observer.stop(): Promise<UIObservation>` - 停止采样，并返回本次录制结果。

`UIObservation` 表示调用 `stop()` 时确定下来的录制结果。它提供 `aiQuery()`、`aiBoolean()`、`aiNumber()`、`aiString()`、`aiAsk()` 和 `aiAssert()`。这些方法只分析已经录下的图片，不会读取调用时的页面 DOM。因此，`UIObservation` 的 options 不包含 `domIncluded`；即使调用方绕过 TypeScript 类型检查传入该参数，运行时也会抛错。

`UIObservation` 还提供 `frameCount`、`startedAt` 和 `endedAt`。完成最后一次查询或断言后，可以调用 `observation.dispose()` 提前删除临时图片；如果没有调用，`agent.destroy()` 也会清理这些文件。
