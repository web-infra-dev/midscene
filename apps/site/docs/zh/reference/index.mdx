...
- AI 页面选择 action —— Browser Agent 的 `actionSpace` 包含 `ListBrowserPages`、`GetBrowserPageInfo` 和 `SetActivePage`。`ListBrowserPages` 每次显示 8 个标签页，默认从 offset 0 开始；传入 `{ offset: 8 }` 可查看接下来的 8 个。列表反馈会分别截断过长的 title 和 URL，尽量保留相似页面之间的区别。需要完整信息时，可调用 `GetBrowserPageInfo({ index })` 读取完整 title 和 URL，且不会切换 active page；它的反馈会原样进入下一轮规划，不做截断。index 或 offset 超出范围会报错，Agent 可重新获取页面列表。AI 可以按 index、title 或 URL 切换 active page；组合多个 selector 时，它们必须匹配同一个页面。Page Agent 不包含这些 action。
