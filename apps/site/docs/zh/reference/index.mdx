...
每次 RDP 截图都读取当前 framebuffer，包括连接后的首次截图。仅每次连接后的首次截图会等待连续 300ms 没有 framebuffer 更新，最多等待 3 秒。持续变化的内容可能用满首次等待时间。后续截图直接复制最新 framebuffer，不再等待绘制间歇。纯黑画面也是有效的截图内容，不会因此报错。连接中断会结束首次等待；无效的 framebuffer 数据仍会抛出错误。

这个等待窗口可以减少在局部更新期间截取画面的情况，但不保证页面已加载完成，也不代表 Windows 已登录。开始业务操作前，请用 `aiWaitFor()` 检查预期的桌面或应用界面。完整的 Windows 登录页属于有效截图，仍需要完成身份认证。
