...
- 参数：
  ...
  - `options?: object` - 可选，一个配置对象，包含：
    ...
    - `screenshotIncluded?: boolean` - 是否向模型发送截图。默认值为 true。
    - `context?: string` - 本次调用的额外上下文。详见[单次调用上下文](#单次调用上下文)。
    - `observationRecord?: string | UIObservationRecord` - 观察记录 JSON 清单的路径，或已经解析的记录对象。设置后，断言会基于这段画面记录执行，而不是读取当前屏幕。
...
function startObserving(options?: {
  outputPath?: string; // JSON 清单路径；图片保存在相邻的 .frames 目录
...
- `observer.exportRecord(): Promise<string>` - 完成 JSON 清单和相邻图片目录的写入，并返回清单的绝对路径。
