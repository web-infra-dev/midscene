...
除了 `fileChooserAccept` option 外，也可以直接在 `aiAct` 提示词中写入文件的准确路径，让 Midscene 上传文件。Midscene 会在打开文件选择器的操作之前规划文件选择器配置。

```typescript
await agent.aiAct(
  '先点击“上传头像”按钮并上传 /absolute/path/to/avatar.png；然后点击“上传封面”按钮并上传 /absolute/path/to/cover.png。上传完成后，请确认页面头像显示一只猫、封面显示一片海滩。',
);
```

如果一次 `aiAct` 需要上传多个文件，请在提示词中把每个路径与对应的上传操作明确写出。后出现的路径会覆盖此前配置的文件选择器路径。不要将提示词中的文件路径与 `options.fileChooserAccept` 混用：模型在规划过程中生成的后续文件选择器配置可能覆盖该 option 的值。

此能力仅适用于 web 页面（Playwright、Puppeteer 和 Chrome extension Bridge mode）。请提供准确路径，推荐使用绝对路径；相对路径会基于当前命令执行目录解析。
