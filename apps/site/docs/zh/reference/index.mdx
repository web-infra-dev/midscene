...
- `keyboardEventMode?: 'logical' | 'physical'`：控制 macOS AppleScript 键盘后端发送修饰键的方式。`'logical'` 使用标准 AppleScript 键盘事件，也是默认值。`'physical'` 为 VNC 客户端发送明确的按下和释放事件。Physical 模式要求本地与远程系统使用相同的 en-US 键盘布局。文本输入还需要使用 `'sequential'`，或设置正数 `keyboardTypeDelay`。这个选项仅对 macOS AppleScript 键盘后端生效。详见[通过 macOS 上的 VNC 客户端控制远程桌面](../platforms/desktop#vnc-keyboard)。
