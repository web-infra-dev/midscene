...
- `scrcpyConfig?: object` —— scrcpy 截图配置，默认关闭。
  ...
  - `videoBitRate?: number` —— scrcpy H.264 编码码率，单位为 bps。仅当设备端点或 ADB Server host 包含字面量非回环 IP 时，Midscene 才默认使用 `4000000`；其他情况默认使用 `100000000`。端口号不参与判断，因此 `127.0.0.1:5555` 仍使用本地默认值；显式配置始终优先。
