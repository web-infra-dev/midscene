...
- 参数：
  ...
  - `options?: object` - 可选，一个配置对象，包含：
    ...
    - `deepThink?: 'unset' | 'auto' | true | false` - 已弃用并被忽略。Planning 按需使用 SubGoals，详见[Planning 与按需 SubGoals](#aiact-deepthink)。
...
// Planning 按任务需要跟踪子目标
await agent.aiAct('完成 github 账号注册的表单填写。地区必须选择「加拿大」。确保表单上没有遗漏的字段，确保所有的表单项能够通过校验。 只需要填写表单项即可，不需要发起真实的账号注册。 最终请返回表单上实际填写的字段内容');
```

##### Planning 与按需 SubGoals {#aiact-deepthink}

标准 `aiAct` Planning 每轮分析当前状态，保存相关 Memory，并将 Log 操作预告与执行结果区分记录。任务涉及依赖关系、多个关联结果或覆盖范围时，模型按需创建 SubGoals；简单任务可以不拆解。

`deepThink` 和实验性 `effort` 参数已弃用并被忽略。旧调用仍可传入，但不再选择规划模式，也不触发额外的分类调用。

通过 `MIDSCENE_PLANNING_SCREENSHOT_COUNT=1/2` 设置执行截图上限，通过 `MIDSCENE_PLANNING_SEPARATE_LOCATE=true/false` 选择 Planning 与 Locate 分开或合并调用。默认值、校验规则和报告字段见[Planning 配置](../model-config)。Memory、Log 和 TaskScope 不再提供开关。

这些配置不改变[模型原生思考](../model-config#model-native-reasoning)。
...
- 参数：
  ...
  - `options?: object` - 可选的运行选项
    ...
    - `deepThink?: 'unset' | 'auto' | true | false` - 已弃用；兼容传给 `Given` 和 `When` 步骤对应的 `aiAct`，运行时忽略
    - `deepLocate?: boolean` - 已弃用；兼容传给 `Given` 和 `When` 步骤对应的 `aiAct`，运行时忽略
...
旧 `deepThink` 名称在不同 API 中的兼容行为不同：

- 对于 `aiAct()`，该参数已弃用并被忽略，Planning 按需使用 SubGoals；`deepLocate` 仍控制本节描述的定位增强。
- 对于 `aiTap`、`aiHover` 等单步方法，旧 `deepThink` 别名仍等同于 `deepLocate`；新代码请使用 `deepLocate`。
