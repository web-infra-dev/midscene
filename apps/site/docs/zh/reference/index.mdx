...
设置 `deepThink: 'auto'` 时，Midscene 会在开始规划前额外调用一次当前配置的 Planning 模型，根据任务、最终生效的用户 context、初始截图和参考图片，判断是否开启深度思考。选定的模式在本次 `aiAct` 中保持不变。目标和值明确的直接操作倾向于普通规划；需要追踪依赖目标、跨页面记忆、比较信息或处理条件分支的任务倾向于深度思考。
...
```bash
# 子目标和 memory 仍由 deepThink 控制，规划与定位合并调用。
