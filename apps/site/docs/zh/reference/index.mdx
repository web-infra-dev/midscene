...
- 参数：
  ...
  - `options?: object` - 可选，一个配置对象，包含：
    ...
    - `fileChooserAccept?: string | string[]` - 当文件选择器弹出时，指定对应的文件路径。可以是单个文件路径或路径数组。仅在 web 页面（Playwright、Puppeteer 或 Chrome extension Bridge mode）中可用。该 option 不受 `fileChooserAllowedDir` 限制。
    ...
    - `fileChooserAllowedDir?: string`：限制当前 `aiAct` 通过提示词上传文件时可直接指定的路径范围，默认使用 `process.cwd()`。提示词中的相对路径会相对此目录解析为绝对路径，并校验是否位于此目录下；绝对路径则直接校验。位于此目录下的 symlink 也允许上传。
...
`aiAct` 可以上传提示词中提到的文件，默认以当前工作目录作为路径根目录。若要为该次调用使用其他根目录，请传入 `fileChooserAllowedDir`。Midscene 会在打开文件选择器的操作之前规划文件选择器配置；相对路径和绝对路径都会先解析，再校验解析结果是否仍在所选目录内。
