...
每次 RDP 截图都读取当前 framebuffer，包括连接后的首次截图。截图会等待连续 300ms 没有 framebuffer 更新，每次最多等待 3 秒。视频等持续变化的内容可能用满等待时间，然后返回最新画面。如果 framebuffer 始终为纯黑，截图会在超时后抛出错误；连接中断会结束等待。
