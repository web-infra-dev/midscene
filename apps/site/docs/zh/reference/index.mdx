...
- 一个设备连接对应一个 Agent。
- `customActions` 用于配置 `AndroidDevice` 的额外动作。可以将它传给 Device 构造函数或 `agentFromAdbDevice()`。
...
- 一个设备连接对应一个 Agent。
- `customActions` 用于配置 `IOSDevice` 的额外动作。可以将它传给 Device 构造函数或 `agentFromWebDriverAgent()`。
...
- 一个设备连接对应一个 Agent。
- `customActions` 用于配置 `HarmonyDevice` 的额外动作。可以将它传给 Device 构造函数或 `agentFromHdcDevice()`。
