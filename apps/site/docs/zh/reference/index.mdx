...
- 参数：
  ...
  **推荐用法**：
  ...
  - `opt: object` - 配置对象，包含：
    ...
    - `keyboardTypeDelay?: number` - 非负的按键间延迟（毫秒）。设为正数后，legacy 输入会逐字符执行。适用于输入框在快速输入下丢字的场景。
    - `inputStrategy?: 'legacy' | 'sequential' | 'bulk'` - 控制 Midscene 如何把文本发送给平台。默认值为 `'legacy'`。
      - `'legacy'`：保留各平台现有行为。它是兼容性默认值，升级后不会改变已有测试的输入方式。
      - `'sequential'`：逐个 Unicode code point 发送，即使未设置 `keyboardTypeDelay` 或将其设为 `0` 也会逐个发送。
      - `'bulk'`：在平台支持时，通过一次平台输入操作发送完整字符串。Web 的 `replace` 会先选中旧值，再使用 `insertText`，因此只产生一次插入 `input` 事件，不会先产生清空事件再逐字符触发。其他平台会由 Midscene 向底层输入后端发起一次调用，但操作系统或驱动仍可能合成多个应用事件。
      - 正数 `keyboardTypeDelay` 不能与 `'bulk'` 同时使用；配置冲突时 Midscene 会抛出错误。动作级配置优先于 Device 或 Agent 默认值。
...
- `inputStrategy: 'legacy' | 'sequential' | 'bulk'` —— `Input` 动作的默认文本输入策略。使用 `'bulk'` 一次性插入完整字符串，或使用 `'sequential'` 强制逐个 code point 输入。不要同时使用 `'bulk'` 和正数 `keyboardTypeDelay`。默认值为 `'legacy'`。
...
- `keyboardTypeDelay?: number` —— 输入文本时每个按键之间的延迟（毫秒）。在 `'legacy'` 模式下，正数延迟会让原生 `input text` 路径逐个 Unicode code point 输入，yadb 仍会忽略该选项，以保留现有行为。如需拆分 yadb 输入，请将 `inputStrategy` 设为 `'sequential'`。
- `inputStrategy?: 'legacy' | 'sequential' | 'bulk'` —— `Input` 动作的默认文本输入策略。`'sequential'` 对每个 code point 调用一次 ADB/yadb，`'bulk'` 在所选 IME 支持时只调用一次底层文本接口。不要同时使用 `'bulk'` 和正数 `keyboardTypeDelay`。默认值为 `'legacy'`。
...
- `keyboardTypeDelay?: number` —— 非负的按键间延迟（毫秒）。设为正数后，legacy 输入会通过 WDA 的 `/wda/keys` 接口逐字符执行。适用于输入框在快速输入下丢字的场景。
- `inputStrategy?: 'legacy' | 'sequential' | 'bulk'` —— `Input` 动作的默认文本输入策略。`'sequential'` 对每个 code point 调用一次 WDA，`'bulk'` 只调用一次 WDA 文本接口。不要同时使用 `'bulk'` 和正数 `keyboardTypeDelay`。默认值为 `'legacy'`。
...
- `keyboardTypeDelay?: number` —— 非负的按键间延迟（毫秒）。设为正数后，legacy 输入会通过 `uitest uiInput inputText` 逐字符执行。适用于输入框在快速输入下丢字的场景。
- `inputStrategy?: 'legacy' | 'sequential' | 'bulk'` —— `Input` 动作的默认文本输入策略。`'sequential'` 对每个 code point 调用一次 HDC，`'bulk'` 只调用一次 HDC 文本接口。不要同时使用 `'bulk'` 和正数 `keyboardTypeDelay`。默认值为 `'legacy'`。
- `screenshotResizeScale?: number` —— **已废弃。** 此选项已移除，不再生效。如需控制发送给 AI 模型的截图尺寸，请使用 `AgentOpt` 中的 `screenshotShrinkFactor`。
- `customActions?: DeviceAction[]` —— 添加额外的自定义动作，让 Agent 可以调用你定义的领域特定动作。
...
- `inputStrategy?: 'legacy' | 'sequential' | 'bulk'`：默认文本输入策略。`'sequential'` 在本机使用真实按键事件，在 RDP 中为每个 code point 调用一次后端；`'bulk'` 在本机使用一次剪贴板粘贴，在 RDP 中只调用一次后端。不要同时使用 `'bulk'` 和正数 `keyboardTypeDelay`。默认值为 `'legacy'`。
