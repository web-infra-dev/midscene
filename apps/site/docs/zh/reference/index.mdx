...
- AI 页面选择 action —— Browser Agent 的 `actionSpace` 包含 `ListBrowserPages` 和 `SetActivePage`。AI 可以先查看已打开的标签页，再按 index、title 或 URL 切换 active page。index 可以与 title 或 URL 一起使用。此时，所有 selector 必须匹配同一个页面。匹配失败后，Agent 需要重新获取页面列表。Page Agent 不包含这些 action。
