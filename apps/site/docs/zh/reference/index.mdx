...
- `globalContext: string`：所有需要调用 AI 模型的 Agent API 都会携带的背景上下文。各方法的单次调用 `options.context` 会追加在它之后。默认值为空。
...
创建 Agent 时可以通过 `globalContext` 设置所有 AI 方法共享的背景上下文。每个 `agent.ai*` 方法的 options 也支持可选的 `context` 字符串，用于补充仅与本次请求相关的业务知识、用户状态、术语或其他背景。
...
- 其他 `ai*` 方法：`globalContext + options.context`

因此，对于 `aiAct`，单次调用的 `context` 仍然会替换 `aiActContext`，显式传入空字符串也会替换；它不会替换 `globalContext`。其他 AI 方法不会继承 `aiActContext`。
...
- 其余字段与[通用构造参数](#common-parameters)一致，包括 `generateReport`、`reportFileName`、`globalContext`、`aiActContext`、`modelConfig`、`cache`、`createOpenAIClient` 和 `onTaskStartTip` 等。
