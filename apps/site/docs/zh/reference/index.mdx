...
- 参数：
  ...
  **推荐用法**：
  - `locate: string | object` - 用自然语言描述的元素定位，或[使用图片作为提示词](#使用图片作为提示词)。
  - `opt: object` - 配置对象，包含：
    - `keyName: string` - **必填**，要按下的键，如 `Enter`、`Tab`、`Escape` 等。Web 和 Computer Device 还支持 modifier shortcut，如 `Control+A`、`Shift+Enter`；使用 `+` 连接按键。内置的 Android、iOS 和 Harmony Device 仅支持单键；自定义 Device 是否支持组合键取决于其实现。可在[我们的源码中查看完整的按键名称列表](https://github.com/web-infra-dev/midscene/blob/main/packages/shared/src/us-keyboard-layout.ts)。
    - `deepLocate?: boolean` - 是否开启[深度定位](#深度定位deeplocate)。该参数原来叫 `deepThink`，现已更名为 `deepLocate`。默认值为 false。
    - `context?: string` - 本次调用的额外上下文。详见[单次调用上下文](#单次调用上下文)。
    - `xpath?: string` - 目标元素的 xpath 路径，用于执行当前操作。如果提供了这个 xpath，Midscene 会优先使用该 xpath 来找到元素，然后依次使用缓存和 AI 模型。默认值为空
    - `cacheable?: boolean` - 当启用 [缓存功能](../caching.mdx) 时，是否允许缓存当前 API 调用结果。默认值为 true
...
在一段时间内持续采样屏幕。停止 observer 后，Midscene 会返回固定的 `UIObservation`。它的只读 insight 方法基于整段有序画面判断状态或变化，适合捕捉实时单屏 insight 容易错过的 toast、横幅或页面切换。
...
- `observer.stop(): Promise<UIObservation>` - 停止采样、截取最后一张代表性截图，并返回固定的观察窗口。

`UIObservation` 与 `Agent` 提供相同的只读 insight 方法：`aiQuery()`、`aiBoolean()`、`aiNumber()`、`aiString()`、`aiAsk()` 和 `aiAssert()`。这些方法只分析录制截图，不会读取当前 DOM，也不支持 `domIncluded`。如果无类型的 JavaScript 代码传入 `domIncluded`，Midscene 会直接抛错，避免把实时 DOM 与固定观察窗口混合。

`UIObservation` 还提供 `frameCount`、`startedAt` 和 `endedAt`。最后一次 insight 调用完成后，可以调用 `observation.dispose()` 提前释放图片文件；否则 `agent.destroy()` 会负责清理。
...
各平台 CLI 会在前台持续录制。可以在一个终端中启动录制，在另一个终端或通过手动操作触发交互，然后回到录制终端按 Ctrl+C，完成录制并保存产物：

```bash
# 终端 1：命令会持续运行，直到按下 Ctrl+C
npx -y @midscene/web@1 record start \
  --output ./submission-observation.json

# 终端 2：在终端 1 录制期间执行操作
npx -y @midscene/web@1 act --prompt "点击 Submit 按钮"

# 回到终端 1：按 Ctrl+C，并等待命令输出保存路径

# 录制结束后执行断言
npx -y @midscene/web@1 assert \
  --record ./submission-observation.json \
  --prompt "提交过程中曾出现成功提示 toast"
```

`@midscene/android`、`@midscene/ios`、`@midscene/harmony` 和 `@midscene/computer` 也提供 `record start`。各平台原有的目标参数和 `--output` 都应传给这个前台命令。录制参数包括 `--interval-ms`、`--max-frames` 和 `--watchdog-ms`。默认 watchdog 会在 5 分钟后自动结束并保存录制，只有确实需要无限期录制时才应传入 `--watchdog-ms 0`。

产物由带版本号的 JSON 清单和相邻图片目录组成，既不是编码后的视频，也不是压缩包。对于 `submission-observation.json`，图片会保存在 `submission-observation.frames/` 中；移动或保留产物时，两者必须放在一起。JSON 只保存 JPEG 或 PNG 的相对路径，不包含 base64 图片正文：
...
如果只关心当前画面，直接使用不带 `--record` 的普通 `assert` 命令。在 Web Bridge 模式下，前台录制会占用唯一的 Bridge 连接，因此应手动操作浏览器，不要再启动第二个 Bridge CLI 操作。
...
- 对于 data URL 画面，采样时就会写入图片文件，observer 不会在内存中长期保留整组画面。设备的不透明帧句柄会在录制停止时分小批解码并落盘；执行断言时再按需读取图片。
...
- 一个设备连接对应一个 Agent。
- `customActions` 属于 `IOSDevice` 选项，不属于 `IOSAgent` 构造参数。`agentFromWebDriverAgent()` 可以接收它，是因为这个工厂函数在同一个对象中同时接收 `IOSAgentOpt` 和 `IOSDeviceOpt`。
