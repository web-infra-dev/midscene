...
- `Swipe` —— 按住鼠标主键并连续移动。适用于滑块，以及需要通过起点加终点或方向与距离来操作的控件。
...
`ComputerDevice` 与 `RDPDevice` 支持以下操作。两者共享默认动作名称，但会调用各自的本机或 RDP 输入后端。
...
<a id="desktop-swipe滑动"></a>

**Swipe（滑动）**

按住鼠标主键并连续移动。可以用起点和终点描述控件，也可以用方向和距离描述手势；该动作适用于滑块等直接手势交互，不用于浏览屏幕外内容。

```typescript
await agent.aiAct('将音量滑块从当前位置向右滑动 300 像素');
await agent.aiAct('把进度条滑块拖到最右侧');
```

如需浏览屏幕外内容，请使用 `Scroll`。
