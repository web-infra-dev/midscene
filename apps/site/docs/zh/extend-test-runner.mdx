...
`@midscene/test/midscene` 导出了七个系统内置 Node：`aiAct`、`aiAssert`、`recordToReport`、`launch`、`terminate`、`wait` 和 `agent`。
...
const midsceneNodes = createMidsceneNodes<ProjectContext>({
  ...
  // Web 使用 gotoUrl，不注册设备生命周期 Node。
  includeLaunch: false,
});
```

`includeLaunch: false` 默认也会关闭 `terminate`。只有需要分别控制这两个能力时，
才显式设置 `includeTerminate`。
...
`setCookies` 有意不接受 inline Cookie value。请在 `cookiesEnv`、`profile` 和
`storageStatePath` 中选择且只选择一个。环境变量可以包含 Cookie header、Cookie JSON
数组或 Playwright storage-state JSON。Cookie value 由 Node 内部解析，不会进入 Node
结果或 workflow history。相对的 storage-state 路径默认从当前工作目录解析；如果项目
需要使用其他根目录，请配置 `resolveStorageStatePath`。
...
Android 项目将 `createMidsceneNodes()` 的设备生命周期 Node 与 `runAdbShell` 组合：
...
iOS 项目注册 `runWdaRequest`；`launch` 和 `terminate` 继续使用 Agent 生命周期方法：
...
生成的说明书包含当前 Test Project 实际注册的全部 Node（包含 `createMidsceneNodes()` 返回的七个系统内置 Node），按名称排序。运行器会自动将 Zod `inputSchema` 转换为标准的 JSON Schema。
