...
该命令需要 pnpm 和 Node.js `^20.19.0 || ^22.12.0 || >=24.0.0`。它会通过 pnpm 安装依赖，再运行 `describe-nodes`，将说明书写入 `midscene-nodes.md`。
...
在交互式终端中，`pnpm dlx @midscene/test create` 会询问目录和平台。在 CI 等非交互环境中，必须提供这两个参数。命令不会覆盖已有的目标文件，`--yes` 也不代表允许覆盖。
