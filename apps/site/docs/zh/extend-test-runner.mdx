...
使用 `create` 生成项目。项目包含平台 Agent、示例用例和 Node 说明书：
...
该命令需要 Node.js `^20.19.0 || ^22.12.0 || >=24.0.0`，以及选定的包管理器（npm 或 pnpm）。它会安装依赖，再运行 `describe-nodes`，将说明书写入 `midscene-nodes.md`。
...
使用 pnpm 时，生成的项目包含以下文件。使用 npm 时，锁文件为 `package-lock.json`，而非 `pnpm-lock.yaml`。README 文件只生成 `README.md`：
