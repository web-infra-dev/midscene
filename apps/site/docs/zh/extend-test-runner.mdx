...
`createMidsceneNodes()` 保留 `launch`，用于兼容已有的 Agent 集成。Android 和 iOS
项目应使用各自平台预置提供的生命周期 Node，并在这里设置 `includeLaunch: false`，
避免重复注册 `launch`。
...
Android 平台预置会注册 `launch`、`terminate` 和 `runAdbShell`，并要求 Agent
同时提供这三个能力：
...
iOS 平台预置会注册 `launch`、`terminate` 和 `runWdaRequest`，并要求 Agent
同时提供这三个能力：
...
`runAdbShell` 和 `runWdaRequest` 会在 Node 结果和 workflow history 中保留完整
response。Test Runner 只限制传给后续 Midscene Agent 的 history 内容：过大的 value
