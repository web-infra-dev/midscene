...
### `Doubao-1.5-thinking-vision-pro` （火山引擎）

Doubao-1.5-thinking-vision-pro 是火山引擎提供的模型。它在视觉定位和断言方面表现更好。

**配置**

你可以在 [火山引擎](https://volcengine.com) 上使用 `Doubao-1.5-thinking-vision-pro` 模型。从火山引擎获取 API 密钥后，可以使用以下配置：
...
### `Qwen-2.5-VL` （Openrouter 或阿里云）

从 0.12.0 版本开始，Midscene.js 支持 Qwen-2.5-VL-72B-Instruct 模型系列。

Qwen-2.5-VL 是阿里巴巴发布的开源模型系列。它提供视觉定位能力，可以准确返回页面上目标元素的坐标。在用于交互、断言和查询时表现相当出色。我们建议使用最大版本（72B）以获得可靠的输出。
...
**在 Midscene.js 中使用的限制**

- **小图标识别能力不佳**：识别小图标时，你可能需要[启用 `deepThink` 参数](./blog-introducing-instant-actions-and-deep-think)并优化描述，否则识别结果可能不准确。
- **断言表现不佳**：我们观察到它在断言方面可能不如 `GPT-4o` 或 `Doubao-1.5-thinking-vision-pro`。

**关于在阿里云上部署模型的说明**

虽然开源版本的 Qwen-2.5-VL (72B) 名为 `qwen2.5-vl-72b-instruct`，但阿里云上还托管着一个名为 `qwen-vl-max-latest` 的增强且更稳定的版本。在阿里云上使用 `qwen-vl-max-latest` 模型时，你将获得更大的上下文支持和低得多的价格（可能只有开源版本的 19%）。

简而言之，如果你想要使用阿里云服务，请使用 `qwen-vl-max-latest`。
