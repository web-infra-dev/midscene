...
从 1.0 版本开始，Midscene.js 推荐使用以下新的环境变量名：

- `MODEL_API_KEY` - API 密钥（推荐）
- `MODEL_BASE_URL` - API 接入地址（推荐）

为了保持向后兼容，以下旧的变量名仍然支持：

- `OPENAI_API_KEY` - API 密钥（已弃用，但仍兼容）
- `OPENAI_BASE_URL` - API 接入地址（已弃用，但仍兼容）

当新旧变量同时设置时，新变量（`MODEL_*`）将优先使用。

在本文的配置示例中，我们将使用新的变量名。如果你正在使用旧的变量名，无需立即更改，它们会继续正常工作。
...
```bash
MODEL_API_KEY="......"
MODEL_BASE_URL="https://custom-endpoint.com/compatible-mode/v1" # 可选，如果你想要使用不同于 OpenAI 默认的接入点
