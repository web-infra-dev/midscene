...
### Planning 截图数量与独立 Locate

标准 `aiAct` Planning 固定包含规划分析、Memory、Log 和任务范围／完成规则。模型按任务需要创建和更新 SubGoals，简单任务可以不拆解；不再额外调用 Auto 分类。

原来的 `MIDSCENE_PLANNING_MEMORY`、`MIDSCENE_PLANNING_LOG` 和 `MIDSCENE_PLANNING_TASK_SCOPE` 环境变量已移除。API 的 `deepThink` 和实验性 `effort` 参数仅兼容接收，运行时忽略并提示弃用，不再改变 prompt、截图数量或 Locate 策略。

| 名称 | 说明 |
|------|-------------|
| `MIDSCENE_PLANNING_SCREENSHOT_COUNT` | 支持 `1`（默认）或 `2`。`1` 发送当前执行截图；`2` 发送当前截图和最近一张可用历史截图。第一轮只有一张。用户参考图片单独计算。 |
| `MIDSCENE_PLANNING_SEPARATE_LOCATE` | `true` / `1` 分开调用 Planning 和 Locate；`false` / `0` 由 Planning 同时输出动作和坐标。未设置或为空时，使用默认模型则合并，配置了独立 Planning 模型则分开。 |

这两项配置在每次 `aiAct` 开始时读取一次，并记录到 Plan 报告中。Locate 布尔值不区分大小写。非法值或向自定义 Planning 适配器显式设置时，会在模型推理前报错。合并调用要求 Planning 模型配置支持坐标格式的 family。命中可执行的计划缓存仍跳过 Planning。

Memory 保存已观察到的事实；Log 包含模型操作预告和累计执行结果。预告表示操作意图，不能作为执行证据。TaskScope 规则保留在 Completion Rules 中，包括明确的停止位置，以及不擅自提交或保存的约束。
