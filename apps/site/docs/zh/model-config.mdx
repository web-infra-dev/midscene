...
每条 JSON 事件都会包含 `type` 和 `executionId` 两个字段：
...
- `executionId` 用于关联模型调用与对应的报告 execution。同一个 execution 内发起的模型调用（包括自动重试）会使用相同的 `executionId`。不计入报告 execution 的调用，例如连通性检查，会使用以 `fallback-` 为前缀生成的 ID。
