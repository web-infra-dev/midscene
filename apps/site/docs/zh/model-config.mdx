...
| `MIDSCENE_MODEL_BASE_URL` | API 的接入 URL，常见以版本号结尾（如`/v1`）；不需要编写最后的 `/chat/completions` 或 `/responses` 部分，底层 SDK 会自动添加 |
| `MIDSCENE_MODEL_NAME` | 模型名称 |
| `MIDSCENE_MODEL_FAMILY` | 模型系列，用于确定坐标处理方式 |

## 协议类型 {#model-api-type}

Midscene 默认使用 OpenAI 的 Chat Completions 协议，同时也支持 Responses 协议。两者是向模型发送请求的不同方式，不影响 Midscene Agent API 的使用。Chat Completions 是 OpenAI 较早推出的协议，Responses 是后推出的协议，目前 Chat Completions 的支持范围更广。

通过环境变量选择协议：`chat-completion`（默认）或 `responses`。例如，使用 Responses 协议：

```bash
MIDSCENE_MODEL_API_TYPE="responses"
```

为 Planning 或 Insight 单独配置模型时，使用对应的环境变量，各模型的协议可以分别设置：
...
- 部分模型或供应商可能尚不支持 Responses 协议，使用前请向模型供应商确认支持情况。
- 即使是同一供应商提供的 Chat Completions 和 Responses API，也可能使用不同的 Base URL，请向模型供应商确认对应的接入地址。
- Midscene 对部分模型供应商的 Responses API 可能尚未完成适配，使用前请确认 Midscene 的兼容性支持。
...
| 名称 | 描述 |
|------|-------------|
| `MIDSCENE_MODEL_API_TYPE` | HTTP 协议：`chat-completion`（默认）或 `responses`。后者要求服务商支持 Responses API。 |
...
| `MIDSCENE_MODEL_RESPONSE_FORMAT` | 结构化响应策略：`auto`（默认）表示 Midscene 会在合适的场景自动使用 `response_format`（Chat Completions）或 `text.format`（Responses）参数指定模型输出的结构化格式（一般是 JSON），来尽可能保证模型返回值能够被结构化解析；`none` 表示不指定上述格式参数，适用于模型不支持结构化输出的情况。 |
...
| `MIDSCENE_MODEL_EXTRA_BODY_JSON` | 合并到每次模型请求体中的 JSON。与 `MIDSCENE_MODEL_INIT_CONFIG_JSON`（配置 SDK 客户端）不同，此参数会展开到每次发送到模型的协议请求体中，例如在 vLLM 中启用思考模式：`'{"chat_template_kwargs":{"enable_thinking":true}}'` |
...
| `MIDSCENE_INSIGHT_MODEL_BASE_URL` | API 的接入 URL，常见以版本号结尾（如`/v1`）；不需要编写最后的 `/chat/completions` 或 `/responses` 部分 |
| `MIDSCENE_INSIGHT_MODEL_API_TYPE` | HTTP 协议：`chat-completion`（默认）或 `responses`。 |
...
| `MIDSCENE_PLANNING_MODEL_BASE_URL` | API 的接入 URL，常见以版本号结尾（如`/v1`）；不需要编写最后的 `/chat/completions` 或 `/responses` 部分 |
| `MIDSCENE_PLANNING_MODEL_API_TYPE` | HTTP 协议：`chat-completion`（默认）或 `responses`。 |
