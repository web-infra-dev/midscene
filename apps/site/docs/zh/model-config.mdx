...
| `MIDSCENE_MODEL_REASONING_EFFORT` | 控制模型的推理深度。根据 `MIDSCENE_MODEL_FAMILY` 映射为模型特定参数（如豆包的 `reasoning_effort`、GPT-5 的 `reasoning.effort`、千问的 `thinking_budget`）。常用值：`off`、`low`、`medium`、`high` |
