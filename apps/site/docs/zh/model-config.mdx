...
| `MIDSCENE_INSIGHT_MODEL_TIMEOUT` | Insight 意图的 AI 接口调用超时时间（毫秒）。如果未设置，则使用 `MIDSCENE_MODEL_TIMEOUT` |
| `MIDSCENE_PLANNING_MODEL_TIMEOUT` | Planning 意图的 AI 接口调用超时时间（毫秒）。如果未设置,则使用 `MIDSCENE_MODEL_TIMEOUT` |
...
更多详情，请参阅 [API 文档](./api#使用-langsmith)。
...
更多详情，请参阅 [API 文档](./api#使用-langfuse)。
