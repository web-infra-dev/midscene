...
本篇文档会重点介绍 Midscene 的模型配置参数。如果你对 Midscene 的模型策略感兴趣，请阅读 [模型策略](./model-strategy)。如果你想查看常用模型的配置示例，请阅读 [配置你的模型](./model-common-config)。
...
设置 `MIDSCENE_RECORD_MODEL_CALL=true` 后，Midscene 会记录与模型服务之间的实际交互，便于调试和排查问题。该功能与 `DEBUG` 日志独立。

:::warning 注意事项

- 该功能会以明文记录模型请求时的参数（HTTP body，包括自定义的 `extraBody`）以及模型服务返回的 body 和 headers。记录文件中可能包含敏感信息。请仅在可信环境中启用，并妥善保管或删除生成的文件。
- 记录内容可能包含截图的 base64 数据和完整的流式响应，因此文件体积可能很大。不建议在长期运行的任务中持续开启该功能。
- 这些记录仅用于调试与排查。随着 Midscene 版本更新，事件类型和字段细节可能发生变化，请不要将其作为稳定的程序化接口。

:::

开启后，Midscene 会将每次模型调用的请求和响应写入本地文件；流式调用还会记录每个 chunk。记录文件位于：

```text
midscene_run/model-requests/<启动时间>-<pid>.jsonl
```

每个进程会创建一个独立的 JSONL 文件；JSONL 中每一行都是一个 JSON 事件。该功能仅在 Node.js 和 Electron 环境中生效；浏览器页面和 Worker 环境不会写入本地文件。

文件会记录：

- 模型请求时的 HTTP body，包括自定义的 `extraBody`；请求 HTTP headers 不会被记录；
- 模型服务返回的 HTTP 响应 headers 和 body；
- 如果使用的是 Codex App Server，记录其请求、响应以及可获得的协议元数据。

记录采用 JSONL 格式，每行都是一个 JSON 事件。例如：
...
每条 JSON 事件都会包含 `type` 和 `interactionId` 两个字段：

- `type` 表示事件类型，包含以下四种：
  - `request`：本次模型请求的参数；每次重试都会生成新的 `request` 事件。
  - `chunk`：OpenAI 流式调用中收到的 SDK chunk，或 Codex App Server 当前 turn 的服务端通知或响应。
  - `response`：请求完成后的最终结果。OpenAI-compatible 调用包含 HTTP 元数据和最终拼接结果；Codex App Server 包含最终协议元数据。
  - `error`：一次请求尝试失败时的错误，以及可获得的 HTTP 状态码、响应 headers 和响应 body。
- `interactionId` 用于区分不同 AI 操作产生的模型调用记录。一次 `aiTap` 这样的原子操作对应一个 `interactionId`；对于 `aiAct` 这样会在内部多次调用模型进行规划和执行的操作，同一个 `aiAct` 的所有记录会使用相同的 `interactionId`。如果模型调用发生自动重试，重试产生的记录也会沿用该 `interactionId`。
...
你也可以使用 JavaScript 来为每个 Agent 配置模型参数，详见 [API 参考](./reference/#common)
...
- 如果使用 [`createOpenAIClient`](./reference/#自定义-openai-客户端) 参数，环境变量方式会被覆盖

如果需要更细粒度的控制（例如只对特定任务启用 LangSmith），可以通过 [`createOpenAIClient`](./reference/#自定义-openai-客户端) 手动包装客户端。
