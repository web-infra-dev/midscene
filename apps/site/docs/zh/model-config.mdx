...
**关于 `MIDSCENE_PLANNING_STYLE` 配置**

`MIDSCENE_PLANNING_STYLE` 用于指定 UI-TARS 版本，使用以下值之一：
...
:::tip
旧版本使用 `MIDSCENE_USE_VLM_UI_TARS=DOUBAO` 或 `MIDSCENE_USE_VLM_UI_TARS=1.5` 配置，该配置仍然兼容但已废弃，建议迁移到 `MIDSCENE_PLANNING_STYLE`。
...
| 名称 | 描述 | 新配置方式 |
|------|-------------|-----------|
| `MIDSCENE_USE_DOUBAO_VISION` | 已弃用。启用豆包视觉模型 | 使用 `MIDSCENE_PLANNING_STYLE="doubao-vision"` |
| `MIDSCENE_USE_QWEN3_VL` | 已弃用。启用千问 Qwen3-VL 模型 | 使用 `MIDSCENE_PLANNING_STYLE="qwen3-vl"` |
| `MIDSCENE_USE_QWEN_VL` | 已弃用。启用千问 Qwen2.5-VL 模型 | 使用 `MIDSCENE_PLANNING_STYLE="qwen-vl"` |
| `MIDSCENE_USE_GEMINI` | 已弃用。启用 Gemini 模型 | 使用 `MIDSCENE_PLANNING_STYLE="gemini"` |
| `MIDSCENE_USE_VLM_UI_TARS` | 已弃用。启用 UI-TARS 模型 | 使用 `MIDSCENE_PLANNING_STYLE="vlm-ui-tars*"` |
