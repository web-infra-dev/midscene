...
| `MIDSCENE_MODEL_REASONING_ENABLED` | 开启或关闭模型原生推理/思考。设为 `"true"` 或 `"false"`。对于已适配的模型系列，如果不配置该变量，Midscene 会默认关闭模型原生思考。见[模型原生的思考模式](./model-strategy#模型原生的思考模式)。显式配置 reasoning 时必须使用已支持的 `MIDSCENE_MODEL_FAMILY` |
| `MIDSCENE_MODEL_REASONING_EFFORT` | 推理力度字符串。透传为豆包的 `reasoning_effort`，以及 Codex app-server 的 reasoning effort。常用值：`low`、`medium`、`high` |
