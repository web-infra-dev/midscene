...
- 任务失败现在只保留有界的 `SerializedError`。任意上游 payload、完整 runner 对象图、executor function 和原始错误对象引用会在进入测试运行器、报告 dump 或结构化日志之前被移除。
- `TaskExecutionError` 会保留可操作的 wrapper stack、`cause.stack` 中的原始 executor stack、白名单诊断字段和有界的任务摘要。每个自由文本诊断字段最多保留 4,096 个字符。
- `ExecutionTask.error` 现在是 `SerializedError`，不再是原始 `Error`。调用方应读取 `task.error.message`、`task.error.stack`、`task.error.code`、`task.errorMessage` 或 `task.errorStack`；该字段不再支持原始错误对象引用或 `instanceof Error` 检查。
- `Agent.recordErrorToReport()` 现在接受任意 thrown value，并在报告捕获边界完成归一化。通用的用户可见错误格式化也已增加长度限制，不再序列化无 message 对象中的任意 payload。
