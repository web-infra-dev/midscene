...
- `aiAct` 新增规划模式 `planningStrategy`：`fast` 合并视觉模型的规划和定位能力，执行更快；`standard`（默认）拆分规划和定位并提供上一步的截图，帮助模型理解整个流程
- `aiAct` 支持深度思考能力：帮助模型在复杂场景下保持反思、灵活操作的能力
- 支持多模型结合：任务级别模型配置，可为 Insight、Planning 额外配置更强的 AI 理解模型（如 GPT-5.1 规划 + Doubao 交互），获得最佳表现
...
#### 3⃣ 统一 Puppeteer 和 Playwright 配置
