...
## v1.9 - HarmonyOS YAML 目标与模型适配重构

v1.9 版本将 YAML 自动化扩展到 HarmonyOS，并重构了模型适配器内部实现。
...
- 修复 CLI 依赖解析问题，确保 `@rstest/core` 从 CLI 包自身解析，而不是从 `process.argv[1]` 解析

## v1.8 - YAML 工作流、设备集成与模型行为更新

v1.8 版本改进了 YAML 执行、模型思考模式默认行为、报告工具链，并补充了多项设备与平台集成能力。
...
- Android CLI 新增启用 `scrcpy` 预览的参数
- `WebPageOpt` 支持配置 Web integration 的键盘输入延迟
...
- report-tool CLI 新增 `merge` 操作
- assert CLI / MCP 工具支持传入图片 prompt
- 移除了报告中动作执行前冗余的截图
...
- 修复在 `0.0.0.0` 上探测端口时与实际监听 host 不一致的问题
- 修复 HarmonyOS 清空输入、键盘收起策略和系统动作延迟覆盖问题
- 修复 Computer 默认滚动距离与 Web 不一致的问题，并在销毁 agent 时取消未完成输入
