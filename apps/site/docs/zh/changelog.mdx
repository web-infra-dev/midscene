...
支持通过 CDP (Chrome DevTools Protocol) 直接连接已有的浏览器实例进行自动化，无需由 Midscene 启动浏览器，适用于需要复用已有浏览器会话的场景。详见：[YAML 脚本运行器 - CDP 连接模式](./yaml-script-runner#使用-cdp-连接模式)、[Puppeteer 远程连接](./integrate-with-puppeteer#连接远程-puppeteer-浏览器并接入-midscene-agent)
