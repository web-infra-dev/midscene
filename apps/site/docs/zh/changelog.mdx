...
恢复了 deepThink 模式下的子目标规划（sub-goal planning）支持。在复杂任务场景中，模型可以将大任务拆解为多个子目标逐步执行，提升复杂流程的完成率。同时 MCP 的 act 工具也支持开启深度思考能力。
...
- **时序显示格式优化**：调整时序展示格式，提升可读性
- **报告截图逻辑修正**：修正报告中截图的展示逻辑，确保正确呈现
- **合并报告支持目录模式**：`mergeReports` 支持目录模式的报告文件
- **历史滚动暂停**：用户在可视化面板中向上滚动查看历史时，自动暂停自动滚动
