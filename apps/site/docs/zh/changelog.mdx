...
各平台包（`@midscene/android`、`@midscene/ios`、`@midscene/web` 等）现在各自暴露了独立的 CLI 入口，Skills 正是基于此能力构建。你也可以直接通过 `npx @midscene/android` 等方式在命令行中运行自动化脚本。
...
详见文档：[Computer MCP](./computer-mcp)
