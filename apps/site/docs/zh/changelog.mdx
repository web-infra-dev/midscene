...
## v1.9.x - Studio 录制、Markdown 回放与 HarmonyOS YAML 目标

v1.9.x 覆盖 v1.9.0 到 v1.9.2。当前最新版本可从 [GitHub Releases](https://github.com/web-infra-dev/midscene/releases/latest) 获取，该链接会自动跳转到最新 tag。

### Midscene Studio Beta 录制功能

Midscene Studio 仍处于 **Beta** 阶段。可从 [latest release 页面](https://github.com/web-infra-dev/midscene/releases/latest) 下载，选择 `midscene-studio-beta-*` 资源即可。当前 v1.9.2 直接下载链接：[macOS Apple Silicon DMG](https://github.com/web-infra-dev/midscene/releases/download/v1.9.2/midscene-studio-beta-v1.9.2-darwin-arm64.dmg)、[Windows x64 ZIP](https://github.com/web-infra-dev/midscene/releases/download/v1.9.2/midscene-studio-beta-v1.9.2-win32-x64.zip)、[Linux x64 ZIP](https://github.com/web-infra-dev/midscene/releases/download/v1.9.2/midscene-studio-beta-v1.9.2-linux-x64.zip)。

- **Record and replay 面板**：Studio 可以录制实时设备预览中的人工操作，保留带截图的可视化事件时间线，并在 Studio 内回放录制流程。
- **脚本生成与导出**：录制会话可以生成结构化 Midscene 脚本、带截图的 Markdown 回放内容；Web 录制还可以生成 Playwright 输出。
- **多平台上下文**：录制能力与 Android、iOS、HarmonyOS、Computer、Web 预览共用同一个 Studio Shell，手动设备控制和 API Playground 执行可以在同一个桌面应用内完成。

![Studio API Playground 执行流](/blog/studio-beta-api-playground.png)

![Studio Record and replay 时间线](/blog/studio-beta-record-replay.png)
...
- Report Markdown 导出会让图片路径与导出的截图保持一致。
- Computer 自动化修复了 RDP 首帧空白截图、phased-scroll helper 缺失可执行权限的自修复问题，并补充 elevated Windows 应用输入丢失的警告。
...
- Web integration 不再把 `longPress` 时长限制在 600ms。
...
- **Android `scrcpy` 预览参数**：Android CLI 新增启用 `scrcpy` 预览的参数
- **Web 输入延迟配置**：`WebPageOpt` 支持配置 Web integration 的键盘输入延迟
...
- **Qwen3.7 支持**：新增 Qwen3.7 模型支持
- **Gemini 思考模式适配**：适配 Gemini 的思考模式
...
- 修复销毁 agent 时 Computer 未完成输入未取消的问题
...
- 修复 iOS 输入时偶发的字符丢失问题
- 修复 HarmonyOS 清空输入、键盘收起策略和 system action 延迟覆盖逻辑
- 修复 Android 截图时传入 `null` 作为 `displayId` 的问题
- 修复可能导致模型验证失败的 service import cycle 问题
