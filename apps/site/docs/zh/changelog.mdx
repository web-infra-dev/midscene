...
## v1.7 - Qwen 3.6、报告工具增强与多项修复

v1.7 版本新增了 Qwen 3.6 模型支持、Chrome 扩展 YAML 输出语言配置、报告工具增强，同时包含大量稳定性修复和平台改进。

### 新增 Qwen 3.6 模型支持

适配了 Qwen 3.6 模型，可以在 Midscene 中使用最新的通义千问模型。详见：[模型配置](./model-config)

### Chrome 扩展支持配置 YAML 输出语言

Chrome 扩展录制脚本时，可以自定义输出的语言。

### 报告工具增强

- 报告合并工具现在支持处理单个报告文件
- 新增报告拆分工具，可将大型报告拆分为多个独立文件
- 报告 Markdown 输出中增加了屏幕尺寸和定位中心点信息，便于调试

详见：[解析报告文件](./consume-report-file)

### Android / 鸿蒙端改进

- Android 和鸿蒙端新增 terminate 操作，支持强制停止指定应用，方便在测试中重置应用状态。详见：[Android API](./android-api-reference)
...
- 模型配置保存时可运行连通性测试，及时发现配置错误
- Skill CLI 支持通过图片进行提示
