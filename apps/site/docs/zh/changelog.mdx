...
- `report-tool` 命令行工具新增 `split` 和 `to-markdown` 操作，可以将 HTML 报告拆分为独立截图和 JSON 数据，或转录为 Markdown 供其他工具消费。详见：[解析报告文件](./consume-report-file)
- `mergeReports()` API 现在支持合并单个报告，不再要求至少两个报告
