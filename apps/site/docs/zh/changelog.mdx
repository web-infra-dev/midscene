...
## v1.1 - 深度思考与可扩展的 MCP SDK

v1.1 版本在模型规划能力与 MCP 扩展性上实现突破，让复杂场景的自动化更稳定，同时为企业级部署提供更灵活的方案。

### 模型深度思考能力（deepThink）

#### 为 Qwen3-vl 与 Doubao-vision 开放深度思考
- **更高准确率**: 模型在规划阶段会执行更深层次的推理，特别适用于复杂表单、多步骤流程、元素遮挡或相似控件密集的页面
- **灵活控制**: 可在单次调用或全局配置中启用，按需平衡速度与准确性
- **适用场景**: 关键业务流程、回归测试失败率高的用例、复杂 UI 交互

示例用法：

```typescript
// 方式 1: 在单次调用中开启
await agent.aiAct('提交复杂订单表单', { deepThink: true });

// 方式 2: 在 Agent 配置中统一开启
...
#### Launcher 支持与 SDK 开放
- **多平台 MCP 服务**: 为 Android、iOS 与 Web MCP 新增 launcher 支持，可在自有服务器上启动和管理 MCP 服务
- **企业级部署**: 支持私有设备池管理、内网环境部署，把 Midscene 能力以微服务形式对接到现有平台
- **灵活扩展**: 开放 SDK 接口，方便集成自定义工具链与业务逻辑
...
- **动态配置**: 支持从环境变量读取 replanning cycle limit，方便在不同环境中调整重试策略

### 跨平台兼容性提升
- 优化 `getWindowSize` 方法，在新版本 API 不可用时自动回退到 legacy endpoint，提高跨浏览器兼容性
