...
|模型系列|部署|Midscene 评价|
|---|---|---|
|豆包 Seed 模型<br />[快速配置](./model-common-config#doubao-seed-model)|火山引擎版本：<br />[Doubao-Seed-2.1-turbo](https://www.volcengine.com/docs/82379/1799865)|⭐⭐⭐⭐⭐<br/>定位能力较强<br />目前私有测评集中定位速度最快|
...
- `MIDSCENE_MODEL_REASONING_ENABLED`：显式控制是否开启模型原生思考。
  ...
  - 目前有以下模型支持该参数：
    ...
    - Gemini：对应 Gemini 文档中的 `thinking_config.thinking_level`，开启时默认使用 `medium`，关闭时使用 `minimal`。
...
可以，你可以为每个 Agent 实例单独配置模型，具体请参考 [API 参考](./reference/#common) 中的 `modelConfig` 参数。

### 希望将浏览器 DOM 信息发送给模型？

Midscene 默认不发送浏览器 DOM 信息给模型。如果你仍希望在界面理解时发送 DOM 信息，例如附加一些截图里不可见的信息，可以在 `aiAsk` 或 `aiQuery` 等界面理解接口的 options 参数中，将 `domIncluded` 设置为 `true`。更多详情请参考 [API 参考](./reference/#common)。
