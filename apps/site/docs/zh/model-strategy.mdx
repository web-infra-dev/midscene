...
* [Gemini 3.x 系列](./model-common-config#gemini-3-pro)
* [GPT-5 系列](./model-common-config#gpt-5-4)
...
- **能校验用户真正看到的效果**：由于基于渲染后的画面进行推理，视觉方案可以对视觉结果做断言——颜色、高亮状态、布局等——这些是基于 DOM 的校验完全无法覆盖的。
...
|Gemini 3.x 系列<br />[快速配置](./model-common-config#gemini-3-pro)|[Google Cloud](https://ai.google.dev/gemini-api/docs/models/gemini)|⭐⭐⭐⭐⭐<br />`gemini-3.5-flash` 在私有测评集中定位表现最好<br />价格高于豆包和千问|
...
如果收到了 "MIDSCENE_MODEL_FAMILY is not set to a multimodal model with UI localization" 错误，请确认已经正确配置多模态模型的 `MIDSCENE_MODEL_FAMILY` 环境变量。
