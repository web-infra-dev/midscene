...
如果模型服务商提供了思考策略相关的参数，用 `deepThink` 可以显式控制是否开启它。如果开发者不配置这个参数，它的策略将由模型服务商决定（而不是一定关闭）。

目前 Midscene 在 `qwen3-vl` 和 `doubao-seed-1.6` 系列模型下适配了这个参数，一旦思考特性被开启，你可以在报告右侧看到 `Reasoning` 栏目，Planning 耗时会增加，但结果会更准确。
