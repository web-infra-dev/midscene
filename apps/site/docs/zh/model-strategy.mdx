...
* [Qwen3.X 系列](./model-common-config#qwen3x)、[Qwen3-VL](./model-common-config#qwen3-vl)
...
经过大量项目实测，我们推荐使用这些模型作为使用 Midscene 的默认模型：豆包 Seed、千问 Qwen3.x、智谱 GLM-V、智谱 AutoGLM、Gemini-3-Flash。
...
|Gemini-3-Flash<br />[快速配置](./model-common-config#gemini-3-pro)|[Google Cloud](https://ai.google.dev/gemini-api/docs/models/gemini)|⭐⭐⭐<br />价格高于豆包和千问|
...
`deepThink` 参数用于控制 Midscene 在规划时引导模型思考的力度。

- 默认情况下，`aiAct` 会让模型在同一次规划请求里同时完成下一步规划和目标元素定位。
- 开启 `deepThink` 后，`aiAct` 的规划阶段会专注于规划，不再在规划阶段同时对目标元素做定位，并通过引导模型拆解子任务、梳理任务结构等方式获得更好的规划结果。
...
显式配置 reasoning 时必须使用已支持的 `MIDSCENE_MODEL_FAMILY`。如果没有配置模型系列，或者模型系列不支持 reasoning 配置，Midscene 会直接抛出错误，而不会猜测服务商的私有参数。
