...
- **效果稳定**：这些模型在 UI 操作规划、界面理解等方向表现综合较好，能够帮助开发者更快上手。
- **UI 操作规划**：视觉模型通常具备不错的 UI 操作规划能力，能够完成一些复杂的任务流程。
...
Midscene 的运行报告上详细记录了任务过程，你可以分析这些细节，找到出错的原因。

### 检查或调整模型

1. 确保模型的 `MIDSCENE_MODEL_FAMILY` 环境变量配置正确，否则定位结果会出现明显偏移
2. 使用最新的模型版本，使用更大的模型尺寸。比如 Qwen2.5-VL 的效果会优于 Qwen3-VL，同一个模型的 72B 版本准确性会优于 30B 版本
3. 调整 `aiAct` 的规划模式 `planningStrategy`，提升思考深度
