...
* [千问 Qwen3.5](./model-common-config#qwen35)
...
|千问 Qwen3.5<br />[快速配置](./model-common-config#qwen35)|[阿里云](https://help.aliyun.com/zh/model-studio/vision)<br/>[OpenRouter](https://openrouter.ai/qwen)|⭐⭐⭐⭐<br/>运行时行为与 Qwen3-VL 一致 |
...
`deepThink` 参数用于控制模型在规划时是否启用深度思考。目前支持 `qwen3-vl`、`qwen3.5`、`doubao-seed-1.6`、`glm-v` 等系列模型。

启用后，你可以在报告中看到 `Reasoning` 栏目。开启后，规划耗时会增加，但结果会更准确。

**不同服务商的默认行为：**
- `qwen3-vl` / `qwen3.5` 在阿里云：默认关闭（更快，但准确性差）
