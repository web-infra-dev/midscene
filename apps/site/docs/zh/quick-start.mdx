...
> 想用其他模型（Doubao、GLM、Gemini、GPT-5……）？请查看[支持的模型与配置](./model-common-config)，其中列出了所有受支持的服务商。

下面无论选择哪条路径，都会复用这组配置。你可以把它粘贴到插件的设置里，或作为环境变量提供给 SDK。

全部配置项请参考[模型配置](./model-config)。

## 零代码体验（Chrome 插件） {#chrome-extension}
...
更多使用案例请参考[案例展示](./showcases)。

1. 前往 Chrome 扩展商店安装 [Midscene](https://chromewebstore.google.com/detail/midscene/gbldofcpkknbggpkmbdaefngejllnief)：
...
## Chrome 插件常见问题 {#chrome-extension-faq}

### 是否可以手动安装 Chrome 扩展？

如果无法访问 Chrome 扩展商店，可以从 [GitHub Releases 页面](https://github.com/web-infra-dev/midscene/releases) 下载安装包手动安装。但这种方式无法获得自动更新。
...
- **其他平台**：在 [Android](./platforms/android)、[iOS](./platforms/ios)、[HarmonyOS](./platforms/harmonyos) 或 [桌面端](./platforms/desktop) 上开始使用。
- **进一步了解**：通过[模型策略](./model-strategy)了解模型职责，或在 [API 参考](./reference/#common) 中查阅所有方法。
