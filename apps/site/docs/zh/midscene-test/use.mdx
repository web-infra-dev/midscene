...
所有平台都提供以下常用能力：`aiAct` 根据自然语言操作界面，`aiAssert` 检查预期结果，`aiWaitFor` 等待可观察条件成立，`wait` 等待指定时长。
...
使用可重复传入的 `--file` 运行一个或多个文件或目录。路径相对于测试项目根目录解析：
...
还可以通过用例 ID 或标签筛选。重复参数会组合生效：ID 精确匹配，包含标签使用 OR 关系，排除标签始终会移除对应的用例。在 CI 或调度器中持久化选择时应使用显式用例 ID；自动生成的 ID 会随用例在文件中的位置变化。
