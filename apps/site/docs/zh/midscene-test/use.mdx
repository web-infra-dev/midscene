...
### 生成当前 Project 的 Node 说明书

不同平台、Execution Project 和项目版本实际注册的 Node 可能不同。不要根据其他项目的示例推测当前项目的可用能力；`midscene-test nodes` 根据最终生效的注册结果生成 `midscene-node-reference.md`，它是当前 Project 可用 Node 名称、输入字段和字符串简写方式的事实来源。人类和 AI Agent 都应根据这份 Node 说明书编写 YAML 用例。
...
如果多个 Execution Project 的 Node 定义完全相同，可以生成一份共享说明书；定义不同时，命令会要求使用 `--project` 选择一个 Project，避免把不同平台的同名能力混在一起。
...
为某个 Execution Project（执行项目）生成 Node 说明书时，也可以指定 `--project`：
