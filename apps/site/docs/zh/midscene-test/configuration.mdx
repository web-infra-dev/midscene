...
| `executor` | 可选。将隔离后的单个用例委托给其他进程或服务。要求设置 `test.executionUnit: 'case'`。 |
...
`aiAssert`、`aiWaitFor`、`aiBoolean`、`aiNumber`、`aiString`、`aiAsk`、
...
`agentClass` 是 Agent Node 定义的唯一来源。如果该类没有提供 `getTestRunnerNodeDefinitions()`，工厂会在注册阶段抛出异常。传入 PlaywrightAgent、PlaywrightPageAgent、PlaywrightBrowserAgent、PuppeteerAgent、PuppeteerPageAgent、PuppeteerBrowserAgent、AndroidAgent、IOSAgent 或 HarmonyAgent 时，会同时注册通用 Node 定义和对应的平台 Node 定义。基础 Agent 和 ComputerAgent 仅提供通用 Agent Node。
...
Puppeteer 提供同名的 `gotoUrl`、`setCookies`、`clearCookies` 和 `setViewportSize` Node。安装 `puppeteer` 同级依赖，并把 `PuppeteerAgent`（或 `PuppeteerBrowserAgent`）传给 `createMidsceneNodes()`：
...
Playwright 和 Puppeteer 使用同一套 Web Node Schema 和执行实现，浏览器相关代码只保留 Driver 适配层。为了让 YAML 可以跨浏览器运行，`gotoUrl.waitUntil` 应使用 `domcontentloaded`、`load` 或 `networkidle`；Puppeteer 适配器会把 `networkidle` 映射为 `networkidle0`。现有的浏览器专用值继续兼容：Playwright 支持 `commit`，Puppeteer 支持 `networkidle0` 和 `networkidle2`；把这些值用于另一种 Driver 时会返回明确错误。两种接入的 Cookie 引用和结果脱敏规则也保持一致。相对 URL 会先基于 `testRunner.baseURL` 解析；没有配置时，再基于当前 HTTP(S) 页面地址解析。
...
export default defineTestProject<ProjectContext>({
  ...
  test: {
    executionUnit: 'project', // 使用 'case' 时逐个隔离并调度用例
    maxConcurrency: 1, // 同时运行的调度单元数量上限
    bail: 0,           // 大于 0 时，失败用例数达到此值后停止调度新任务
    testTimeout: 120_000,
    executorRetry: 0,  // 仅重试已分类的执行器故障，不重试用例失败
  },
  output: {
...
`test.executionUnit` 用于选择调度单元。默认值 `project` 保留共享项目生命周期：`setup` 只运行一次，项目内的 YAML 文件和用例顺序执行；`test.maxConcurrency` 限制同时运行的项目数。

设置 `executionUnit: 'case'` 后，每个用例会被单独隔离和调度。每个用例都有自己的项目 `setup`、文档 `beforeAll`/`afterAll` 和项目清理。`maxConcurrency` 限制同时运行的用例数。如果两个用例不能同时使用同一个账号、设备或其他独占资源，可以为它们声明相同的 `resources`：
...
拥有相同资源键的用例会相互等待，其他用例仍可并发。资源键由项目自行定义，只负责互斥，不会自动申请资源。

显式设置的用例 `id` 也会作为单层结果目录名，因此最长 128 个字符，以 ASCII 字母或数字开头，并且只能包含字母、数字、点、下划线和连字符。省略 `id` 时，Midscene 会根据 Project、源文件路径和用例位置生成安全、确定性的 ID。由于自动值包含用例位置，插入或调整用例顺序会改变它。在 CI 或外部调度器中持久化 `--case-id` 选择前，应先声明显式 `id`。

默认情况下，用例调度在同一个 Node.js 进程内异步运行。如需把隔离后的用例委托给其他进程、容器、设备服务或远程执行平台，可以配置自定义 [Test Executor](./extend#使用-test-executor-委托用例)。Midscene Test 不依赖任何特定的基础设施提供方。
