...
使用 `defineTestProject()` 定义并导出项目配置。Execution Project（执行项目）负责选择用例并提供运行环境。

Midscene Test 支持以下配置文件：

| 配置文件的选择方式 | 支持的文件 | 加载方式 |
| --- | --- | --- |
| 未指定配置路径，由 Midscene Test 自动查找 | `midscene.config.ts` 或 `midscene.config.mjs` | `.ts` 会先经过 TypeScript 转换；`.mjs` 由 Node.js 作为原生 ESM 直接加载 |
...
`createMidsceneNodes()` 返回两部分能力：`agentClass.getTestRunnerNodeDefinitions()` 声明的通用 Agent Node 和平台 Node，以及 Test 提供的 `wait` Node。通用能力包含 AI 操作、断言与信息提取、界面定位、报告记录等；平台 Agent 再追加浏览器或设备操作。完整名称和输入字段不在网站中维护重复清单，请以当前 Project 生成的 [Node Spec](./use#生成当前-project-的-node-spec) 为准。
...
注册后，可在 YAML 中调用这些 Node。参数写法见[其他 Node 的调用模式](./use#其他-node-的调用模式)，具体字段以项目生成的 Node Spec 为准。
...
| `projects[].files.order` | `sorted` 按路径排序；`listed` 保留 include 规则的声明顺序和重复文件调用。默认为 `sorted`。 |
| `projects[].tags` | 根据标签包含或排除用例。 |
| `projects[].variables` | 为 YAML 中的 `${variable}` 引用提供值。 |
| `projects[].retry` | 用例失败后的重试次数，默认为 `0`。 |
| `projects[].retryScope` | `case` 重试失败 Case，`document` 重试整个 YAML 文件。默认为 `case`。 |
| `projects[].fileConcurrency` | 当前 Project 内同时运行的 YAML 文件数，默认为 `1`。 |
| `projects[].setupFile` | 在主文件之前执行的一个前置 YAML 文件；失败时不再调度主文件。 |
| `projects[].documentSetup` | 每次文件执行（包括整文件重试）前创建文件独享的 context 和资源。 |
...
`setupFile` 必须匹配一个文件，并且不能同时出现在主文件列表中。它与 Project 的 TypeScript `setup` 不同：`setup` 创建运行时资源，`setupFile` 是先执行的一份 YAML 测试文档。

默认的 `retryScope: 'case'` 只重新执行失败 Case 的 `beforeEach → steps → afterEach`。设置为 `document` 后，会重新执行整个 YAML 文件，包括已通过的 Case、`beforeAll` 和 `afterAll`。如果正在从旧版 YAML 的整文件重试迁移，应使用 `document`。

开启 `fileConcurrency` 时，Project `setup` 返回的 context 仍由并发文件共享。需要为每个文件创建独立 Page 或 Agent 时，使用 `documentSetup` 从 Project context 派生文件 context，并通过它的 `onTeardown()` 注册文件级清理。设备不能安全并发时，应保持 `fileConcurrency: 1`，或拆分为连接不同设备的多个 Project。
...
### 为各 Project 生成 Node Spec

运行 `midscene-test nodes` 可以生成 Node Spec，查看项目中可用的 Node。使用 `--project` 指定执行项目后，Spec 会列出该项目合并全局和局部注册后实际生效的 Node：

```bash
pnpm exec midscene-test nodes --project android-smoke
```

未指定 Project 时，如果所有 Project 中生效的 Node 定义都相同，命令会生成一份共享 Spec。如果定义不同，命令会报错，并提示你通过 `--project` 选择项目，避免混合不同项目中的同名定义。

Spec 内容与通用命令参数见[生成当前 Project 的 Node Spec](./use#生成当前-project-的-node-spec)。
