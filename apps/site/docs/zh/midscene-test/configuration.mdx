...
- `context` 保存运行时资源。Node 通过回调获取其中的页面；首次需要 Agent 时，`getAgent` 创建实例并保存在 `context.agent` 中，供后续调用复用。
...
每个执行项目在执行 YAML 文件前运行一次 `setup`。`setup` 返回的 `context` 由该项目的所有 Node 共享。上面的示例将浏览器和页面保存在 `context` 中，Node 再通过 `getPage`、`getAgent` 等回调获取所需资源。
...
Android/iOS 的注册示例见[配置多个执行项目](#配置多个执行项目)。平台 Agent 工厂已包含相应的平台定义，注册时应避免重复添加同名 Node。
...
你可以从各平台对应的入口导入预置 Node 工厂函数。这些函数通过回调获取页面或 Agent 等运行时资源，因此你可以自行定义 `context` 的字段名。

注册后，可在 YAML 中调用这些 Node。参数写法见[其他 Node 的调用模式](./use#其他-node-的调用模式)，具体字段以项目生成的 Node 说明书为准。

### Playwright

使用 `createPlaywrightNodes()` 可以注册 `gotoUrl`、`setCookies`、`clearCookies` 和 `setViewportSize`。使用这些 Node 前，需要在项目中安装 `playwright`。它是 `@midscene/web` 的同级依赖（peer dependency）：

```bash
pnpm add -D playwright
```

以下示例沿用上文 Playwright 配置示例中的 `ProjectContext`。将返回的 Node 加入项目的 `nodes` 数组：
...
`gotoUrl` 的成功与否取决于导航是否完成，与 Playwright 的行为一致。即使服务器返回 HTTP 4xx 或 5xx 响应，只要导航完成，Node 就会成功并返回状态码，后续步骤可以继续检查错误页面。网络错误和导航超时仍会让 Node 失败。

### Android

使用下面的设备平台示例时，请在 `ProjectContext` 中声明对应平台类型的 `agent`，并在 `setup` 返回的对象中提供该实例。各平台示例独立使用，按需选择即可。

使用 `createAndroidNodes()` 可以注册 `launch`、`terminate`、`runAdbShell`、`back`、`home` 和 `recentApps`。传入的 Agent 需要提供这些 Node 对应的方法：
...
使用 `createIOSNodes()` 可以注册 `launch`、`terminate`、`runWdaRequest`、`home` 和 `appSwitcher`：
...
使用 `createHarmonyNodes()` 可以注册 `launch`、`terminate`、`runHdcShell`、`back`、`home` 和 `recentApps`：
