...
如果配置已构建为 JavaScript，可以通过 CLI 的 `--config` 参数或 `loadTestProject()` 传入 `.mjs` 文件路径。`.mjs` 会作为原生 ESM 加载，不经过 TypeScript 转换；默认的 `midscene.config.ts` 用法不变。
...
* **`loadTestProject()`**：异步加载 TypeScript（`.ts`）或 JavaScript ESM（`.mjs`）项目配置（Midscene Test 不支持同步加载）。
