...
两种浏览器接入使用相同的 Cookie 引用和结果脱敏规则。Puppeteer 的 `gotoUrl.waitUntil` 支持 `domcontentloaded`、`load`、`networkidle0` 和 `networkidle2`。相对 URL 会先基于 `testRunner.baseURL` 解析；没有配置时，再基于当前 HTTP(S) 页面地址解析。
