...
| `nodes` | 注册默认 Node 列表，仅在执行项目未配置 `nodes` 时继承。 |
...
顶层 `nodes` 提供默认 Node 列表，省略时默认为 `[]`。Project 只有在未配置 `nodes` 时才继承全局列表。一旦配置，Project 的 `nodes` 就会完整替换全局列表；`nodes: []` 表示该 Project 不使用任何 Node。需要复用共享 Node 时，应显式写入列表，例如 `nodes: [...sharedNodes, ...platformNodes]`。顶层列表或任一 Project 列表内出现重复名称时，框架会报错。
...
运行 `midscene-test nodes` 可以生成 Markdown 说明书，查看项目中可用的 Node。使用 `--project` 指定执行项目后，说明书会列出该项目实际生效的 Node：
