...
更多详情请参考 [Playwright FAQ — 浏览器界面持续闪动](./integrate-with-playwright#浏览器界面持续闪动)。

### 自定义网络超时

Midscene 在执行操作后会自动等待网络空闲，你可以自定义或关闭超时时间——详见 [Playwright FAQ — 自定义网络超时](./integrate-with-playwright#自定义网络超时)。

## 更多

- 更多 Agent 的 API 文档请参考 [API 参考](./reference/#interaction-methods)。
- Puppeteer 的 API 文档请参考 [Puppeteer Agent API](./reference/#puppeteer-agent)。
- 样例项目
