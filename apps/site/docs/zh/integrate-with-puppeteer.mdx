...
每个 Agent 实例与页面唯一绑定。Midscene 默认拦截新 tab(如 `target="_blank"` 链接)并改为当前页面打开。如需允许新标签页,设置 `forceSameTabNavigation` 构造选项为 `false`,并为每个新标签页创建新的 Agent。

完整构造函数签名请参阅 [API 参考](./api#puppeteeragent-web)。
...
- Puppeteer 的 API 文档请参考 [API 参考](./api#puppeteeragent-web)。
- 样例项目
