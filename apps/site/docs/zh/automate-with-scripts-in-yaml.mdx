...
Midscene 支持两种类型的 `.yaml` 文件：

### 1. 标准脚本文件

在标准脚本 `.yaml` 文件中，有两个部分：`web/android` 和 `tasks`。

`web/android` 部分定义了任务的基本信息，浏览器下的自动化使用 `web` 参数（曾用参数名 `target`），安卓设备下的自动化使用 `android` 参数，二者是互斥的。

### 2. 批量执行索引文件

对于更复杂的批量执行需求，Midscene 支持使用包含多个子 `.yaml` 文件的入口 `.yaml` 文件来定义执行计划。这种方式提供了并发执行和复用浏览器状态（比如 cookie）的能力。
...
- **order**: 必填，定义要执行的文件列表，支持 glob 模式匹配
...
1. **并发执行**: 可以同时执行多个 YAML 文件，提高执行效率
2. **复用浏览器状态**: 可以复用浏览器状态（比如 cookie），提高执行效率
3. **结果汇总**: 自动生成包含所有执行结果的汇总报告
4. **配置复用**: 全局配置可以被所有文件共享，减少重复配置
5. **执行计划**: 清晰的执行计划展示，包括文件数量、并发数、输出目录等信息

批量执行完成后，会在输出目录中生成：

- 汇总的 `index.json` 文件，包含所有文件的执行状态和统计信息
