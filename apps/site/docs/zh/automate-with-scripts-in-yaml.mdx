...
本文介绍旧版 `tasks/flow` YAML 字段。该格式仍然兼容，已有用例和配置可以不改文件，继续使用 `midscene`，或直接切换为 `midscene-test` 执行。

旧格式不建议用于新项目或新增用例。后续新增和重构建议采用 Midscene Test 的原生 `cases/steps`、生命周期和 Node。先阅读 [Midscene Test 概览](./midscene-test/overview)，再按照[从旧版 YAML 迁移](./midscene-test/migrate-from-yaml)切换入口或转换字段。
:::
