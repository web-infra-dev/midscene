...
对于接受 `string | Object` 提示词的步骤（参见 [API 参考](./api.html#prompting-with-images)），你可以提供一个对象，其中包含文本提示词以及 `images` 数组。适用于 `aiTap`、`aiHover`、`aiInput.locate`、`aiKeyboardPress.locate`、`aiScroll.locate`、`aiDoubleClick`、`aiRightClick`、`aiAsk`、`aiQuery`、`aiBoolean`、`aiNumber`、`aiString`、`aiAssert`。
