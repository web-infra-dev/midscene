...
对于提示词支持 `string | Object` 的步骤（参见 [API 参考](./api.html#prompting-with-images)），可以将原本的字符串替换为一个对象，该对象包含以下字段：

- `prompt`：发送给模型的文本描述。
- `images`（可选）：提示词中引用的参考图片，每一项需要提供 `name` 和 `url`。
- `convertHttpImage2Base64`（可选）：在图片链接无法公开访问时，将 HTTP 链接转换为 Base64 再发送给模型。

这适用于 `aiTap`、`aiHover`、`aiDoubleClick`、`aiRightClick` 的 `prompt` 参数、`aiInput`、`aiKeyboardPress`、`aiScroll` 的 `locate` 提示词，以及 `aiAsk`、`aiQuery`、`aiBoolean`、`aiNumber`、`aiString`、`aiAssert` 等需要提示词的能力。
