...
### 关闭 dotenv 的 debug 模式

默认情况下，Midscene 会打印 dotenv 的 debug 信息，如果你不想看到这些信息，你可以使用 `--dotenv-debug` 选项。
...
默认情况下，Midscene 会优先使用全局的环境变量，如果你希望使用 .env 中的环境变量覆盖全局的环境变量，你可以使用 `--dotenv-override` 选项。
