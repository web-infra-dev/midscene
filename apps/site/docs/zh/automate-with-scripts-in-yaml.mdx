...
YAML 脚本现在支持 `AndroidDevice` 构造函数的所有配置选项。完整的配置项列表请参考 [API 参考中的 AndroidDevice](./api#androiddevice)。
...
YAML 脚本现在支持 `IOSDevice` 构造函数的所有配置选项。完整的配置项列表请参考 [API 参考中的 IOSDevice](./api#iosdevice)。
