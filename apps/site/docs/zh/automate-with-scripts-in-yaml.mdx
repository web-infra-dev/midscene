...
agent:
  ...
  # aiContexts.aiAct 的已废弃兼容字段，可选
  aiActContext: <string>
  # 更早的已废弃字段 aiActionContext 仅为兼容而保留
...
- **aiContexts**：提供给 AI 的补充信息，例如业务事实、规则、约束或输出要求。如果配置了 `default`，仅当本次调用和对应 API 都没有自己的 context 时才使用它。单次调用的 `context` 覆盖 API 级值，API 级值覆盖 `default`，这些值不会自动合并；空字符串会显式清空继承的用户 context。
- **aiActContext**：`aiContexts.aiAct` 的已废弃兼容字段。两者同时存在时以 `aiContexts.aiAct` 为准；更早的 `aiActionContext` 字段也已废弃，仅为兼容而保留。
...
tasks:
  - name: <name>
    ...
    flow:
      ...
      # 等待指定的正数毫秒。在报告中记录 Sleep 步骤及请求时长和实际耗时，
      # 不调用模型，也不截图。
      - sleep: <ms>
