...
agent:
  ...
  # contexts.aiAct 的已废弃兼容字段，可选
  aiActContext: <string>
  # 更早的兼容字段 aiActionContext 仍可使用
...
- **contexts**：提供给 AI 的补充信息，例如业务事实、规则、约束或输出要求。仅当本次调用和对应 API 都没有自己的 context 时，才使用 `default`。单次调用的 `context` 覆盖 API 级值，API 级值覆盖 `default`，这些值不会自动拼接；空字符串会显式清空继承的用户 context。
- **aiActContext**：`contexts.aiAct` 的已废弃兼容字段。两者同时存在时以 `contexts.aiAct` 为准；更早的 `aiActionContext` 字段也继续兼容。
