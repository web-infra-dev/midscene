...
- **MCP**：允许其他 MCP Client 直接使用 Midscene 的能力，覆盖桥接模式与 Android 设备。详见 [MCP 服务指南](./mcp)。
...
- **[Android Playground](./android-playground)**：使用 Android playground 来控制你的本地 Android 设备。
- **[iOS Playground](./ios-playground)**：使用 iOS playground 来控制你的本地 iOS 设备。

## 模型选择

Midscene 推荐使用视觉模型进行 UI 自动化，以提升任务的成功率，帮助开发者快速完成调试。

- 视觉方案：优先使用具备强大 UI 理解能力的视觉模型，以获得更高的定位和操作准确度。
- 已支持模型：Doubao Seed（视觉模型）、Qwen3-VL、Gemini-3-ProPro 等
- 多模型组合：可在任务规划与交互阶段分别配置不同模型，支持商用与开源模型搭配使用，进一步提升完成率。详见[多模型配合](./model-strategy#高阶特性多模型配合)。

请阅读[模型策略](./model-strategy)了解更多关于模型选择的信息。
...
* **调试体验**: 开始使用 UI 自动化时，你会发现调试和维护自动化脚本才是真正的痛点。无论模型多么强大，你仍然需要投入调试过程以确保其保持长期稳定。Midscene.js 提供了可视化报告、内置的 Playground 和 Chrome 插件，以调试整个运行过程。这是大多数开发者真正需要的特性，我们也在持续努力改进调试体验。
