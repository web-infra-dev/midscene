...
传统 UI 测试依赖选择器和 DOM，界面一重构就会失效，而且根本触及不到 `<canvas>`、原生移动应用或跨域 iframe。让一套用例长期保持通过，往往变成持续的维护负担。

Midscene 走了一条不同的路：它**仅凭截图**、借助视觉语言模型来定位元素，你只需用自然语言描述每一步。这会改变 UI 测试的体验：

- **不再因每次重构而失效。** 标记或样式变化时无需再追着改选择器，用例的维护成本大幅下降。
- **凡所见皆可测。** Web、Android、iOS、HarmonyOS、桌面端——一套 API，处处一致，包括 DOM 无法描述的画面。
