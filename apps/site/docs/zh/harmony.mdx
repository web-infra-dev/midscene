...
当你需要自定义设备行为、把 Midscene 接入独立框架，或排查 HDC 问题时，可参考本节内容。更多构造函数参数可前往 [API 参考](./api#harmonyagent)。
...
Midscene 在输入文本后会自动隐藏键盘。HarmonyOS 默认发送 ESC 以避免触发页面返回。如果 ESC 无法关闭键盘，可设置 `keyboardDismissStrategy: 'back-first'`。如果输入框监听了 Back 并执行清空操作，可设置 `autoDismissKeyboard: false` 关闭自动隐藏，改用 `aiAct` 手动收起键盘。

完整构造函数签名请参阅 [API 参考](./api#harmonydevice)。

### 如何使用自定义的 HDC 路径？

通过 `HDC_HOME` 环境变量指定 HDC 所在目录，或通过 `HarmonyDevice` 构造函数传入 `hdcPath`。详见 [API 参考](./api#harmonydevice)。

## 更多

- 查看所有 Agent 方法：[API 参考（通用）](./api#interaction-methods)
- HarmonyOS 专属参数与接口：[API 参考](./api#harmonyagent)
- 示例项目
