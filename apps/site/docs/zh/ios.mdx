...
Midscene 通过 WebDriverAgent 连接 iOS 设备。
...
需要查看构造函数、辅助方法或平台专属设备 API？请移步 [API 参考](./api#iosagent) 获取详细参数及自定义操作等高级主题。若想了解跨平台共用的 API，可阅读 [通用 API 参考](./api)。
...
- iOS 专属参数与接口：[API 参考](./api#iosagent)
- 示例项目
