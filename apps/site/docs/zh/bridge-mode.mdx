...
- **模型配置（如 `MIDSCENE_MODEL_API_KEY`）应该配置在浏览器还是终端？**
  ...
  使用桥接模式时，请在终端（Node.js 环境）中配置模型参数。支持的模型和配置示例请参考[支持的模型与配置](./model-common-config)。

## 更多

- 更多 Agent 的 API 请参考 [API 参考](./reference/#interaction-methods)。
- 完整的 Chrome 桥接 API 可参阅 [API 参考（Web）](./reference/#chrome-bridge-agent)。
- 样例项目
