...
桥接模式复用桌面浏览器配置,以下构造选项将被忽略:`userAgent`、`viewportWidth`、`viewportHeight`、`deviceScaleFactor`、`waitForNetworkIdle`、`cookie`、`extraHTTPHeaders`、`downloadPath`、`chromeArgs`。

完整说明请参阅 [API 参考](./api#agentoverchromebridge-web)。

## 远程访问配置

默认情况下,Bridge Server 只监听 `127.0.0.1`。如需跨机器通信,可通过 `allowRemoteAccess`、`host`、`port` 构造选项配置：
...
完整构造函数签名请参阅 [API 参考](./api#agentoverchromebridge-web)。
...
- 完整的 Chrome 桥接 API 可参阅 [API 参考](./api#agentoverchromebridge-web)。
- 样例项目
