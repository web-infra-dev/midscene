...
1. 配置 OpenAI API Key，或参考[模型配置](./model-config)进行自定义
...
你需要使用 `AgentOverChromeBridge` 这个构造器。它继承了[通用 Agent 构造器](./api.mdx) ，并接受以下构造参数：
...
除了 [通用的 Agent 接口](./api.mdx)，`AgentOverChromeBridge` 还提供了一些额外的接口来控制桌面 Chrome。
