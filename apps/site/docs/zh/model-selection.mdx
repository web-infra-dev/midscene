...
:::info 快速开始
如果你想快速开始体验 Midscene，请选择以下任意一个模型并进行配置：
参见 [模型配置 - 快速接入](./model-config#快速接入) 章节完成所需的环境配置。
:::

在 Midscene 中，我们将模型的使用意图（Intent）分为两种，*Planning* (操作规划) 与 *Insight* (洞察)。

开发者必须选择一个默认模型用于 *Planning* 意图。*Insight* 意图会默认复用 *Planning* 的模型，无需特殊配置。

本篇文档会重点介绍 Midscene 的模型选用策略。如果你需要进行模型配置，请参考 [模型配置](./model-config)。

## *Planning* 意图，选用视觉驱动方案

Midscene 把 UI 操作相关的任务定义为 *Planning（规划）* 意图，这是 Midscene 开发者首要关心的模型选型任务。
...
Midscene 提供了基于页面理解的数据处理接口，如 AI 断言（`aiAssert`）、数据提取（`aiQuery`，`aiAsk`） 等，我们把这类意图归类为 *Insight*。

在默认情况下，Midscene 会复用 *Planning* 模型来完成 *Insight* 类的工作，开发者无需额外配置，这个策略可以满足绝大多数项目。

若开发者对 Insight 的准确性、细节理解力有特殊要求，也可以为 Insight 意图额外增加一个专用模型。此时你可以选用任意接受图片输入的多模态模型来承接，如 `GPT-5`,`Gemini-2.5-pro` 等。一般来说，在视觉问答（VQA）领域测评分数更高的模型会获得更好的表现。
