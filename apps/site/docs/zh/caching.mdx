...
想要启用缓存特性，需要在创建 Agent 时配置 `cache` 选项，缓存配置支持以下模式：

1. **禁用缓存**: `cache: false` 
2. **读写模式**: `cache: { id: "my-cache-id" }` - 自动读取和写入缓存
3. **只读模式**: `cache: { strategy: "read-only", id: "my-cache-id" }` - 只读取缓存，需要手动调用 `agent.flushCache()` 写入

### Playwright

在 Playwright 模式下，需要在测试中配置 cache 选项：
...
// 只读模式
const agent = new PuppeteerAgent(page, {
  cache: { strategy: "read-only", id: "login-test" }
});
...
**向后兼容**: 仍支持旧的 `cacheId` + `MIDSCENE_CACHE=1` 环境变量方式：
...
在 Yaml 模式下，仍需要设置 `MIDSCENE_CACHE=1` 环境变量，`cacheId` 会自动设置为 yaml 文件名。
...
注意：Yaml 模式暂时仍使用旧的缓存配置方式，未来版本将支持新的配置格式。
