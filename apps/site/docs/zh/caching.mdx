...
内部实现上，原生平台适配器会把 UI 树转换成统一的 `UiNode` 结构，包含 `type`、`attrs`、`bounds` 和 `children`。缓存生成时先选择定位点上面积更小、层级更深的节点，再把唯一的稳定属性或语义属性记录为目标身份。位置型 XPath 可以作为兜底候选，但回放时仍必须通过目标身份校验。如果节点没有唯一的稳定属性或语义属性，Midscene 会跳过本次原生元素缓存。未命中、零尺寸、命中多个节点或目标身份不一致时，都会回退到 AI 定位。
