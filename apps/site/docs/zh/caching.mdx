...
Midscene 会在 Agent 销毁时自动清理未使用的缓存记录，确保缓存文件保持精简。

### 自动清理机制

当调用 `agent.destroy()` 时，系统会：
...
根据不同的使用方式，缓存清理的触发时机有所不同：

**Playwright AI Fixture**：自动清理
- 使用 `PlaywrightAiFixture` 时，当页面关闭时会自动调用 `destroy()`
- 用户无需手动管理，缓存会在测试结束时自动清理

**直接创建 PuppeteerAgent**：需要手动清理
- 使用 `new PuppeteerAgent(page, {...})` 时，需要手动调用 `agent.destroy()` 来触发缓存清理
- 建议在测试的 `afterEach` 中统一调用：
...
**YAML 脚本模式**：自动清理
- 使用 YAML 脚本时，系统会在脚本执行完毕后自动调用 `destroy()`

### 清理行为说明

- **read-write 模式**：自动清理并写入文件
- **read-only 模式**：清理但需要手动调用 `flushCache()` 持久化
