...
内部实现上，各平台适配器会把原生树转换成统一的 `UiNode` 结构，包含 `type`、`attrs`、`bounds` 和 `children`。缓存生成时会优先选择稳定属性，其次选择语义文本属性，最后使用位置型 XPath 作为兜底。缓存回放只有在某条 XPath 命中唯一且非零尺寸的 rect 时才算命中；未命中、零尺寸或命中多个节点都会被视为过期缓存，并回退到 AI 定位。
...
1. 相同 XPath 下的元素已经无法匹配缓存元素。
2. 页面 DOM 或原生 UI 树结构与缓存时的结构不同。
3. XPath 命中多个元素，因为有歧义的匹配会被视为过期缓存。
...
对于 Android、iOS、HarmonyOS 和 macOS 桌面端自动化，元素定位缓存依赖平台 accessibility 或 UI inspector tree。纯截图像素、没有暴露到该树的视图，或没有暴露可用 macOS accessibility window tree 的桌面应用，都无法使用元素定位缓存。
