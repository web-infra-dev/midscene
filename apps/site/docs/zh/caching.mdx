...
Midscene 会在运行结束时自动清理未使用的缓存记录，确保缓存文件保持精简。

### 自动清理机制

当 Agent 销毁时（通常在测试用例结束或脚本执行完毕），系统会：
...
这意味着：
- 当你删除测试步骤时，对应的缓存会自动清理
- 当你修改测试流程时，不再使用的缓存会被移除
- 缓存文件不会无限增长

### 不同模式的清理行为

- **read-write 模式**：自动清理并写入文件
- **read-only 模式**：在内存中清理，需手动调用 `agent.flushCache()` 持久化
- **write-only 模式**：不执行清理（因为不读取缓存）

### 手动清理缓存

如果需要完全清空缓存，可以直接删除缓存文件：
