...
自动读取已有缓存，执行过程中自动更新缓存文件。这是推荐的使用方式，适合开发和测试阶段。

```javascript
// 直接创建 Agent - 显式设置 cache ID（默认为 read-write 策略）
const agent = new PuppeteerAgent(page, {
  cache: { id: "my-cache-id" },
});

// 或者显式指定 strategy（效果相同）
const agent = new PuppeteerAgent(page, {
...
```yaml
# YAML 配置 - 显式设置 cache ID（默认为 read-write 策略）
agent:
  cache:
    id: "my-cache-test"
...
只读取缓存，不自动写入缓存文件，需要手动调用 `agent.flushCache()` 写入缓存文件。适合生产环境或 CI/CD 场景，确保缓存的一致性和可控性。
