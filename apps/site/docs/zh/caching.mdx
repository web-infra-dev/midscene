...
对于 Android、iOS、HarmonyOS 和 macOS 桌面端自动化，元素定位缓存依赖平台 accessibility 或 UI inspector tree。纯截图像素、没有暴露到该树的视图、没有唯一稳定属性或语义身份的节点，或没有暴露可用 macOS accessibility window tree 的桌面应用，都无法使用元素定位缓存。仅有位置型 XPath 不足以判定原生缓存命中。
