...
更准确地说，规划缓存是在**机械性重放上一次实际走过的 flow**。它不会重新理解 prompt，也不会在命中缓存时重新做条件判断；只有当本次运行的初始状态仍然适合执行上次记录下来的步骤时，缓存才有意义。

因此，请把规划缓存用于稳定、可重复的操作路径。对于包含条件分支的 prompt，例如“如果出现弹窗，就先关闭弹窗，再提交表单”，上一次运行只会实际走过其中一个分支，另一个分支并没有被记录到缓存里。下一次如果页面进入了另一个分支，Midscene 没有可以重放的步骤，缓存就无法覆盖这个分支，只能回退到 AI 重新规划。更推荐把条件判断放在测试代码里，用 `aiBoolean` 等查询类 API 先判断状态，再分别执行更稳定的、可缓存的操作步骤。
...
启用缓存后，Midscene 默认使用 AI 验证命中缓存的 Tap 是否确实生效。缓存仍然复用已有定位结果或规划步骤，但只有在验证器确认操作前后的可见效果后，才会继续信任缓存。当前实现优先向模型发送一张聚焦区域的前后对比图；只有结果为 uncertain 时，才会回退为两张完整截图。

如需关闭验证并避免这次额外的 AI 调用，可设置 `verify: false`：
...
目前仅支持紧随 Tap 操作后的效果验证。其他操作类型保持原有缓存行为。
...
### 包含“如果 xx，就 yy”的条件流程可以缓存吗？

不适合把这种条件流程作为一条整体 prompt 依赖缓存。规划缓存记录的是上一次实际执行过的 flow，而不是所有可能分支的决策树。如果上一次走的是“出现弹窗并关闭”的分支，缓存里就没有“没有弹窗时直接继续”的分支；反过来也一样。

如果需要处理条件分支，建议先用测试代码或 `aiBoolean` 这类不会被缓存的查询 API 判断当前状态，再对每个分支里的稳定操作分别调用 `aiAct`、`aiTap` 等可缓存 API。这样缓存命中的是已经实际发生并记录过的具体操作路径。
