...
- `opts?: AndroidDeviceOpt` - 可选参数，用于初始化 AndroidDevice 的配置
  ...
  - `minScreenshotBufferSize?: number` - 可选参数，用于指定截图 buffer 大小校验阈值，单位为字节；低于该值的 buffer 会被视为截图采集失败或已损坏。默认值为 `1024`（1KB）。设置为 `0` 仅跳过此大小校验；Midscene 仍会拒绝空 buffer 和无效图片格式。
...
> 注意：该方法本质上已经封装了 `adb shell`，所以传入的内容只需要包含 shell 命令本身。例如传 `input tap 100 200`，不要传 `adb shell input tap 100 200`。
...
- 参数：
  ...
  - `command: string` - 要执行的 adb shell 命令，不需要包含 `adb shell` 前缀
...
// 等同于执行 `adb shell dumpsys battery`，并设置 60 秒超时
console.log(result);

await agent.runAdbShell('input tap 100 200');
// 等同于执行 `adb shell input tap 100 200`
```
...
const device = new AndroidDevice('s4ey59', {
  ...
  // 要连接的 adb server 地址(等价于 `adb -H <host> -P <port>`)，不是设备的 adbd，也不是 `adb connect <ip:port>` 里的设备地址
  remoteAdbHost: '192.168.1.100',
  remoteAdbPort: 5037,
});
```

> 云真机 / 远程设备只需先 `adb connect <ip:port>` 挂到本地 adb server，再走正常的 `getConnectedDevices()` 流程即可；只有当设备是通过独立的远端 adb server 暴露时，才需要设置 `remoteAdbHost` / `remoteAdbPort`。
