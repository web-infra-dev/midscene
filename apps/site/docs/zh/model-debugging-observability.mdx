...
# MIDSCENE_MODEL_API_TYPE="responses" # 使用 Responses API 时取消本行注释；默认使用 Chat API。
...
确认 `MIDSCENE_MODEL_BASE_URL` 指向服务商的 API 接入地址。该地址通常以 `/v1` 等版本号结尾。请勿添加 `/chat/completions` 或 `/responses`，底层 SDK 会根据所选 API 类型自动添加请求路径。
