...
## 通过 macOS 上的 VNC 客户端控制远程桌面 {#vnc-keyboard}

`@midscene/computer` 不会直接连接 VNC（Virtual Network Computing，虚拟网络计算）服务器。请在运行 Midscene 的 Mac 上打开 VNC 客户端，并连接远程桌面。操作期间，VNC 客户端窗口需要保持在前台。Midscene 会把它作为本地桌面应用进行控制。

### 保留修饰键

VNC 客户端可能无法转发默认的 macOS AppleScript 修饰键事件。此时，大写字母会变成小写。包含 `Shift` 或 `Control` 的快捷键也可能失效。

请将 `keyboardEventMode` 设置为 `physical`。Midscene 会为每个修饰键发送明确的按下和释放事件。文本也需要逐键输入。你可以将 `inputStrategy` 设置为 `sequential`，也可以在默认的 `legacy` 输入策略中设置正数 `keyboardTypeDelay`。
