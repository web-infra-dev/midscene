...
## 通过桌面客户端转发键盘输入 {#keyboard-event-mode}

`@midscene/computer` 会控制 Midscene 所在电脑的前台应用。这个应用可以是 VNC、TeamViewer 等远程控制客户端，也可以是虚拟机控制台。请先打开客户端并建立连接，再让客户端保持在前台。Midscene 会把它作为本地桌面应用进行控制。Midscene 不会直接连接这些产品或协议。

### 调整本机 libnut 的修饰键时序

Windows 和 Linux 的本机桌面控制使用 libnut 键盘驱动。macOS 设置 `keyboardDriver: 'libnut'` 后，也会进入这条输入路径。

部分远程控制客户端在全屏模式下会单独读取修饰键状态。紧凑的 `Control+S` 事件可能只剩下 `s`。文本输入也可能出现相同问题。例如，大写 `K` 丢失隐含的 `Shift` 后会变成 `k`，`!` 则会变成 `1`。

请设置正数 `keyboardShortcutDelay`，让 Midscene 分阶段发送修饰键。发送顺序是：按下修饰键、等待、发送主键、等待、释放修饰键。逐键输入文本时，大写字母和 en-US 键盘中的 Shift 标点也会使用这套时序。
...
- `keyboardTypeDelay` 控制相邻文本字符之间的等待时间。
- `keyboardShortcutDelay` 控制修饰键状态变化前后的等待时间。它也会作用于 `Control+S` 等组合键。

`keyboardShortcutDelay` 的默认值是 `0`。此时，libnut 会沿用原来的一次调用方式。只有当前台应用丢失修饰键时，才建议从 `50` 开始测试。这个选项只对本机 libnut 驱动生效。直接使用 RDP 或 macOS AppleScript 驱动时，它不会生效。

:::warning
Shift 标点的转换以 en-US 键盘布局为准。使用其他布局时，请分别验证本机和远端系统中的大写字母与标点。
:::

### 选择 macOS 修饰键的发送方式

macOS AppleScript 键盘后端提供两种事件模式。两种模式都调用 AppleScript `System Events`。`physical` 表示显式的按键状态变化，不表示硬件级输入，也不会发送 HID 扫描码。

| 模式 | AppleScript 形式 | 适用场景 |
| --- | --- | --- |
| `logical`（默认） | 一条 `keystroke ... using` 命令 | 原生应用，或者能够保留修饰键状态的客户端 |
| `physical` | 分别发送 `key down`、主键和 `key up` 命令 | 需要分别观察修饰键状态变化的前台应用 |

部分远程控制客户端和虚拟机控制台无法保留 `keystroke ... using` 形式中的修饰键。此时，大写字母会变成小写。包含 `Shift` 或 `Control` 的快捷键也可能失效。这个兼容性问题不限于 VNC。

如果遇到这个问题，请将 `keyboardEventMode` 设置为 `physical`。Midscene 会为每个修饰键发送明确的按下和释放事件。文本也需要逐键输入。你可以将 `inputStrategy` 设置为 `sequential`，也可以在默认的 `legacy` 输入策略中设置正数 `keyboardTypeDelay`。
...
`keyboardEventMode` 的默认值是 `logical`。这个模式保留标准的 AppleScript 行为。

`keyboardShortcutDelay` 不影响 AppleScript 输入路径。如果 AppleScript 控制的前台应用需要显式的修饰键状态变化，请使用 `keyboardEventMode: 'physical'`。

:::warning
`physical` 是可选的兼容模式。请根据前台应用、远程或虚拟系统和键盘布局进行验证。当前测试覆盖 AppleScript 生成结果和集成行为，没有覆盖所有远程控制产品和虚拟机控制台。

仅当本地和远程系统使用相同的 en-US 键盘布局时，才使用 `physical`。这个模式会把 Shift 标点转换为基础键和 `Shift`。因此，原生 macOS 应用可能把 `!@#` 输入为 `123`。

这个选项在非 macOS 系统中无效。使用 `keyboardDriver: 'libnut'` 时，它也不会生效。
:::
...
请使用 `Control+S`，不要使用 `Control + S`。如果目标已经获得焦点，请把第一个参数设置为 `undefined`。这样可以避免额外点击改变当前选区或光标位置。
...
> 仅当光标已经到达正确位置时，才需要排查管理员权限。Midscene 连接 Windows 时会执行健康检查。如果 Midscene 未以管理员身份运行，健康检查日志会显示本节链接。
