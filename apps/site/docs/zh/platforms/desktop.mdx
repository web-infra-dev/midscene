...
## 通过 VNC 客户端控制远程桌面 {#vnc-keyboard}

`@midscene/computer` 不会直接连接 VNC（Virtual Network Computing，虚拟网络计算）服务器。请在运行 Midscene 的电脑上打开 VNC 客户端，并通过客户端连接远程桌面。操作期间，VNC 客户端需要保持在前台。Midscene 会把它作为本地桌面应用进行控制。这个流程不限于 macOS。

### 在 macOS 上转发修饰键

部分 VNC 环境无法保留标准 macOS AppleScript `keystroke ... using` 形式中的修饰键。此时，大写字母会变成小写。包含 `Shift` 或 `Control` 的快捷键也可能失效。

现有证据只能确认兼容性问题。它没有包含 macOS 底层输入事件或 RFB 报文。因此，目前无法定位修饰键丢失的环节。
...
`keyboardEventMode` 的默认值是 `logical`。这个模式保留标准的 AppleScript 行为，适合原生 macOS 应用。

:::warning
`physical` 是针对兼容性问题的可选模式。请根据实际使用的 VNC 客户端、VNC 服务器和键盘布局进行验证。当前测试覆盖 AppleScript 生成结果和集成行为，没有覆盖真实 VNC 会话。
