...
yadb 工具通过 `SurfaceControl.createDisplay` 的 `secure` 参数创建虚拟显示器，在 Android 11 及以下系统上可以直接截取安全页面内容。在 Android 12 及以上系统中，由于系统限制了该参数，需要配合 root 和 Magisk hook 才能绕过 `FLAG_SECURE`。
...
默认值为 `auto`，即按 scrcpy → `adb.takeScreenshot` → `screencap` 的顺序尝试。只有 `screencap` 命令本身执行失败时，才会改用 yadb；`auto` 不会识别黑屏。安全页面生成有效但全黑的图片时，请设置 `always-yadb`，跳过前面的路径并直接使用 yadb 截图。yadb 只能截取默认显示器（`displayId=0`）；如果同时设置 `always-yadb` 和非零 `displayId`，Midscene 会抛出错误。
