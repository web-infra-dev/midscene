...
- `deviceId: string` - 设备 device Id 或模拟器标识符
...
- 参数：
  ...
  - `deviceId?: string` - 可选参数，要连接的 iOS 设备 ID，如果未传入，则使用第一个连接的设备
...
const agent = await agentFromIOSDevice('00008120-001234567890123E'); // 传入 deviceId
const agent = await agentFromIOSDevice(); // 不传入 deviceId，则使用第一个连接的设备
```
...
interface Device {
  /**
   * The device deviceId or simulator identifier.
   */
  deviceId: string;
  /**
   * Device name
...
const page = new IOSDevice('your-device-id');
const agent = new IOSAgent(page, {
