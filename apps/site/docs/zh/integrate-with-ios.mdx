...
2. **构建并部署到设备**：
   ```bash
   # 将 YOUR_DEVICE_UDID 替换为你的设备 UDID
   xcodebuild -project WebDriverAgent.xcodeproj \
             -scheme WebDriverAgentRunner \
...
Promise.resolve(
  (async () => {
    ...
    // 👀 启动 Safari 并导航到 ebay.com
    await page.launch('com.apple.mobilesafari');
    await sleep(3000);

    // 👀 输入关键词，执行搜索
    await agent.aiAction('导航到 ebay.com 并搜索 "Headphones"');
...
- `opts?: IOSDeviceOpt` - 可选参数，用于初始化 IOSDevice 的配置
  ...
  - `keyboardDismissStrategy?: 'done-first' | 'escape-first'` - 可选参数，关闭键盘的策略。'done-first' 优先尝试 Done 键，如果键盘仍存在则尝试 Escape 键。'escape-first' 优先尝试 Escape 键，如果键盘仍存在则尝试 Done 键。默认值为 'done-first'。
...
- 参数：
  ...
  - `bundleId: string` - 要启动的 iOS 应用包标识符
...
await agent.launch('com.apple.camera'); // 启动相机应用
```

### `agentFromIOSDevice()`

从已连接的 iOS 设备中，创建一个 IOSAgent。
...
- 参数：
  ...
  - `udid?: string` - 可选参数，要连接的 iOS 设备 UDID，如果未传入，则使用第一个连接的设备
...
const agent = await agentFromIOSDevice('00008120-001234567890123E'); // 传入 UDID
const agent = await agentFromIOSDevice(); // 不传入 UDID，则使用第一个连接的设备
```
...
你可以通过 IOSDevice 的构造函数来指定 WebDriverAgent 的端口和主机：
