...
- **midscene_android_connect**：连接到指定的 Android 设备
  ...
  - activeDisplayId：（可选）多屏 Android 设备的显示屏 ID（如 0、1、2）。
    当指定时，所有 ADB 输入操作将针对此特定显示屏。
  ```
...
Midscene MCP 支持具有多个显示屏的 Android 设备，如折叠屏手机或带有外接显示器的设备。

### 使用 activeDisplayId

当连接到具有多个显示屏的 Android 设备时，可以指定要控制的显示屏：
...
- `activeDisplayId`：指定要控制的显示屏 ID（0、1、2 等）
- 所有输入操作（点击、滑动、键盘）都将指向指定的显示屏
- 截图将捕获指定的显示屏

### 显示屏 ID 格式

Android 使用两种类型的显示屏 ID：

- **短显示屏 ID**：简单的数字，如 0、1、2（用于输入操作）
- **长显示屏 ID**：复杂的标识符，如 4630946423637606531（用于某些系统操作）

### 高级选项

- `useLongDisplayIdForScreenshot`：控制截图是否使用长或短显示屏 ID 格式
- `useLongDisplayIdForDisplayLookup`：控制显示信息查询是否使用长或短 ID 格式
