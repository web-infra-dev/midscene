...
使用 `@midscene/test` 时，在 `midscene.config.ts` 的 `setup` 中创建 Agent，并通过 `aiContexts.aiAct` 传入应用知识，再将该 Agent 提供给 `createMidsceneNodes`。下面沿用 Android 示例，将 `deviceId` 替换为已连接的设备 ID，并确保 App 已打开到首页：
...
对应的 Case YAML 只需描述操作目标；使用这个 Agent 的 `aiAct` 步骤都会获得上面配置的应用知识：
...
cases:
  - name: 筛选降价榜
    steps:
      - aiAct: 进入降价榜
      - aiAct: 筛选近一年上市的新能源 MPV，并应用 10–40 万元自定义价格
```

`aiActContext` 是 `aiContexts.aiAct` 的旧名称，仍受支持，新配置请使用 `aiContexts.aiAct`。这里配置的是 Agent 的默认上下文，不是 Case YAML 的顶层字段；`setup` 返回的项目 `context` 则用于在 Node 之间共享 Agent 等资源，不会自动作为文本发送给模型。项目配置方式见 [Midscene Test 配置](./midscene-test/configuration)。
...
在 `@midscene/test` 的 Case YAML 中，使用 `aiAct` 的对象写法，通过 `options.context` 传入单次调用的上下文：

```yaml
- aiAct:
    prompt: 将价格范围设为 10–40 万元并应用筛选
    options:
      context: |
        自定义价格支持点击刻度设置，先设置上限，再设置下限。
        调整后需要点击面板底部的结果按钮才会应用。
