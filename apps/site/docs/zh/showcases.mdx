...
:::info Doubao 计价口径
价格数据截至 2026 年 9 月 3 日，豆包费用使用[火山引擎官方价格](https://ark.volcengine.com/region:cn-beijing/model/detail?name=doubao-seed-2-1-turbo)计算。推理输入按 `¥3 / M Tokens`、缓存命中按 `¥0.6 / M Tokens`、推理输出按 `¥15 / M Tokens` 计算。5 个用例总费用约 ¥1.556，平均消耗约 182,253 Token，平均费用约 ¥0.311。以上数据仅代表本次测试，实际费用会随任务复杂度、缓存命中率和模型价格变化。
:::
