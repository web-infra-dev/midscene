...
- `keyboardTypeDelay: number` —— 透传给 Puppeteer `page.keyboard.type` 的每字符延迟（毫秒）。未设置时 Midscene 不传该选项，Puppeteer 会立即输入（`delay` 默认 `0`）。当受控输入框在快速输入下出现丢字时调大（例如 `80`）；追求最快输入则设为 `0`。
...
- `waitForNetworkIdleTimeout: number` —— 每次操作后等待网络空闲的时间，默认 `2000`（设为 `0` 关闭）。
- `enableTouchEventsInActionSpace: boolean` —— 在动作空间里增加触摸手势（如滑动），用于需要触摸事件的页面，默认 `false`。
- `keyboardTypeDelay: number` —— 透传给 Playwright `page.keyboard.type` 的每字符延迟（毫秒）。未设置时 Midscene 不传该选项，Playwright 会立即输入（`delay` 默认 `0`）。当受控输入框在快速输入下出现丢字时调大（例如 `80`）；追求最快输入则设为 `0`。
