...
- `screenshot?(): Promise<{ bytes: Uint8Array; format: 'png' | 'jpeg' | 'webp' }>`：返回原始编码字节及其格式。提供此方法后，Midscene 会优先使用它，避免 Base64 往返转换。仍需保留 `screenshotBase64()` 以兼容原有调用。字节内容必须与声明的格式一致，返回后不得修改。
