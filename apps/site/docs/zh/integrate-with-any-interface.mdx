...
为 Agent 启动一个 MCP 服务，即可在 Claude Desktop、Cursor 等 MCP 客户端中使用你的自定义界面。
...
// stdio 模式（用于 Claude Desktop、Cursor 等客户端）
await mcpServerForAgent(agent).launch();
```

你也可以启动 HTTP 模式的 MCP 服务：

```ts
await mcpServerForAgent(agent).launchHttp({ port: 3000 });
```

在 Claude Desktop 中配置此 MCP 服务（`claude_desktop_config.json`）：
...
如果你只需要提取工具定义（用于集成到其他 MCP 服务中），可以使用 `mcpKitForAgent`：

```ts
import { mcpKitForAgent } from '@midscene/core/mcp';

const { description, tools } = await mcpKitForAgent(agent);
// tools 是一个 ToolDefinition 数组，可以注册到任意 MCP 服务器
```

### 步骤 5. 添加 Skill 支持（可选）

[Agent Skills](https://github.com/anthropics/skills) 让 AI 编程助手（如 Claude Code、Cline 等）可以通过 CLI 命令驱动你的自定义界面，无需配置 MCP 服务。了解更多请参考 [Skills 文档](./skills)。
...
### 步骤 7. 在 Midscene CLI 和 YAML 脚本中调用你的类
