...
从 Midscene v0.28.0 开始，我们推出了与任意界面集成的功能。通过继承 `AbstractInterface` 类并告诉 Midscene 如何与界面交互，就可以创建一个功能齐全的 Midscene AI Agent。你会默认拥有这些工具：交互方法、数据提取方法、报告、GUI Playground、CLI 命令行工具、yaml 脚本运行器、MCP 服务器等。
