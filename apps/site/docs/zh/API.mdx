...
在 Android 中，也有一个额外的参数：

* `autoDismissKeyboard: boolean`: 如果为 true，则键盘会在输入文本后自动关闭。默认值为 true。
...
* 参数：
  ...
  * `options?: Object` - 可选，一个配置对象，包含：
    * `domIncluded?: boolean` - 是否向模型发送精简后的 DOM 信息，一般用于提取 UI 中不可见的属性，比如图片的链接。默认值为 False
...
const dataD = await agent.aiQuery('{name: string, age: string, avatarUrl: string}[], 表格中的数据记录', { domIncluded: true });
```
...
// 使用 domIncluded 功能提取 UI 中不可见的属性
const boolB = await agent.aiBoolean('忘记密码按钮是否存在链接', { domIncluded: true });
```
...
// 使用 domIncluded 功能提取 UI 中不可见的属性
const numberB = await agent.aiNumber('账户剩余的积分元素的 value 值', { domIncluded: true });
```
...
// 使用 domIncluded 功能提取 UI 中不可见的属性
const stringB = await agent.aiString('当前列表的第一条记录的跳转链接', { domIncluded: true });
```
