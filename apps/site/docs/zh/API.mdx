...
* 参数：
  ...
  * `opt?: Object` - 可选，一个配置对象，包含：
    * `cacheable?: boolean` - 当启用 [缓存功能](./caching.mdx) 时，是否允许缓存当前 API 调用结果。
...
* 参数：
  ...
  * `options?: Object` - 可选，一个配置对象，包含：
    * `deepThink?: boolean` - 是否开启深度思考。如果为 true，Midscene 会调用 AI 模型两次以精确定位元素。
    * `cacheable?: boolean` - 当启用 [缓存功能](./caching.mdx) 时，是否允许缓存当前 API 调用结果。 [缓存功能](./caching.mdx) 时，是否允许缓存当前 API 调用结果。
