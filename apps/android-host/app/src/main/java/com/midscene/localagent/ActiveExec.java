...
public final class ActiveExec {
    ...
    public static Status probe(Context context) {
        ...
        if (!CHANNEL_ADB.equals(channel(app))) {
            ...
            return new Status(CHANNEL_SHIZUKU, state.ready,
                    state.ready ? "Shizuku user service bound as uid " + state.uid
                            : Provisioner.describeBinding(state),
                    state);
        }
        ...
        if (!LocalAdbBackend.isPaired(app)) {
            ...
            return new Status(CHANNEL_ADB, false,
                    LocalAdbBackend.hasKey(app)
                            ? "Pairing was never completed. Pair this phone."
                            : "Not paired with this device yet. Pair once, then the "
                                    + "connection renews itself.", null);
        }
        if (LocalAdbBackend.serial(app).isEmpty()) {
            return new Status(CHANNEL_ADB, false,
                    "Paired, but no connection yet. Turn wireless debugging on and tap "
                            + "Reconnect.", null);
        }
        if (!LocalAdbBackend.reachable(app)) {
            return new Status(CHANNEL_ADB, false,
                    "adb is not reachable at " + LocalAdbBackend.serial(app)
                            + ". Keep wireless debugging on, then retry.", null);
        }
        return new Status(CHANNEL_ADB, true,
                "adb over loopback · " + LocalAdbBackend.serial(app) + " · uid 2000", null);
    }
    ...
    public static String describe(Context context) {
        ...
        if (!CHANNEL_ADB.equals(channel(app))) {
            ShizukuExecBridge.BindingState state = ShizukuExecBridge.probeBinding(app, 8_000);
            return "shizuku · " + (state.ready ? "uid " + state.uid
                    : Provisioner.describeBinding(state));
        }
        if (!LocalAdbBackend.isPaired(app)) {
            return "local adb · not paired yet";
        }
        String serial = LocalAdbBackend.serial(app);
        if (serial.isEmpty()) {
            return "local adb · paired, not connected yet";
        }
        return LocalAdbBackend.reachable(app)
                ? "local adb · " + serial + " · uid 2000"
                : "local adb · " + serial + " · unreachable";
    }
