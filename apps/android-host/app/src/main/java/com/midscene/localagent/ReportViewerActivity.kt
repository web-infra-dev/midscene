...
class ReportViewerActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val title = intent.getStringExtra(EXTRA_TITLE) ?: "Run artefact"
...
private fun MissingArtefact(path: String?) {
    Box(Modifier.fillMaxSize().padding(24.dp)) {
        Text("File not found: $path", style = MaterialTheme.typography.bodyMedium)
    }
}
...
private fun LogArtefact(file: File) {
    val text = remember(file.path) { readLog(file) }
    ArtefactPanel {
        Box(
...
private fun readLog(file: File): String = try {
