...
public final class AdbMdns {
            ...
            public void onServiceFound(NsdServiceInfo info) {
                manager.resolveService(info, new NsdManager.ResolveListener() {
                    ...
                    public void onServiceResolved(NsdServiceInfo resolved) {
                        ...
                        Log.i(TAG, "resolved " + resolved.getServiceName() + " at "
    ...
    /** How many discovery rounds one lookup may spend, and the pause between them. */
    private static final int LOOKUP_ATTEMPTS = 3;
    private static final long LOOKUP_PAUSE_MS = 1_500;
     ...
     * just fails the same way. This is the difference between the first attempt at
     * something and the second one working.
     */
    public static String resolveBlocking(Context context, String serviceType, long timeoutMs) {
        for (int attempt = 0; attempt < LOOKUP_ATTEMPTS; attempt += 1) {
            if (attempt > 0) {
                try {
            ...
            String found = resolveOnce(context, serviceType, timeoutMs);
            if (found != null) {
                return found;
            }
            Log.i(TAG, serviceType + ": no answer on round " + (attempt + 1)
                    + "/" + LOOKUP_ATTEMPTS);
        }
        return null;
    }
    ...
    /**
     * One discovery, one answer.
     *
     * Separate from {@link #resolveBlocking} because a caller sometimes wants a single
     * long round rather than several short ones: while the user walks to the pairing
     * dialog, the listener has to still be up when they get there.
     */
    public static String resolveOnce(Context context, String serviceType, long timeoutMs) {
        {
