  ...
  ({ serverUrl = 'http://localhost:1412', autoConnect = false }, ref) => {
    ...
    const autoDetectMirror = async () => {
      ...
      try {
        ...
        messageApi.error(
          `Auto-detection error: ${error instanceof Error ? error.message : 'Unknown error'}`,
        );
      } finally {
          ...
          {connected && mirrorConfig && mirrorConfig.enabled && (
            ...
            >
              <Text style={{ fontSize: '12px', color: '#52c41a' }}>
                ✅ Configured: {mirrorConfig.estimated_ios_width}×
                {mirrorConfig.estimated_ios_height} device →{' '}
                {mirrorConfig.mirror_width}×{mirrorConfig.mirror_height} at (
                {mirrorConfig.mirror_x}, {mirrorConfig.mirror_y})
              </Text>
            </div>
          )}
